...
test('AQ GPS diagnostics remain present in the 10.13.140 deployment',()=>{
  assert.match(html,/id="nitrosGpsDiagnostic"/);
  assert.match(html,/nitros-single-build-and-service-worker-authority/);
  assert.match(html,/Authoritative Connection-State Reconciliation/);
  assert.match(sw,/const VERSION = '10\.13\.140'/);
});
