...
test('AQ GPS diagnostics remain present in the 10.13.142 deployment',()=>{
  assert.match(html,/id="nitrosGpsDiagnostic"/);
  assert.match(html,/nitros-single-build-and-service-worker-authority/);
  assert.match(html,/AI Usage Live Verification & Cost Calibration/);
  assert.match(sw,/const VERSION = '10\.13\.142'/);
});
