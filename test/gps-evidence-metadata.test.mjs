...
test('AQ GPS diagnostics remain present in the 10.13.144 deployment',()=>{
  assert.match(html,/id="nitrosGpsDiagnostic"/);
  assert.match(html,/nitros-single-build-and-service-worker-authority/);
  assert.match(html,/Vercel Upstash Environment Compatibility/);
  assert.match(sw,/const VERSION = '10\.13\.144'/);
});
