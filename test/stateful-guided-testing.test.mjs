...
test('10.12.64 electrical shop-language observations map through the shared contextual interpreter',()=>{
...
test('V9 incomplete synchronized document extraction remains at repair-information gate and names exact missing fields',()=>{
  const h=atCorrelation();h.handleGuidedFinding('Correlation passed');h.state.intakeStep='complete';const guided=h.state.diagnosticTestState,runId='doc-incomplete';h.handleRepairInformationImport({kind:'image-analysis',fileName:'partial-oem-screen.png',imageHash:'fresh-hash',analysis:{runId,category:'DOCUMENT_OR_TEXT_SCREENSHOT',documentRepairInformation:{extractionRunId:runId,verificationRunId:runId,synchronizationStatus:'PASS',extractionStatus:'INCOMPLETE',verifiedRepairInformationStatus:'PENDING',freshResultVerification:'PASS',testName:'Cam Reference Voltage Test',componentOrCircuit:'cam sensor',testLocation:'',method:'measure voltage with key ON',criterion:'',missingRequiredFields:['test location','criterion']}}});assert.equal(guided.currentTestId,'verified-repair-information-required');assert.equal(h.state.pendingRepairInformation.eligible,false);assert.match(h.state.lastReply,/Missing or unsupported: test location, criterion/i);assert.equal(h.state.repairInformationRequired,true)
});

test('V9 complete synchronized document test automatically transitions the gate to PASS from canonical extraction',()=>{
  const h=atCorrelation();h.handleGuidedFinding('Correlation passed');h.state.intakeStep='complete';const runId='document-fresh';assert.equal(h.handleRepairInformationImport({kind:'image-analysis',fileName:'complete-oem-screen.png',imageHash:'fresh-hash',analysis:{runId,category:'DOCUMENT_OR_TEXT_SCREENSHOT',documentRepairInformation:{extractionRunId:runId,verificationRunId:runId,synchronizationStatus:'PASS',extractionStatus:'COMPLETE',verifiedRepairInformationStatus:'VERIFIED',freshResultVerification:'PASS',dtcs:[],testName:'Cam Reference Voltage Test',componentOrCircuit:'cam sensor',testLocation:'connector terminal 3',method:'measure voltage with key ON',criterion:'4.8–5.2 volts',comparator:'range',minimum:4.8,maximum:5.2,missingRequiredFields:[]}}}),true);assert.equal(h.state.pendingRepairInformation.eligible,true);assert.equal(h.state.pendingRepairInformation.canonicalExtraction.criterion,'4.8–5.2 volts');assert.equal(h.state.diagnosticTestState.tests.find(test=>test.id==='verified-repair-information-required').status,'pass');assert.equal(h.state.repairInformation.status,'verified');assert.equal(h.state.diagnosticTestState.currentTestId,'verified-document-document-fresh');assert.match(h.state.lastReply,/4\.8–5\.2 volts/)
});

test('V9 contradiction guard fails closed when extraction and verification disagree',()=>{
  const h=atCorrelation();h.handleGuidedFinding('Correlation passed');h.state.intakeStep='complete';const runId='contradiction-run';h.handleRepairInformationImport({kind:'image-analysis',fileName:'contradictory.png',imageHash:'fresh-hash',analysis:{runId,category:'DOCUMENT_OR_TEXT_SCREENSHOT',documentRepairInformation:{extractionRunId:runId,verificationRunId:runId,synchronizationStatus:'PASS',extractionStatus:'COMPLETE',freshResultVerification:'PASS',testName:'Cam Reference Voltage Test',componentOrCircuit:'cam sensor',testLocation:'connector terminal 3',method:'measure voltage with key ON',criterion:'',missingRequiredFields:[]}}});assert.equal(h.state.pendingRepairInformation.eligible,false);assert.match(h.state.lastReply,/INTERNAL STATE MISMATCH/);assert.match(h.state.lastReply,/verification reports criterion/);assert.equal(h.state.diagnosticTestState.currentTestId,'verified-repair-information-required')
});
