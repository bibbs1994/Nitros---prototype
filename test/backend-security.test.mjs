...
test('10.12.32 fails closed on inconsistent LTFT while retaining valid sibling PID evidence',()=>{
  ...
  const ltft=corrected.numericEvidence.find(item=>item.pidName==='Long FT #1'),stftEvidence=corrected.numericEvidence.find(item=>item.pidName==='Short FT #1'),text=corrected.interpretation.join(' ');assert.equal(ltft.current,6.249);assert.equal(ltft.validityState,'NUMERIC_INCONSISTENCY');assert.equal(ltft.inconsistencyReason,'CURRENT_ABOVE_MAX');assert.equal(stftEvidence.validityState,'VALID');assert.equal(stftEvidence.crossesZero,true);assert.equal(corrected.reasoningEvidence.combinedTrim,null);assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');assert.equal(corrected.diagnosticSignificanceReason,'NUMERIC_EVIDENCE_INCONSISTENCY_DETECTED');assert.doesNotMatch(text,/combined Bank 1|Long FT #1 shows intermittent|Short FT #1 crosses zero/i);assert.match(text,/interpretation stopped/i);assert.match(corrected.nextTest[0],/Re-read or recapture the inconsistent PID evidence/i);assert.deepEqual(corrected.numericValidation.invalidPidEvidence,['Long FT #1']);assert.equal(corrected.numericValidation.currentMinMaxConsistency,'FAIL');assert.equal(corrected.numericValidation.dependentInterpretationSuppressed,'PASS');assert.equal(corrected.freshResultVerification,'FAIL');
});

test('10.12.32 distinguishes Min greater than Max from Current outside range',()=>{
  const corrected=correctAutomotiveGraphReasoning({status:'PARTIAL',confidence:97,observed:['Engine Speed Current: 2 RPM','Engine Speed Min: 5 RPM','Engine Speed Max: 1 RPM'],interpretation:['Engine Speed is significant.'],diagnosticSignificance:'SIGNIFICANT',nextTest:[],pidNames:['Engine Speed'],sensorNames:[],valuesAndScales:[],traceFindings:[],unreadableOrUncertain:[],visibleVehicle:{description:'',evidence:[]}}),evidence=corrected.numericEvidence[0];assert.equal(evidence.inconsistencyReason,'MIN_GREATER_THAN_MAX');assert.equal(evidence.validityState,'NUMERIC_INCONSISTENCY');assert.match(corrected.evidenceConsistencyFailures[0].failure,/minimum is greater than recorded maximum/i);assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');
});
...
test('10.12.34 hard gate reports exact Current above Max failure and bypasses normal reasoning',()=>{
  const corrected=correctAutomotiveGraphReasoning({status:'PARTIAL',confidence:98,observed:['Long FT #1 Current: +6.249%','Long FT #1 Min: −6.249%','Long FT #1 Max: +5.467%','plotted traces visible across horizontal axis'],interpretation:['Long FT #1 is significant.','O2S B1S2 responds normally.'],diagnosticSignificance:'SIGNIFICANT',nextTest:['Add Fuel System Status / Closed Loop Status.'],pidNames:['Long FT #1','O2S B1S2'],sensorNames:[],valuesAndScales:[],traceFindings:['Long FT #1 plotted trace varies','O2S B1S2 plotted trace responds'],classifierGraphEvidence:['axes gridlines plotted traces'],unreadableOrUncertain:[],visibleVehicle:{description:'',evidence:[]}}),failure=corrected.evidenceConsistencyFailures[0];assert.equal(corrected.freshResultVerification,'FAIL');assert.equal(corrected.semanticConsistencyStatus,'FAIL_NUMERIC_EVIDENCE');assert.equal(failure.pidName,'Long FT #1');assert.equal(failure.current,'+6.249');assert.equal(failure.minimum,'−6.249');assert.equal(failure.maximum,'+5.467');assert.equal(failure.failureCode,'CURRENT_ABOVE_MAX');assert.match(failure.failure,/exceeds recorded maximum/i);assert.deepEqual(corrected.traceFindings,[]);assert.doesNotMatch(corrected.interpretation.join(' '),/Long FT|O2S|normal|significant/i);assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');assert.equal(corrected.nextTestSelection,'CORRECTIVE_ACQUISITION_ONLY');assert.match(corrected.nextTest[0],/Re-read or recapture/i);
});
