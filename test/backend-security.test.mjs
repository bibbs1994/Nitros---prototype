    ...
    const result = await analyzeSemanticImage(body, {
      ...
      fetchImpl: async (_url, options) => {
        ...
        const payload = calls === 1
          ...
          : { status: 'NO_VISIBLE_CONCERN_DETECTED', conditionConfidence: 0.82, observedCondition: [], possibleConcerns: [], connectionAssessments: [{location:'Alternator belt path',seatingStatus:'NO_GAP_OR_SEPARATION_VISIBLE',findingType:'NO_DEFECT_VISIBLE',severity:'LOW',findingConfidence:82,visibleEvidence:'The belt is fully seated in the pulley grooves and the complete belt-to-pulley interface shows no abnormal separation.',matingComponentVisible:true,directDamageVisible:false,missingContext:null,recommendedVerification:'Physically inspect the belt path before repair authorization.',safetyDrivabilityImpact:null}], noVisibleConcernMessage: 'No visible defect can be confirmed from this image. Inspect the component physically before making a repair decision.', unableToInspectReason: null, visibleEvidence: ['Alternator housing and the complete belt-to-pulley interface are visible.'], recommendedVerification: ['Inspect the alternator and belt physically before making a repair decision.'], safetyDrivabilityImpact: null };
