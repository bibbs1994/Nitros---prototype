...
test('10.12.32 fails closed on inconsistent LTFT while retaining valid sibling PID evidence',()=>{
  ...
  const ltft=corrected.numericEvidence.find(item=>item.pidName==='Long FT #1'),stftEvidence=corrected.numericEvidence.find(item=>item.pidName==='Short FT #1'),text=corrected.interpretation.join(' ');assert.equal(ltft.current,6.249);assert.equal(ltft.validityState,'NUMERIC_INCONSISTENCY');assert.equal(ltft.inconsistencyReason,'CURRENT_OUTSIDE_MIN_MAX');assert.equal(stftEvidence.validityState,'VALID');assert.equal(stftEvidence.crossesZero,true);assert.equal(corrected.reasoningEvidence.combinedTrim,null);assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');assert.equal(corrected.diagnosticSignificanceReason,'PENDING_NUMERIC_VERIFICATION');assert.doesNotMatch(text,/combined Bank 1|Long FT #1 shows intermittent/i);assert.match(text,/Long FT #1 numeric evidence is internally inconsistent/i);assert.match(text,/Short FT #1 crosses zero/i);assert.match(corrected.nextTest[0],/Reconfirm Long FT #1 Current\/Min\/Max/i);assert.deepEqual(corrected.numericValidation.invalidPidEvidence,['Long FT #1']);assert.equal(corrected.numericValidation.currentMinMaxConsistency,'FAIL');assert.equal(corrected.numericValidation.dependentInterpretationSuppressed,'PASS');
});

test('10.12.32 distinguishes Min greater than Max from Current outside range',()=>{
  const corrected=correctAutomotiveGraphReasoning({status:'PARTIAL',confidence:97,observed:['Engine Speed Current: 2 RPM','Engine Speed Min: 5 RPM','Engine Speed Max: 1 RPM'],interpretation:['Engine Speed is significant.'],diagnosticSignificance:'SIGNIFICANT',nextTest:[],pidNames:['Engine Speed'],sensorNames:[],valuesAndScales:[],traceFindings:[],unreadableOrUncertain:[],visibleVehicle:{description:'',evidence:[]}}),evidence=corrected.numericEvidence[0];assert.equal(evidence.inconsistencyReason,'MIN_GREATER_THAN_MAX');assert.equal(evidence.validityState,'NUMERIC_INCONSISTENCY');assert.match(corrected.interpretation.join(' '),/reported Min is greater than Max/i);assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');
});
