...
test('10.12.34 accepts valid signed ranges and rejects Current below Min',()=>{
  ...
  const below=correctAutomotiveGraphReasoning({status:'PARTIAL',confidence:98,observed:['Long FT #1 Current: −7.0%','Long FT #1 Min: −6.0%','Long FT #1 Max: +6.0%'],interpretation:['Long FT is abnormal.'],diagnosticSignificance:'SIGNIFICANT',nextTest:[],pidNames:['Long FT #1'],sensorNames:[],valuesAndScales:[],traceFindings:[],unreadableOrUncertain:[],visibleVehicle:{description:'',evidence:[]}});assert.equal(below.freshResultVerification,'FAIL');assert.equal(below.evidenceConsistencyFailures[0].failureCode,'CURRENT_BELOW_MIN');assert.equal(below.evidenceConsistencyFailures[0].failure,'Current is below Min.');
});
...
test('10.12.36 one invalid PID fails the complete evidence set while valid evidence retains 10.12.35 routing',()=>{
  ...
  const failed=correctAutomotiveGraphReasoning(base);assert.equal(failed.evidenceConsistencyFailures.length,1);assert.equal(failed.freshResultVerification,'FAIL');assert.deepEqual(failed.interpretation,[]);assert.deepEqual(failed.nextTest,[]);
