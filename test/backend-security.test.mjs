...
test('10.12.11 validation case corrects O2, A/F, trim, operating-state, snapshot, and catalyst reasoning',()=>{
  ...
  const text=corrected.interpretation.join(' ');assert.equal(corrected.reasoningEvidence.combinedTrim,-8.591);assert.equal(corrected.reasoningEvidence.fuelTrimPolarity,'NEGATIVE_PCM_REMOVING_FUEL');assert.equal(corrected.reasoningEvidence.operatingState,'WARM_NEAR_IDLE');assert.equal(corrected.reasoningEvidence.dynamicTraceEvidenceAvailable,false);assert.equal(corrected.diagnosticSignificance,'INCONCLUSIVE');assert.match(text,/high-voltage\/rich-side/);assert.match(text,/removing approximately 8\.6% fuel/);assert.match(text,/one value cannot verify sensor performance/i);assert.match(text,/Insufficient dynamic graph evidence/);assert.doesNotMatch(text,/low and lean|vacuum leak|sensor is good|no irregularities/i);assert.match(corrected.nextTest[0],/upstream A\/F or O2 and downstream O2 traces together/);
});
