...
test('10.12.13 static snapshot guard removes temporal inference and defaults significance to indeterminate',()=>{
  ...
  const text=corrected.interpretation.join(' ');assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');assert.equal(corrected.reasoningEvidence.dynamicTraceEvidenceAvailable,false);assert.equal(corrected.reasoningEvidence.temporalEvidenceSource,'STATIC_SNAPSHOT_ONLY');assert.match(text,/O2S B1S2 is currently 0\.055 V/);assert.match(text,/single captured value does not establish mixture state, sensor switching behavior, sensor health, or catalyst efficiency/i);assert.doesNotMatch(text,/minimal activity|stable|unstable|responding|stuck|biased|trending|fluctuating|mildly abnormal/i);assert.match(corrected.nextTest[0],/simultaneously in closed loop over time/);assert.match(corrected.unreadableOrUncertain.join(' '),/Temporal behavior is unreadable or unavailable/);
});
