...
test('10.12.11 validation case corrects O2, A/F, trim, operating-state, snapshot, and catalyst reasoning',()=>{
  ...
  const text=corrected.interpretation.join(' ');assert.equal(corrected.reasoningEvidence.combinedTrim,-8.591);assert.equal(corrected.reasoningEvidence.fuelTrimPolarity,'NEGATIVE_PCM_REMOVING_FUEL');assert.equal(corrected.reasoningEvidence.operatingState,'WARM_NEAR_IDLE');assert.equal(corrected.reasoningEvidence.dynamicTraceEvidenceAvailable,false);assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');assert.match(text,/high-voltage\/rich side/);assert.match(text,/approximately 8\.6% fuel removal/);assert.match(text,/one value cannot verify sensor performance/i);assert.match(text,/Insufficient dynamic graph evidence/);assert.doesNotMatch(text,/low and lean|vacuum leak|sensor is good|no irregularities/i);assert.match(corrected.nextTest[0],/upstream A\/F sensor and downstream O2 sensor data simultaneously/i);
});
...
test('10.12.13 static snapshot guard removes temporal inference and defaults significance to indeterminate',()=>{
  ...
  const text=corrected.interpretation.join(' ');assert.equal(corrected.diagnosticSignificance,'INDETERMINATE');assert.equal(corrected.reasoningEvidence.dynamicTraceEvidenceAvailable,false);assert.equal(corrected.reasoningEvidence.temporalEvidenceSource,'STATIC_SNAPSHOT_ONLY');assert.match(text,/B1S2 is approximately 0\.055 V at the captured instant/);assert.match(text,/single frame does not establish sensor switching, mixture-control behavior, sensor health, or catalyst efficiency/i);assert.doesNotMatch(text,/minimal activity|stable|unstable|oscillat|responding|stuck|biased|trending|fluctuating|mildly abnormal/i);assert.match(corrected.nextTest[0],/simultaneously in closed loop over time/);assert.match(corrected.unreadableOrUncertain.join(' '),/Temporal behavior is unreadable or unavailable/);
});
