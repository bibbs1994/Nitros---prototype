...
test('10.12.28 keeps the proven analyzer and production endpoint', () => {
  ...
  assert.match(serviceWorker, /const VERSION = '10\.12\.40'/);
...
test('10.12.40 final shared triplet gate validates and renders one frozen canonical object',()=>{
...
test('10.12.40 follows final displayed rounding and sign values',()=>{
...
test('10.12.40 reproduces the live dual fuel-trim failure safely',()=>{
...
test('10.12.40 shared numeric source covers missing, zero, signed, decimal, voltage, and RPM cases',()=>{
  const start=analyzer.indexOf('  function canonicalSourceNumber('),end=analyzer.indexOf('  function finalizeRenderedNumericEvidence(',start),context={};vm.runInNewContext(`${analyzer.slice(start,end)};this.make=createRenderedNumericTriplet;this.assertRows=assertFinalRenderedPidEvidence;`,context);const make=(current,minimum,maximum,unit='%')=>context.make({pidName:'PID',unit,current,minimum,maximum});const cases=[['valid positive',6.249,5.467,7.03,'PASS'],['valid negative',-6.249,-7.03,-5.467,'PASS'],['above max',8,2,7,'FAIL'],['below min',1,2,3,'FAIL'],['min above max',3,5,4,'FAIL'],['missing current',null,2,7,'FAIL'],['missing min',6,null,7,'FAIL'],['missing max',6,5,null,'FAIL'],['zero',0,0,0,'PASS']];for(const [name,current,minimum,maximum,expected] of cases)assert.equal(make(current,minimum,maximum).invariantResult,expected,name);
  ...
  const fallback=context.make({pidName:'Long FT #1',unit:'%',rawCurrent:'+6.249%',numericRange:{minimum:5.467,maximum:7.03}});assert.equal(fallback.currentText,'+6.249%');assert.equal(fallback.minText,'+5.467%');assert.equal(fallback.maxText,'+7.030%');assert.equal(fallback.invariantResult,'PASS');const assertion=context.assertRows(Object.freeze([fallback]));assert.strictEqual(assertion.renderedPidEvidence[0],fallback);
});

test('10.12.40 renderer reasserts shared triplets before allowing PASS',()=>{
...
test('AO wiring parser defensively normalizes legacy semantic field shapes', () => {
  ...
  assert.match(html, /version:'10\.12\.40'/);
});
