...
test('BF app build keeps the proven AO analyzer and production endpoint', () => {
  assert.match(analyzer, /const BUILD='10\.12\.21'/);
  assert.match(html, /10\.12\.21/);
  ...
  assert.match(serviceWorker, /const VERSION = '10\.12\.21'/);
...
test('10.12.21 preserves supported semantic response shapes and one transient unusable-result retry',()=>{
...
test('AO wiring parser defensively normalizes legacy semantic field shapes', () => {
  ...
  assert.match(html, /version:'10\.12\.21'/);
});
