...
test('10.12.28 keeps the proven analyzer and production endpoint', () => {
  ...
  assert.match(serviceWorker, /const VERSION = '10\.12\.39'/);
...
test('10.12.39 final rendered-value gate validates and renders one frozen canonical PID object',()=>{
  const start=analyzer.indexOf('  function parseFinalRenderedNumber('),end=analyzer.indexOf('  window.NitrosValidateFinalRenderedPid=',start);assert.ok(start>=0&&end>start);const context={};vm.runInNewContext(`${analyzer.slice(start,end)};this.gate=finalizeRenderedNumericEvidence;`,context);
  ...
  const failed=context.gate(graph),ltft=failed.finalCanonicalPidEvidence[0];assert.equal(failed.finalRenderValidationStatus,'FAIL');assert.equal(failed.freshResultVerification,'FAIL');assert.deepEqual(Array.from(failed.finalCanonicalPidEvidence,row=>row.invariantResult),['FAIL','PASS','PASS','PASS','PASS']);assert.equal(ltft.currentDisplay,'6.249');assert.equal(ltft.maxDisplay,'5.467');assert.equal(ltft.parsedCurrent,6.249);assert.equal(ltft.parsedMax,5.467);assert.equal(ltft.invariantFailureReason,'Rendered Current exceeds rendered Max.');assert.ok(Object.isFrozen(failed.renderedPidEvidence)&&failed.renderedPidEvidence.every(Object.isFrozen));assert.match(failed.interpretation.join(' '),/Long FT #1 displays Current 6\.249, Min -6\.249, and Max 5\.467/);assert.doesNotMatch(failed.traceFindings.join(' '),/Long FT|LTFT/i);assert.match(failed.nextTest[0],/Fuel System Status \/ Closed Loop Status/);assert.match(failed.nextTestReason,/Rendered Current exceeds rendered Max/i);assert.equal(failed.numericValidation.authoritativeSource,'EXACT_FINAL_RENDERED_PID_STRINGS');assert.equal(failed.renderedInvariantLog[0].currentLessThanOrEqualMaximum,'FAIL');
...
test('10.12.39 follows final displayed rounding and sign values',()=>{
  const start=analyzer.indexOf('  function parseFinalRenderedNumber('),end=analyzer.indexOf('  window.NitrosValidateFinalRenderedPid=',start),context={};vm.runInNewContext(`${analyzer.slice(start,end)};this.gate=finalizeRenderedNumericEvidence;`,context);const row=(current,minimum,maximum)=>({pidName:'Long FT #1',current,minimum,maximum,unit:'%',currentPresent:true,minimumPresent:true,maximumPresent:true});
  const roundedPass=context.gate({numericEvidence:[row(1.00049,1.0004,1.00048)],interpretation:[],traceFindings:[],numericValidation:{},freshResultVerification:'PASS'});assert.equal(roundedPass.renderedPidEvidence[0].currentDisplay,'1.000');assert.equal(roundedPass.finalRenderValidationStatus,'PASS');
  const roundedFail=context.gate({numericEvidence:[row(1.00051,1.00049,1.0005)],interpretation:['Long FT #1 is rising.'],traceFindings:['Long FT #1 rises over time.'],numericValidation:{},freshResultVerification:'PASS'});assert.equal(roundedFail.renderedPidEvidence[0].currentDisplay,'1.001');assert.equal(roundedFail.renderedPidEvidence[0].maxDisplay,'1.000');assert.equal(roundedFail.finalRenderValidationStatus,'FAIL');assert.deepEqual(Array.from(roundedFail.traceFindings),[]);
  const signFailure=context.gate({numericEvidence:[row(-6,-5,-4)],interpretation:[],traceFindings:[],numericValidation:{},freshResultVerification:'PASS'});assert.equal(signFailure.renderedPidEvidence[0].currentDisplay,'-6.000');assert.equal(signFailure.renderedPidEvidence[0].parsedCurrent,-6);assert.equal(signFailure.finalRenderValidationStatus,'FAIL');
});

test('10.12.39 reproduces the live dual fuel-trim failure safely',()=>{
...
test('10.12.39 strict post-render parser covers every mandatory final-string invariant case',()=>{
  const start=analyzer.indexOf('  function parseFinalRenderedNumber('),end=analyzer.indexOf('  function finalizeRenderedNumericEvidence(',start),context={};vm.runInNewContext(`${analyzer.slice(start,end)};this.validate=validateFinalRenderedPid;this.assertRows=assertFinalRenderedPidEvidence;`,context);const cases=[
    ['valid positive','6.249','5.467','7.030','PASS'],['valid negative','-6.249','-7.030','-5.467','PASS'],['exact 10.12.38 live failure','-6.249','6.249','-5.467','FAIL'],['current above maximum','8.000','2.000','7.000','FAIL'],['minimum above maximum','3.000','5.000','4.000','FAIL'],['boundary equality','5.000','5.000','5.000','PASS'],['current equals minimum','2.342','2.342','3.125','PASS'],['current equals maximum','1.563','-2.342','1.563','PASS'],['malformed rendered value','--6.249','-7.000','-5.000','FAIL'],['missing value','6.249','','7.030','FAIL']
  ];for(const [name,currentDisplay,minDisplay,maxDisplay,expected] of cases){const result=context.validate({pidName:'Long FT #1',unit:'%',currentDisplay,minDisplay,maxDisplay});assert.equal(result.invariantResult,expected,name)}
  const live=context.assertRows([{pidName:'Long FT #1',unit:'%',currentDisplay:'-6.249',minDisplay:'6.249',maxDisplay:'-5.467'}]);assert.equal(live.status,'FAIL');assert.equal(live.failed[0].parsedCurrent,-6.249);assert.equal(live.failed[0].parsedMin,6.249);assert.equal(live.failed[0].minimumLessThanOrEqualCurrent,false);assert.equal(live.failed[0].minimumLessThanOrEqualMaximum,false);
});

test('10.12.39 renderer reasserts scheduled display strings before allowing PASS',()=>{
...
test('AO wiring parser defensively normalizes legacy semantic field shapes', () => {
  ...
  assert.match(html, /version:'10\.12\.39'/);
});
