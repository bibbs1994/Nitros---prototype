...
test('AY app build keeps the proven AO analyzer and production endpoint', () => {
  assert.match(analyzer, /const BUILD='10\.12\.7AO'/);
  assert.match(html, /10\.12\.7AY/);
  ...
  assert.match(serviceWorker, /const VERSION = '10\.12\.7AY'/);
...
test('AO wiring parser defensively normalizes legacy semantic field shapes', () => {
  ...
  assert.match(html, /version:'10\.12\.7AY'/);
});
