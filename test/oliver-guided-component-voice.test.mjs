...
const mixedIntentHarness=Function(`let state={vehicle:{year:'',make:'',model:'',engine:'',configuration:''},complaint:'',symptoms:'',normalizedSymptom:'',system:'',component:'',diagnosticDomain:'',previousTests:'',technicianObservations:[],liveData:'',intakeStep:'vehicle',stage:'vehicle'},guidance={measurements:[],evidence:[]},reply='';function normalize(text){return String(text||'')}function guidanceState(){return guidance}function vehicleLabel(){return[state.vehicle.year,state.vehicle.make,state.vehicle.model,state.vehicle.engine].filter(Boolean).join(' ')}function selectGuidanceTest(id,name,reason,evidence){guidance.selectedNextTest={id,name};guidance.nextTestReason=reason;guidance.nextRequiredEvidence=evidence}function ask(text){reply=text}${extract('parseVehicle','concern')};${extract('normalizeMixedIntentNumbers','contextualNumericSpeechRecovery')};return{text:text=>handleMixedIntentDiagnosticEvidence(text),state,guidance,get reply(){return reply}}`)();
...
test('10.12.65 active blower question recognizes numbered, high-only, all, none, and partial shop language',()=>{
...
test('10.12.65 accepts terse active-question shorthand without promoting hypotheses or ambiguity',()=>{
...
test('10.12.65 contextually recovers merged speed speech while protecting decimals and pin identifiers',()=>{
...
test('10.12.65 structures commanded state separately from observed blower response',()=>{
...
test('10.12.65 commits vehicle and three contextual measurements from one utterance',()=>{
...
test('10.12.65 natural spoken decimals retain measurement context and vehicle continuity',()=>{
...
test('10.12.65 vehicle-only input remains outside mixed-intent evidence routing',()=>{
...
test('10.12.65 normalizes synonymous blower objectives into one authoritative test projection',()=>{
...
test('10.12.65 mixed result and measurement routing preserves both clauses',()=>{
...
test('10.12.65 commits conversational observations before advancing and guards duplicates and conflicts',()=>{
