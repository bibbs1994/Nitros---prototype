...
const matrixSyncHarness=initial=>Function('initial',`let state=JSON.parse(JSON.stringify(initial.state)),guidance=JSON.parse(JSON.stringify(initial.guidance));function guidanceState(){return guidance}${extractRaw('function synchronizeBlowerOperatingEvidenceMatrix','function renderAll')}return{count:synchronizeBlowerOperatingEvidenceMatrix(),state,guidance}`)(initial);
const atomicBlowerNormalizer=Function(`${extract('normalizeAtomicBlowerUtterance','handleAtomicUtteranceEvidence')};return normalizeAtomicBlowerUtterance`)();
const canonicalSyncHarness=initial=>Function('initial',`let state=JSON.parse(JSON.stringify(initial.state)),guidance=JSON.parse(JSON.stringify(initial.guidance));function guidanceState(){return guidance}${extract('normalizeAuthoritativeEvidenceAndActiveTest','renderAll')}return{result:normalizeAuthoritativeEvidenceAndActiveTest(),state,guidance}`)(initial);
const unitSyncHarness=initial=>Function('initial',`let state=JSON.parse(JSON.stringify(initial.state)),guidance=JSON.parse(JSON.stringify(initial.guidance));function guidanceState(){return guidance}function activeCommandedBlowerSpeed(){const value=state.currentCommandedSpeed||state.current_commanded_speed||'';return /^(low|medium|high)$/i.test(value)?String(value).toUpperCase():''}${extract('normalizeUnitAwareNumericEvidence','renderAll')}return{measurement:normalizeUnitAwareNumericEvidence(),state,guidance}`)(initial);

test('10.12.68 primary utterance additively retains vehicle, complaint, operating states, measurement, and active routing',()=>{
  const h=additiveEvidenceHarness();assert.equal(h.apply("2016 Jeep Wrangler high speed works low and medium don't the control signal is still 7.2 V."),true);assert.deepEqual([h.state.vehicle.year,h.state.vehicle.make,h.state.vehicle.model],['2016','Jeep','Wrangler']);assert.match(h.state.complaint,/high blower speed operative/i);assert.match(h.state.complaint,/medium blower speed inoperative/i);assert.match(h.state.complaint,/low blower speed inoperative/i);assert.equal(h.state.normalizedSymptom,'HVAC blower high speed operative / low and medium blower speeds inoperative');assert.equal(h.state.diagnosticDomain,'HVAC / Blower Diagnostic');const operating=h.guidance.evidence.find(item=>item.type==='operating-state');assert.deepEqual(operating.facts.speedStates,{high:'OPERATIVE',medium:'INOPERATIVE',low:'INOPERATIVE'});assert.deepEqual(h.state.blowerSpeedStates,{low:'INOPERATIVE',medium:'INOPERATIVE',high:'OPERATIVE'});assert.deepEqual(h.state.blowerCommandResponseEvidence.map(item=>[item.commandedSpeed,item.actualBlowerOperation,item.controlSignalAssociated]),[['LOW','INOPERATIVE',false],['MEDIUM','INOPERATIVE',false],['HIGH','OPERATIVE',false]]);assert.equal(h.state.blowerCommandResponseEvidence.length,3);assert.equal(h.state.blowerOperatingPattern,'HIGH_ONLY');assert.equal(h.state.blowerCommandResponseCorrelation,'AWAITING_MULTI_SPEED_CONTROL_MEASUREMENTS');assert.equal(h.state.componentCondemned,'None');assert.equal(h.state.diagnosticConclusionState,'UNCONFIRMED');const measurement=h.guidance.measurements.find(item=>item.testType==='control signal');assert.deepEqual([measurement.value,measurement.unit,measurement.source],[7.2,'V','technician-speech']);assert.equal(h.state.stage,'diagnostic');assert.equal(h.state.intakeStep,'complete');assert.equal(h.state.authoritativeDiagnosticTest.testId,'blower-command-response-correlation');assert.equal(h.state.routingDiagnostics.routingDecision,'ACTIVE_GUIDED_DIAGNOSTIC_EVIDENCE_EVALUATION');assert.equal(h.state.routingDiagnostics.authoritativeSymptomPromoted,'YES');assert.equal(h.state.routingDiagnostics.blowerCommandResponseObservationCount,3);assert.match(h.guidance.nextTestReason,/high blower speed operative.*7\.2 V control signal.*without a commanded-speed association/i);assert.match(h.guidance.nextRequiredEvidence,/control-signal.*low, medium, and high.*already established/i);assert.match(h.reply,/retained the 7\.2-volt control-signal measurement.*which blower speed was commanded.*low, medium, or high/i);assert.doesNotMatch(h.reply,/which speeds work|whether high works|whether medium works|whether low works|diagnostic code|what vehicle|what.*symptom/i);
});
...
test('10.12.74 evaluates changing multi-speed signal evidence without replacing the high-only complaint',()=>{
  const h=additiveEvidenceHarness();assert.equal(h.apply("2016 Jeep Wrangler high speed works low and medium don't the control signal is still 7.2 V."),true);const complaint=h.state.complaint,baselineMeasurement=h.guidance.measurements.find(item=>item.testType==='control signal');assert.equal(h.matrix("On low the blower doesn't run and the control signal is 7.2 volts. On medium it doesn't run and the signal is 7.2 volts. On high the blower runs and the signal changes to 1.1 volts."),true);assert.deepEqual(h.state.blowerCommandResponseEvidence.map(item=>[item.commandedSpeed,item.controlSignalValue,item.controlSignalUnit,item.actualBlowerOperation]),[['LOW',7.2,'V','INOPERATIVE'],['MEDIUM',7.2,'V','INOPERATIVE'],['HIGH',1.1,'V','OPERATIVE']]);assert.equal(h.state.blowerCommandResponseEvidence.length,3);assert.equal(h.state.blowerCommandResponseCorrelation,'COMMAND_RESPONSE_CORRELATION_EVALUATED_SIGNAL_CHANGES_WITH_RESPONSE');assert.equal(h.state.routingDiagnostics.commandResponseCorrelationEvaluated,'YES');assert.equal(h.state.routingDiagnostics.sameCommandResponseTestSuppressed,'YES');assert.match(h.state.previousTests,/Control signal: 7\.2 V.*LOW: Control signal 7\.2 V.*MEDIUM: Control signal 7\.2 V.*HIGH: Control signal 1\.1 V/);assert.equal(h.state.complaint,complaint);assert.equal(h.state.normalizedSymptom,'HVAC blower high speed operative / low and medium blower speeds inoperative');assert.equal(h.state.diagnosticDomain,'HVAC / Blower Diagnostic');assert.equal(h.state.blowerOperatingPattern,'HIGH_ONLY');assert.equal(h.state.componentCondemned,'None');assert.equal(h.state.diagnosticConclusionState,'UNCONFIRMED');assert.equal(baselineMeasurement.value,7.2);assert.equal(h.guidance.completedTests.length,3);assert.match(h.guidance.nextTestReason,/LOW is 7\.2 V.*MEDIUM is 7\.2 V.*HIGH is 1\.1 V.*correlation is evaluated/i);assert.match(h.guidance.nextRequiredEvidence,/HVAC blower-control configuration.*signal strategy/i);assert.equal(h.state.authoritativeDiagnosticTest.testId,'blower-control-signal-identification');assert.doesNotMatch(h.reply,/tell me whether the control signal changes as you command low, medium, and high/i);assert.doesNotMatch(h.reply,/good|bad|condemn(?:ed)? (?:the )?(?:blower|resistor|module|control head|wiring)/i);
});

test('10.12.76 backfills a persisted empty matrix from authoritative operating evidence without assigning voltage',()=>{
  const h=matrixSyncHarness({state:{blowerSpeedStates:{low:'INOPERATIVE',medium:'INOPERATIVE',high:'OPERATIVE'},blowerOperatingPattern:'HIGH_ONLY',blowerCommandResponseCorrelation:'Not evaluated',routingDiagnostics:{}},guidance:{measurements:[{label:'Blower Control Signal',value:7.2,unit:'V',commandContext:'UNKNOWN'}],evidence:[{type:'operating-state',rawObservation:"high works low and medium don't",facts:{speedStates:{low:'INOPERATIVE',medium:'INOPERATIVE',high:'OPERATIVE'}}}]}});assert.equal(h.count,3);assert.deepEqual(h.state.blowerCommandResponseEvidence.map(item=>[item.commandedSpeed,item.actualBlowerOperation,item.controlSignalValue,item.controlSignalAssociated]),[['LOW','INOPERATIVE','UNKNOWN',false],['MEDIUM','INOPERATIVE','UNKNOWN',false],['HIGH','OPERATIVE','UNKNOWN',false]]);assert.equal(h.state.blowerCommandResponseCorrelation,'AWAITING_MULTI_SPEED_CONTROL_MEASUREMENTS');assert.equal(h.state.routingDiagnostics.blowerCommandResponseObservationCount,3);assert.equal(h.guidance.measurements[0].value,7.2);assert.equal(h.guidance.measurements[0].commandContext,'UNKNOWN');
});

test('10.12.77 atomically preserves the exact vehicle symptom and measurement utterance before routing',()=>{
  const h=additiveEvidenceHarness(),exact="2016 Jeep Wrangler high blow speed works low and medium do not control signal measures at 7.2 V.";assert.equal(h.apply(atomicBlowerNormalizer(exact)),true);assert.deepEqual([h.state.vehicle.year,h.state.vehicle.make,h.state.vehicle.model],['2016','Jeep','Wrangler']);assert.match(h.state.complaint,/High blower speed operative/i);assert.equal(h.state.normalizedSymptom,'HVAC blower high speed operative / low and medium blower speeds inoperative');assert.deepEqual(h.state.blowerSpeedStates,{low:'INOPERATIVE',medium:'INOPERATIVE',high:'OPERATIVE'});assert.equal(h.guidance.measurements.find(item=>item.testType==='control signal').value,7.2);assert.equal(h.state.stage,'diagnostic');assert.equal(h.state.diagnosticDomain,'HVAC / Blower Diagnostic');assert.equal(h.state.authoritativeDiagnosticTest.testId,'blower-command-response-correlation');assert.doesNotMatch(h.reply,/what vehicle|diagnostic code|what.*symptom/i);const atomicCall="if(typeof handleAtomicUtteranceEvidence==='function'&&handleAtomicUtteranceEvidence(text))return",correctionCall="if(typeof handleExplicitComplaintCorrection==='function'&&handleExplicitComplaintCorrection(text))return";assert.ok(html.indexOf(atomicCall)<html.indexOf(correctionCall));
});

test('10.12.77 normalizes supported multi-fact blower speech forms without dropping DTC or later tokens',()=>{
  const forms=["2016 Jeep Wrangler high speed works but low and medium don't.","2016 Jeep Wrangler high works, low and medium don't, control signal is 7.2 volts.","Blower only works on high and the control signal is 7.2 volts.","2016 Jeep Wrangler P0480, fan only works on high, control circuit measures 7.2 volts."];for(const input of forms){const normalized=atomicBlowerNormalizer(input);assert.match(normalized,/high/i);if(/7\.2/.test(input))assert.match(normalized,/7\.2/);if(/P0480/.test(input))assert.match(normalized,/P0480/)}assert.match(atomicBlowerNormalizer(forms[3]),/blower only works on high.*control signal is 7\.2 volts/i);
});

test('10.12.78 canonicalizes evidence labels and synchronizes the active multi-speed test state',()=>{
  const intake=additiveEvidenceHarness(),utterance="2016 Jeep Wrangler high works, but medium and low do not work on the blower. The control signal is 7.2 volts.";assert.equal(intake.apply(atomicBlowerNormalizer(utterance)),true);for(const item of intake.state.blowerCommandResponseEvidence){delete item.testId;delete item.testName}intake.state.authoritativeDiagnosticTest.status='AWAITING_CONTEXT';const h=canonicalSyncHarness({state:intake.state,guidance:intake.guidance});assert.ok(h.guidance.evidence.every(item=>item.testId&&item.testName));assert.ok(h.state.blowerCommandResponseEvidence.every(item=>item.testId==='blower-command-response-correlation'&&item.testName));assert.equal(h.state.authoritativeDiagnosticTest.testId,'blower-command-response-correlation');assert.equal(h.state.authoritativeDiagnosticTest.status,'AWAITING_MULTI_SPEED_CONTROL_MEASUREMENTS');assert.equal(h.guidance.selectedNextTest.testId,h.state.authoritativeDiagnosticTest.testId);assert.equal(h.state.routingDiagnostics.canonicalEvidenceLabelsValidated,'YES');assert.equal(h.state.routingDiagnostics.activeTestStateSynchronized,'YES');assert.deepEqual(h.state.blowerSpeedStates,{low:'INOPERATIVE',medium:'INOPERATIVE',high:'OPERATIVE'});assert.equal(h.guidance.measurements.find(item=>item.value===7.2).unit,'V');assert.doesNotMatch(JSON.stringify(h.guidance.evidence),/"(?:testId|testName)":(?:null|""|"undefined")/);assert.match(html,/replace\('Next Test: None',`Next Test: \$\{esc\(active\.displayName/);
});

test('10.12.79 isolates unit-bearing control voltage from vehicle engine metadata',()=>{
  const h=additiveEvidenceHarness();assert.equal(h.apply("2016 Jeep Wrangler, high speed works, low and medium don't, the control signal is still 7.2 volts."),true);h.state.vehicle.engine='7.2L';const synced=unitSyncHarness({state:h.state,guidance:h.guidance});assert.deepEqual([synced.state.vehicle.year,synced.state.vehicle.make,synced.state.vehicle.model,synced.state.vehicle.engine],['2016','Jeep','Wrangler','']);assert.deepEqual([synced.measurement.measurementType,synced.measurement.value,synced.measurement.unit,synced.measurement.status,synced.measurement.commandedSpeed],['CONTROL_SIGNAL_VOLTAGE',7.2,'V','OBSERVED','UNSPECIFIED']);assert.equal(synced.state.unboundControlSignalEvidence.value,7.2);assert.ok(synced.state.blowerCommandResponseEvidence.every(item=>item.controlSignalAssociated===false));assert.equal(synced.state.blowerOperatingPattern,'HIGH_ONLY');
});

test('10.12.79 keeps explicit engine displacement separate and binds voltage only to an authoritative command',()=>{
  const explicit=additiveEvidenceHarness();assert.equal(explicit.apply("2016 Jeep Wrangler 3.6 liter, high works, low and medium don't, control signal is 7.2 volts."),true);const explicitSync=unitSyncHarness({state:explicit.state,guidance:explicit.guidance});assert.equal(explicitSync.state.vehicle.engine,'3.6L');assert.equal(explicitSync.measurement.value,7.2);const boundState=JSON.parse(JSON.stringify(explicit.state));boundState.currentCommandedSpeed='MEDIUM';const bound=unitSyncHarness({state:boundState,guidance:{...explicit.guidance,measurements:[...explicit.guidance.measurements,{id:'blower-control-signal',testType:'control signal',value:7.2,unit:'V',source:'technician-speech',rawUtterance:'Still 7.2 volts.'}]}});const medium=bound.state.blowerCommandResponseEvidence.find(item=>item.commandedSpeed==='MEDIUM');assert.deepEqual([medium.actualBlowerOperation,medium.controlSignalValue,medium.controlSignalUnit,medium.controlSignalAssociated],['INOPERATIVE',7.2,'V',true]);assert.equal(bound.state.unboundControlSignalEvidence,null);assert.equal(bound.measurement.commandedSpeed,'MEDIUM');assert.match(html,/handleActiveBlowerControlMeasurement\(text\)/);
});

test('10.12.79 later voltage evidence cannot overwrite committed 3.6L vehicle metadata',()=>{
  const h=additiveEvidenceHarness();h.state.vehicle={year:'2016',make:'Jeep',model:'Wrangler',engine:'3.6L',configuration:''};h.state.stage='diagnostic';h.state.intakeStep='complete';h.state.complaint='HVAC blower high-only complaint';assert.equal(h.apply('Control signal is 5 volts.'),true);const synced=unitSyncHarness({state:h.state,guidance:h.guidance});assert.equal(synced.state.vehicle.engine,'3.6L');assert.deepEqual([synced.measurement.value,synced.measurement.unit,synced.measurement.commandedSpeed],[5,'V','UNSPECIFIED']);
});
