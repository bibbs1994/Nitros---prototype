...
test('10.13.37 consumes vehicle and alternator-removal intent in one pass',()=>{
  ...
  assert.equal(h.state.component,'Alternator');assert.equal(h.state.requestedOperation,'Removal / Replacement');assert.equal(h.state.guidanceRequest,'YES');assert.equal(h.state.guidanceIntent,'Mechanical Repair / Component Replacement');assert.match(h.reply,/which engine\/configuration/i);assert.doesNotMatch(h.reply,/which model|missing vehicle model/i);
});
...
test('supplied engine proceeds to actionable guidance without asking for known facts',()=>{
  const h=harness();assert.equal(h.run('2019 Chevy Equinox 1.5 turbo, walk me through getting the alternator out'),true);assert.deepEqual([h.state.vehicle.year,h.state.vehicle.make,h.state.vehicle.model,h.state.vehicle.engine],['2019','Chevrolet','Equinox','1.5L']);assert.equal(h.state.component,'Alternator');assert.match(h.reply,/First make the vehicle safe/i);assert.doesNotMatch(h.reply,/which (?:engine|model)/i);
});
