...
test('BA has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7BC',[\s\S]+release:'Deterministic Circuit Isolation Test Selection',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses BC version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7BC'/);
