...
test('10.12.82 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.82',[\s\S]+release:'Guided Multi-Speed Measurement Capture \+ Automatic Command\/Response Correlation',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.82 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.82'/);
