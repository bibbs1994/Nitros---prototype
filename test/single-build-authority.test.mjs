...
test('BA has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7BD',[\s\S]+release:'Verified Repair Information Intake \+ Diagnostic Resume',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses BD version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7BD'/);
