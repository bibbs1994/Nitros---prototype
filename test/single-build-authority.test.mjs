...
test('10.13.115 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.115',[\s\S]+release:'Electrical Circuit Pipeline Execution Fix',[\s\S]+buildDate:'2026-08-29'/);
...
test('service worker uses 10.13.115 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.115'/);
