...
test('10.12.13 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.13',[\s\S]+release:'Static Snapshot Temporal-Inference Guard',[\s\S]+buildDate:'2026-08-12'/);
...
test('service worker uses 10.12.13 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.13'/);
