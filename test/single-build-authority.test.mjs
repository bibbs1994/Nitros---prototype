...
test('AV has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AV',[\s\S]+release:'Diagnostic Finding Interpretation \+ Stateful Guided Testing',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AV version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AV'/);
