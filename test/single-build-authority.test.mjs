...
test('10.12.26 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.26',[\s\S]+release:'Appointment Routing Trace',[\s\S]+buildDate:'2026-08-12'/);
...
test('service worker uses 10.12.26 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.26'/);
