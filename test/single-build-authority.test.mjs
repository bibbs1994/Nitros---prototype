...
test('10.12.91 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.91',[\s\S]+release:'Diagnostic Guidance Intent \+ Forward Route Advancement',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.91 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.91'/);
