...
test('10.13.138 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.138',[\s\S]+release:'Visual Condition Analyzer Timeout Recovery \/ Guaranteed Structured Result',[\s\S]+buildDate:'2026-09-02'/);
...
test('service worker uses 10.13.138 version and preserves safe navigation caching',()=>{
