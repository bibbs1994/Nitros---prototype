...
test('BA has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7BA',[\s\S]+release:'Post-Correlation PASS Routing \/ Circuit Isolation Transition',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses BA version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7BA'/);
