...
test('10.13.94 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.94',[\s\S]+release:'Two-Layer Vehicle Context Isolation & Vehicle-Specific Visual Reasoning',[\s\S]+buildDate:'2026-08-28'/);
...
test('service worker uses 10.13.94 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.94'/);
