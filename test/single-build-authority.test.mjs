...
test('10.13.73 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.73',[\s\S]+release:'License Plate Number Row',[\s\S]+buildDate:'2026-08-25'/);
...
test('service worker uses 10.13.73 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.73'/);
