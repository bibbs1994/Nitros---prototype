...
test('AW has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AW',[\s\S]+release:'Measurement Intent Validation \+ Diagnostic Text Encoding Hardening',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AW version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AW'/);
