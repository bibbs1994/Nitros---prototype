...
test('10.13.68 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.68',[\s\S]+release:'MSI Support Ticket Server Wiring',[\s\S]+buildDate:'2026-08-23'/);
...
test('service worker uses 10.13.68 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.68'/);
