...
test('10.12.72 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.72',[\s\S]+release:'Authoritative Vehicle \+ Measurement Evidence Retention & Command\/Response Correlation Routing',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.72 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.72'/);
