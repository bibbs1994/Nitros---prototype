...
test('AT has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AT',[\s\S]+release:'Combined DTC \+ Vehicle Intake Normalization',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AT version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AT'/);
