...
test('AZ has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AZ',[\s\S]+release:'Cam\/Crank Correlation Natural-Language Result Recognition Hardening',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AZ version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AZ'/);
  assert.match(sw,/self\.skipWaiting\(\)/);
  assert.match(sw,/self\.clients\.claim\(\)/);
  assert.match(sw,/fetch\(request, \{ cache: 'no-store' \}\)/);
  assert.match(sw,/caches\.match\(APP_SHELL, \{ cacheName: CACHE_NAME \}\)/);
