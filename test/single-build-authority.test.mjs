...
test('VL has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7VL',[\s\S]+release:'Diagnostic Completion \/ Repair-Decision Gate',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses VL version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7VL'/);
