...
test('AS has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AS',[\s\S]+release:'Single Build Authority \+ PWA Update Stabilization',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AS version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AS'/);
