...
test('10.12.73 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.73',[\s\S]+release:'Authoritative Symptom Promotion \+ Positive\/Negative Blower-State Retention \+ Workflow Synchronization',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.73 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.73'/);
