...
test('V3 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V3',[\s\S]+release:'Direct AI Diagnostic Combined Intake',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses V3 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V3'/);
