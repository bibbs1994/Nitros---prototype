...
test('10.12.59 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.59',[\s\S]+release:'Oliver Measured Low-Baritone Prosody Reference',[\s\S]+buildDate:'2026-08-13'/);
...
test('service worker uses 10.12.59 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.59'/);
