...
test('10.13.37 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.37',[\s\S]+release:'Natural-Language Vehicle \+ Mechanical Guidance Intake Fix',[\s\S]+buildDate:'2026-08-17'/);
...
test('service worker uses 10.13.37 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.37'/);
