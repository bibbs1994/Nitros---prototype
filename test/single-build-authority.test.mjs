...
test('VJ has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7VJ',[\s\S]+release:'Diagnostic Continuity Result Acceptance and Completion Transition',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses VJ version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7VJ'/);
