...
test('10.13.81 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.81',[\s\S]+release:'Strict Output Schema Validation',[\s\S]+buildDate:'2026-08-27'/);
...
test('service worker uses 10.13.81 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.81'/);
