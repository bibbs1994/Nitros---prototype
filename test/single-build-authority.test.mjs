...
test('AX has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AX',[\s\S]+release:'Atomic Diagnostic Result Transition \+ Oliver Voice Lock',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AX version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AX'/);
