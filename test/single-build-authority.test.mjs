...
test('10.12.95 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.95',[\s\S]+release:'Authoritative Configuration Binding \+ No-Reask Enforcement',[\s\S]+buildDate:'2026-08-15'/);
...
test('service worker uses 10.12.95 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.95'/);
