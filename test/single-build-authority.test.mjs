...
test('BA has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7BF',[\s\S]+release:'Verified Repair Information Gate',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses BF version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7BF'/);
