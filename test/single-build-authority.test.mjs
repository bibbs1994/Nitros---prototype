...
test('10.13.144 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.144',[\s\S]+release:'Vercel Upstash Environment Compatibility',[\s\S]+buildDate:'2026-09-03'/);
...
test('service worker uses 10.13.144 version and preserves safe navigation caching',()=>{
