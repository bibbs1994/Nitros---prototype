...
test('10.13.80 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.80',[\s\S]+release:'Connection Context Verification',[\s\S]+buildDate:'2026-08-27'/);
...
test('service worker uses 10.13.80 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.80'/);
