...
test('10.13.58 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.58',[\s\S]+release:'Internal Technician Notes Contextual Assistance Fix',[\s\S]+buildDate:'2026-08-23'/);
...
test('service worker uses 10.13.58 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.58'/);
...
test('mobile quick toolbar clears the Vehicle control without reducing safe-area protection',()=>{
