...
test('V1 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V1',[\s\S]+release:'Diagnostic Intake State Advancement Fix',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses V1 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V1'/);
