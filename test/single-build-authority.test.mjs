...
test('V6 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V6',[\s\S]+release:'Document Extraction Required-Field Validation Fix',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses V6 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V6'/);
