...
test('10.12.58 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.58',[\s\S]+release:'Oliver Natural Output Level \+ Semantic Cadence Refinement',[\s\S]+buildDate:'2026-08-13'/);
...
test('service worker uses 10.12.58 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.58'/);
