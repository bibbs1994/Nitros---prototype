...
test('10.12.69 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.69',[\s\S]+release:'High-Only Blower Semantic Classification \+ Evidence-Aware Next-Test Selection',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.69 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.69'/);
