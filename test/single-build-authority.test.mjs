...
test('10.13.132 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.132',[\s\S]+release:'Visible Relationship Reconciliation',[\s\S]+buildDate:'2026-09-01'/);
...
test('service worker uses 10.13.132 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.132'/);
