...
test('10.13.87 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.87',[\s\S]+release:'Support Ticket Status Actions',[\s\S]+buildDate:'2026-08-27'/);
...
test('service worker uses 10.13.87 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.87'/);
