...
test('10.12.86 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.86',[\s\S]+release:'Multi-Family Automotive DTC Knowledge Resolution',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.86 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.86'/);
