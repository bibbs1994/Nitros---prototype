...
test('10.13.129 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.129',[\s\S]+release:'10\.13\.124 Visual Baseline Recovery \+ Connection State Reconciliation Lock',[\s\S]+buildDate:'2026-08-30'/);
...
test('service worker uses 10.13.129 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.129'/);
