...
test('10.13.48 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.48',[\s\S]+release:'Unfinished RO Draft Protection',[\s\S]+buildDate:'2026-08-22'/);
...
test('service worker uses 10.13.48 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.48'/);
