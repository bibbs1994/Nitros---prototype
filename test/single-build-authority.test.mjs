...
test('10.13.38 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.38',[\s\S]+release:'Pending-Question Binding \+ Mechanical Guidance Mode Lock',[\s\S]+buildDate:'2026-08-17'/);
...
test('service worker uses 10.13.38 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.38'/);
