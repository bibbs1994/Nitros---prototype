...
test('10.12.49 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.49',[\s\S]+release:'Next-Test Rationale Alignment Guard',[\s\S]+buildDate:'2026-08-13'/);
...
test('service worker uses 10.12.49 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.49'/);
