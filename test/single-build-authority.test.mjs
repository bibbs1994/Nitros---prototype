...
test('10.12.46 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.46',[\s\S]+release:'Fresh-Result \/ Evidence-State Separation',[\s\S]+buildDate:'2026-08-13'/);
...
test('service worker uses 10.12.46 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.46'/);
