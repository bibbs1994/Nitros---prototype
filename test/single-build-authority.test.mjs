...
test('10.12.71 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.71',[\s\S]+release:'Blower Command\/Response Evidence Matrix \+ Correlation Classification',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.71 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.71'/);
