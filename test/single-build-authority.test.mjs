...
test('10.12.29 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.29',[\s\S]+release:'PID Snapshot Interpretation Enforcement Fix',[\s\S]+buildDate:'2026-08-12'/);
...
test('service worker uses 10.12.29 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.29'/);
