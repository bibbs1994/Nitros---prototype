...
test('BA has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7BB',[\s\S]+release:'Correlation PASS → Forced Circuit Isolation Transition',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses BB version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7BB'/);
