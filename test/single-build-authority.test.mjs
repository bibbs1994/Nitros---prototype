...
test('10.13.06 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.06',[\s\S]+release:'Diagnostic Validation Series',[\s\S]+buildDate:'2026-08-16'/);
...
test('service worker uses 10.13.06 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.06'/);
