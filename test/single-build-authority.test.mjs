...
test('10.13.72 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.72',[\s\S]+release:'Saved Job Resume Restore',[\s\S]+buildDate:'2026-08-25'/);
...
test('service worker uses 10.13.72 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.72'/);
