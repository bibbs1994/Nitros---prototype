...
test('V4 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V4',[\s\S]+release:'Diagnostic Measurement Result Ingestion Fix',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses V4 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V4'/);
