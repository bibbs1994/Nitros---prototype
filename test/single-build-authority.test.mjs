...
test('10.13.124 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.124',[\s\S]+release:'Localized Visual Verification Execution Fix',[\s\S]+buildDate:'2026-08-30'/);
...
test('service worker uses 10.13.124 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.124'/);
