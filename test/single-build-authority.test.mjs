...
test('VM has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7VM',[\s\S]+release:'Evidence-Bound Repair Decision',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses VM version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7VM'/);
