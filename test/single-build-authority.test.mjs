...
test('10.12.74 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.74',[\s\S]+release:'Blower Command\/Response Correlation \+ Multi-Speed Evidence Matrix Commit',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.74 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.74'/);
