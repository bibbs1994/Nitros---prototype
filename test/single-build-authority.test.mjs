...
test('V9 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V9',[\s\S]+release:'Verified Repair Information PASS State Fix',[\s\S]+buildDate:'2026-08-11'/);
...
test('service worker uses V9 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V9'/);
