...
test('10.13.77 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.77',[\s\S]+release:'Connection Gap and Separation Priority',[\s\S]+buildDate:'2026-08-26'/);
...
test('service worker uses 10.13.77 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.77'/);
