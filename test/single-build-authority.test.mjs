...
test('V8 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V8',[\s\S]+release:'Verified Repair Information Fresh-Extraction Synchronization Fix',[\s\S]+buildDate:'2026-08-11'/);
...
test('service worker uses V8 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V8'/);
