...
test('10.13.128 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.128',[\s\S]+release:'Deep Visual Identity Lock \+ Multi-Frame Physical Connection Truth',[\s\S]+buildDate:'2026-08-30'/);
...
test('service worker uses 10.13.128 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.128'/);
