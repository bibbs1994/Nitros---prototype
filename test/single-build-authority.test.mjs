...
test('10.12.20 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.20',[\s\S]+release:'Oliver Voice Appointment Structured Capture \/ Persistence Fix',[\s\S]+buildDate:'2026-08-12'/);
...
test('service worker uses 10.12.20 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.20'/);
