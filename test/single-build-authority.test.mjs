...
test('10.13.40 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.40',[\s\S]+release:'Multiple Active RO Support',[\s\S]+buildDate:'2026-08-22'/);
...
test('service worker uses 10.13.40 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.40'/);
