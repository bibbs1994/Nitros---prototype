...
test('AU has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AU',[\s\S]+release:'DTC Character Canonicalization \/ Speech-Text Hardening',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AU version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AU'/);
