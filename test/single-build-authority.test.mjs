...
test('10.13.139 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.139',[\s\S]+release:'Whole-Image Defect Prioritization \/ Connection-State Reasoning',[\s\S]+buildDate:'2026-09-02'/);
...
test('service worker uses 10.13.139 version and preserves safe navigation caching',()=>{
