...
test('10.13.133 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.133',[\s\S]+release:'Physical Connection State Reconciliation',[\s\S]+buildDate:'2026-09-01'/);
...
test('service worker uses 10.13.133 version and preserves safe navigation caching',()=>{
