...
test('10.12.84 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.84',[\s\S]+release:'Circuit Architecture Identification \+ Output-Stage Isolation',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.84 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.84'/);
