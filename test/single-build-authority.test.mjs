...
test('10.12.99 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.99',[\s\S]+release:'Transmission DTC Resolution \+ Intake Consumption Fix',[\s\S]+buildDate:'2026-08-15'/);
...
test('service worker uses 10.12.99 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.99'/);
