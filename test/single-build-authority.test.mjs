...
test('10.12.83 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.83',[\s\S]+release:'Control-Signal Strategy Classification \+ Dynamic Signal Verification',[\s\S]+buildDate:'2026-08-14'/);
...
test('service worker uses 10.12.83 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.83'/);
