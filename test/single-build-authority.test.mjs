...
test('10.13.93 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.93',[\s\S]+release:'Vehicle Context Isolation & Analysis Completion Integrity',[\s\S]+buildDate:'2026-08-28'/);
...
test('service worker uses 10.13.93 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.93'/);
