...
test('VK has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7VK',[\s\S]+release:'Repair-Decision Diagnostic Conclusion Gate',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses VK version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7VK'/);
