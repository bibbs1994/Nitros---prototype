...
test('10.13.130 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.130',[\s\S]+release:'Final Visual Evidence Reconciliation',[\s\S]+buildDate:'2026-08-31'/);
...
test('service worker uses 10.13.130 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.130'/);
