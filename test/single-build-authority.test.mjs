...
test('10.12.55 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.55',[\s\S]+release:'Oliver Natural Speech \/ Prosody Humanization',[\s\S]+buildDate:'2026-08-13'/);
...
test('service worker uses 10.12.55 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.55'/);
