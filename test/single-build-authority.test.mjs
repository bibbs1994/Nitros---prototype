...
test('10.13.131 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.131',[\s\S]+release:'Reconciliation Contract Repair',[\s\S]+buildDate:'2026-08-31'/);
...
test('service worker uses 10.13.131 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.131'/);
