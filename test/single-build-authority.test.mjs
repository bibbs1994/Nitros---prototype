...
test('V7 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V7',[\s\S]+release:'Document Extraction Anti-Inference \/ Missing-Criterion Fix',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses V7 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V7'/);
