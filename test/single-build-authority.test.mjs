...
test('10.13.76 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.76',[\s\S]+release:'Visual Defect Severity Classification',[\s\S]+buildDate:'2026-08-26'/);
...
test('service worker uses 10.13.76 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.13\.76'/);
