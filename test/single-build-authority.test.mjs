...
test('10.13.140 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.13\.140',[\s\S]+release:'Authoritative Connection-State Reconciliation',[\s\S]+buildDate:'2026-09-03'/);
...
test('service worker uses 10.13.140 version and preserves safe navigation caching',()=>{
