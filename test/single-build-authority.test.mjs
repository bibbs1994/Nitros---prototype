...
test('V5 has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7V5',[\s\S]+release:'Verified Repair Information Document\/Text Extraction',[\s\S]+buildDate:'2026-08-10'/);
...
test('service worker uses V5 version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7V5'/);
