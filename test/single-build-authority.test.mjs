...
test('AY has one canonical build authority',()=>{
  assert.match(html,/window\.NitrosBuild=Object\.freeze\(\{[\s\S]+version:'10\.12\.7AY',[\s\S]+release:'Authoritative Diagnostic Result Commit \+ Next-Test Transition Lock',[\s\S]+buildDate:'2026-08-09'/);
...
test('service worker uses AY version and preserves safe navigation caching',()=>{
  assert.match(sw,/const VERSION = '10\.12\.7AY'/);
