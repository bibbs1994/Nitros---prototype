...
test('Need help opens the centralized local-first support ticket flow',()=>{
  for(const id of ['nitrosSupportTicket','nitrosSupportTicketNote','nitrosSupportTicketSummary','nitrosSupportTicketSend','nitrosSupportTicketCancel'])assert.match(html,new RegExp(`id="${id}"`));
...
test('support tickets include persistent IDs, offline sync state, and a sanitized diagnostic snapshot',()=>{
  ...
  assert.match(ticket,/screenshot:\{supported:false,status:'deferred'\}/);
});
