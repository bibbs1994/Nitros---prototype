...
test('visual condition inspection supports visible residue, no-defect, and obstructed-image outcomes',async()=>{
  ...
  assert.equal(residue.semanticResult.visualConditionInspection.connectionAssessments[0].findingType,'CLEAR_DEFECT');
  assert.doesNotMatch(JSON.stringify(residue.semanticResult.visualConditionInspection),/confirmed disconnected/i);
