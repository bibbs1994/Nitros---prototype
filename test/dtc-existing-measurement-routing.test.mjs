...
test('10.12.93 consumes an existing P06DD oil-pressure measurement and requests only missing context',()=>{
  const h=harness();
...
test('10.12.93 applies later measurement context without discarding the stored reading',()=>{
  const h=harness();
...
test('10.12.93 checks existing P06DD evidence before generic status intake',()=>{
...
test('10.12.93 classifies next-test intent and advances forward without storing the question',()=>{
  const h=harness();
...
test('10.12.93 guidance gate precedes evidence and status handlers',()=>{
  const process=html.slice(html.indexOf('function process('),html.indexOf('function renderTranscript('));
...
test('10.12.93 binds warm-idle phrases to the pending 18 psi measurement instead of creating findings',()=>{
...
test('10.12.93 resolves the verified 2016 Silverado 5.3L P06DD criterion from structured knowledge',()=>{
...
test('10.12.93 engine follow-up immediately resolves, interprets, and advances the stored hot-idle result',()=>{
  const h=harness();
  h.state.complaint='MIL on and low oil pressure';
...
test('10.12.93 combined P06DD intake consumes inline prior pressure evidence before returning',()=>{
  const process=html.slice(html.indexOf('function process('),html.indexOf('function renderTranscript('));
