...
function graphPidMeasurement(text, labels) {
  ...
  const acceptedReason='PID_LOCAL_EXPLICIT_ROLE_BINDING',candidateAudit=Object.freeze({canonicalPid:rpmPid?'Engine Speed':labels[0],rawEvidenceTokens:Object.freeze([...segments]),candidates:Object.freeze([{role:'current',raw:rawCurrent,value:current,status:current===null?'REJECTED':'ACCEPTED',reason:current===null?'NO_PID_LOCAL_PARSEABLE_EVIDENCE':acceptedReason},{role:'minimum',raw:rawMinimum,value:minimum,status:minimum===null?'REJECTED':'ACCEPTED',reason:minimum===null?'NO_PID_LOCAL_PARSEABLE_EVIDENCE':acceptedReason},{role:'maximum',raw:rawMaximum,value:maximum,status:maximum===null?'REJECTED':'ACCEPTED',reason:maximum===null?'NO_PID_LOCAL_PARSEABLE_EVIDENCE':acceptedReason}])});
