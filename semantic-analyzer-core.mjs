...
const automotiveGraphSchema = {
  ...
  properties: {
    ...
    diagnosticSignificance: { type: 'string', enum: ['NORMAL_OR_EXPECTED','MILDLY_ABNORMAL','SIGNIFICANT','INCONCLUSIVE'] },
...
function validateAutomotiveGraph(raw) {
  ...
  const diagnosticSignificance=['NORMAL_OR_EXPECTED','MILDLY_ABNORMAL','SIGNIFICANT','INCONCLUSIVE'].includes(raw.diagnosticSignificance)?raw.diagnosticSignificance:'INCONCLUSIVE';
...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const wideband=/\b(?:a\/f sensor|afs|air fuel ratio sensor|wideband|lambda sensor)\b/i.test(all),dynamic=/switch|cross.?count|oscillat|trace (?:rises|falls|changes|responds|mirrors)|correlat|over time/i.test((graph.traceFindings||[]).join(' ')),catalystComparison=dynamic&&/(?:b1s1|upstream)/i.test(all)&&/(?:b1s2|downstream)/i.test(all);
  ...
  const interpretation=(graph.interpretation||[]).filter(item=>!forbidden.some(rule=>rule.test(item))).filter(item=>!(rearO2!==null&&rearO2>=.7&&/(?:low.{0,20}lean|lean.{0,20}low)/i.test(item))).filter(item=>!(wideband&&/0\.1.{0,20}0\.9|narrowband/i.test(item)));
  const reasoningEvidence={sensorTypeDetected:wideband?'WIDEBAND_AIR_FUEL_AND_CONVENTIONAL_DOWNSTREAM_O2':rearO2!==null?'CONVENTIONAL_NARROWBAND_O2':'NOT_CONFIRMED',fuelTrimPolarity:stft!==null||ltft!==null?((stft||0)+(ltft||0)<0?'NEGATIVE_PCM_REMOVING_FUEL':(stft||0)+(ltft||0)>0?'POSITIVE_PCM_ADDING_FUEL':'NEAR_ZERO'):'NOT_AVAILABLE',combinedTrim:stft!==null&&ltft!==null?Math.round((stft+ltft)*1000)/1000:null,operatingState:rpm!==null&&coolant!==null&&rpm>=500&&rpm<=1100&&coolant>=160?'WARM_NEAR_IDLE':'NOT_CONFIRMED',dynamicTraceEvidenceAvailable:dynamic,catalystComparisonEvidenceAvailable:catalystComparison,diagnosticCertainty:dynamic?'SUPPORTED_BY_DYNAMIC_TRACE':'SNAPSHOT_ONLY_INCONCLUSIVE'};
  ...
  if(reasoningEvidence.combinedTrim!==null){const combined=reasoningEvidence.combinedTrim,amount=Math.abs(combined).toFixed(1);interpretation.push(combined<0?`Combined Bank 1 fuel trim is approximately ${combined.toFixed(3)}%. The PCM is removing approximately ${amount}% fuel at this operating point; this is a modest rich-side correction and does not by itself prove a fault.`:combined>0?`Combined Bank 1 fuel trim is approximately +${combined.toFixed(3)}%. The PCM is adding approximately ${amount}% fuel at this operating point; this alone does not identify the cause.`:'Combined Bank 1 fuel trim is near zero at this operating point.');}
  ...
  if(rearO2!==null)interpretation.push(`B1S2 is displaying approximately ${rearO2} V, a ${rearO2>=.7?'high-voltage/rich-side':rearO2<=.3?'low-voltage/lean-side':'midrange/crossover-region'} signal at this captured instant. One snapshot is insufficient to determine mixture control, sensor health, or catalyst condition.`);
  ...
  const nextTest=catalystComparison?(graph.nextTest||[]).slice(0,3):['Capture the upstream A/F or O2 and downstream O2 traces together during a warmed steady-speed road test, include a controlled acceleration/deceleration event when safe, and record fuel trims and operating temperature in the same capture; compare downstream switching with upstream activity and review Mode $06 catalyst-monitor data when available.'];
  return {...graph,interpretation:[...new Set(interpretation)].slice(0,16),diagnosticSignificance:dynamic?graph.diagnosticSignificance:'INCONCLUSIVE',nextTest,reasoningEvidence,contradictionGuard:'PASS'};
}
...
Set distinguishingFeaturesComplete true only when the selected exact drivetrain type has strong visible discriminators. Otherwise lower component confidence, include the closest competing type in possibleAlternatives and drivetrainDiscrimination.competingCandidate, and explain the ambiguity. Supporting evidence must specifically justify the selected primary component. Secondary items such as driveshaft, exhaust, heat shield, crossmember, transmission, differential, CV axle, or suspension must not override the visually dominant intended subject.`;
  ...
  if (semanticResult.category === 'AUTOMOTIVE_GRAPH') {
    ...
    const graphPrompt = `Analyze only the visible pixels of this current automotive diagnostic graph. This is a dedicated diagnostic interpretation stage after classification. Extract only labels, units, numeric values, time scales, operating conditions, and vehicle identity that are genuinely readable. Never invent PID or sensor labels, voltages, scales, wire details, connector pins, specifications, OEM procedures, or vehicle-specific facts. Analyze every visible trace and panel, including RPM, fuel trim, oxygen/air-fuel sensors, temperature, pressure, frequency, duty cycle, upstream/downstream behavior, switching/cross-count activity, flatline or bias, correlation, and abnormal patterns when actually supported. Keep directly visible facts in observed and diagnostic inferences in interpretation, separately rate diagnosticSignificance, and put every limitation in unreadableOrUncertain. A conventional zirconia narrowband O2 signal near 0.1 V is lean-side, near 0.9 V is rich-side, and near 0.45 V is crossover; 0.875 V is high/rich-side at that instant, never low/lean, and one sample does not diagnose mixture, sensor, or catalyst health. Do not apply narrowband rules to A/F, AFS, air-fuel ratio, wideband, or lambda sensors; a Toyota-style upstream A/F value near its center region cannot verify sensor performance without response and fuel-control evidence. Positive trims mean the PCM adds fuel; negative trims mean it removes fuel. When same-bank STFT and LTFT are visible under the same condition, add them and describe the correction conservatively without universal pass/fail thresholds. Use RPM, coolant, speed, loop state, load, and throttle when visible. Never diagnose a dynamic component from one numeric snapshot. For catalyst/P0420 analysis prioritize upstream/downstream trace relationship, switching, stability, temperature, trims, and operating condition; never condemn a converter from the graph alone and never clear or condemn a catalyst from one voltage. Generate Next Test only from observed evidence, not a generic test list. Before returning, discard contradictions including high O2 voltage called low/lean, negative trims called a vacuum leak without support, snapshot-only definitive diagnoses, insufficient evidence followed by condemnation, or A/F data interpreted as narrowband. In visibleVehicle, report a vehicle only if identifying text is visibly readable in the image.`;
