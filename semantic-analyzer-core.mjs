...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const interpretation=(graph.interpretation||[]).filter(item=>!forbidden.some(rule=>rule.test(item))).filter(item=>dynamic||!temporalInference.test(item)).filter(item=>dynamic||supportedCriterion||!/(?:mildly abnormal|\babnormal\b|\bpass(?:es|ed)?\b|\bfail(?:s|ed|ure)?\b)/i.test(item)).filter(item=>!(rearO2!==null&&rearO2>=.7&&/(?:low.{0,20}lean|lean.{0,20}low)/i.test(item))).filter(item=>!(wideband&&/0\.1.{0,20}0\.9|narrowband/i.test(item)));
  ...
  if(rearO2!==null)interpretation.push(`B1S2 is approximately ${rearO2} V at the captured instant${rearO2>=.7?', on the high-voltage/rich side':rearO2<=.3?', on the low-voltage/lean side':', near the midrange/crossover region'}. This single frame does not establish sensor switching, mixture-control behavior, sensor health, or catalyst efficiency.`);
  ...
  return {...graph,interpretation:[...new Set(interpretation)].slice(0,16),diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',nextTest,unreadableOrUncertain:dynamic?graph.unreadableOrUncertain:[...new Set([...(graph.unreadableOrUncertain||[]),'Temporal behavior is unreadable or unavailable because this capture contains only instantaneous PID values.'])],reasoningEvidence,contradictionGuard:'PASS'};
}
