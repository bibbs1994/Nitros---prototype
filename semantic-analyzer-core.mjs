...
function graphPidMeasurement(text, labels) {
  ...
  return {current,range:!numericInvalid?range:null,invalidRange:range&&numericInvalid?range:null,numericInvalid,rawCurrent,rawMinimum,rawMaximum,signNormalizationApplied,inconsistencyReason};
}
...
function canonicalNumericEvidence(name,unit,reading) {
  ...
  const hasRange=minimum!==null&&maximum!==null,consistent=!reading.numericInvalid,includesZero=hasRange&&minimum<=tolerance&&maximum>=-tolerance,crossesZero=hasRange&&minimum<-tolerance&&maximum>tolerance;
...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const readings=[['Long FT #1',ltftReading],['Short FT #1',stftReading],['O2S B1S2',rearO2Reading],['AFS B1S1',afsReading],['Engine Speed',rpmReading]],invalidReadings=readings.filter(([,reading])=>reading.numericInvalid),invalidLabels=invalidReadings.map(([name])=>name);
  const validStft=stftReading.numericInvalid?null:stft,validLtft=ltftReading.numericInvalid?null:ltft;
  ...
  const normalInterpretation=[...new Set(interpretation)].filter(item=>!dynamic||!staticContradiction.test(item)).slice(0,16),evidenceConsistencyFailures=invalidReadings.map(([name,reading])=>{const evidence=numericEvidence.find(item=>item.pidName===name),failure=reading.inconsistencyReason==='CURRENT_ABOVE_MAX'?'Current value exceeds recorded maximum.':reading.inconsistencyReason==='CURRENT_BELOW_MIN'?'Current value is below recorded minimum.':reading.inconsistencyReason==='MIN_GREATER_THAN_MAX'?'Recorded minimum is greater than recorded maximum.':reading.inconsistencyReason==='CONTRADICTORY_DUPLICATE_VALUES'?'Contradictory duplicate numeric values were reported.':reading.inconsistencyReason==='NUMERIC_PARSE_FAILURE'?'A named numeric field could not be parsed.':reading.inconsistencyReason==='NON_FINITE_VALUE'?'A numeric field is not finite.':'Numeric evidence is internally inconsistent.';return{pidName:name,current:evidence?.rawCurrent??evidence?.current??'ABSENT',minimum:evidence?.rawMinimum??evidence?.minimum??'ABSENT',maximum:evidence?.rawMaximum??evidence?.maximum??'ABSENT',unit:evidence?.unit||'',failureCode:reading.inconsistencyReason,failure}}),hardNumericFailure=evidenceConsistencyFailures.length>0,finalInterpretation=hardNumericFailure?['Diagnostic interpretation stopped because numeric evidence inconsistency was detected. Re-read or recapture the failed PID evidence before continuing.']:normalInterpretation,contradictionPresent=dynamic&&normalInterpretation.some(item=>staticContradiction.test(item)),semanticConsistencyStatus=hardNumericFailure?'FAIL_NUMERIC_EVIDENCE':contradictionPresent?'FAIL':initialStaticContradiction?'RECONCILED':'PASS',freshResultVerification=hardNumericFailure||contradictionPresent?'FAIL':'PASS';
  ...
  return {...graph,analysisMode,evidenceType,observed:hardNumericFailure?[]:observed,valuesAndScales:hardNumericFailure?[]:valuesAndScales,traceFindings:hardNumericFailure?[]:traceFindings,interpretation:finalInterpretation,diagnosticSignificance:hardNumericFailure||contradictionPresent?'INDETERMINATE':dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',diagnosticSignificanceReason:hardNumericFailure?'NUMERIC_EVIDENCE_INCONSISTENCY_DETECTED':contradictionPresent?'PENDING_SEMANTIC_RECONCILIATION':'VALIDATED_EVIDENCE',evidenceConsistencyFailures,numericEvidence,numericValidation,evidenceInventory,evidenceInventoryStatus:hardNumericFailure?'WITHHELD_INVALID_EVIDENCE':'PASS',semanticConsistencyStatus,freshResultVerification,unresolvedQuestion:hardNumericFailure?'Which source values are correct for the failed PID evidence?':unresolvedQuestion,nextTest:hardNumericFailure?['Re-read or recapture the inconsistent PID evidence before diagnostic interpretation continues.']:contradictionPresent?[]:nextTest,nextTestReason:hardNumericFailure?'The current PID evidence contains an internally contradictory Current/Min/Max relationship and cannot be treated as verified diagnostic evidence.':contradictionPresent?'Withheld until classifier and interpretation evidence agree.':nextTestReason,redundantTestCheck:hardNumericFailure?'NOT_RUN_INVALID_EVIDENCE':'PASS',candidateNextTestRejected:hardNumericFailure?'NORMAL_SELECTION_BYPASSED':rejectedCandidates.length?'DUPLICATES ACTIVE EVIDENCE':'NONE',nextTestSelection:hardNumericFailure?'CORRECTIVE_ACQUISITION_ONLY':contradictionPresent?'WITHHELD':'PASS',unreadableOrUncertain,reasoningEvidence,contradictionGuard:hardNumericFailure||contradictionPresent?'FAIL':'PASS'};
}
