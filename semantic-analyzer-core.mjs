...
const visualConditionInspectionSchema = {
  ...
  properties: {
    ...
    connectionAssessments: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['location','seatingStatus','findingType','severity','findingConfidence','visibleEvidence','recommendedVerification','safetyDrivabilityImpact'], properties: {
      location: { type: 'string', maxLength: 240 }, seatingStatus: { type: 'string', enum: ['SEPARATION_OR_GAP_VISIBLE','POSSIBLE_IMPROPER_SEATING','NO_GAP_OR_SEPARATION_VISIBLE','NOT_RELIABLY_VISIBLE'] }, findingType: { type: 'string', enum: ['CLEAR_DEFECT','POSSIBLE_CONCERN','RESIDUE_OR_STAINING','SEATING_NOT_RELIABLY_VISIBLE','NO_DEFECT_VISIBLE'] }, severity: { type: 'string', enum: ['CRITICAL','HIGH','MODERATE','LOW','UNDETERMINED'] }, findingConfidence: { anyOf: [{ type: 'number', minimum: 0, maximum: 100 }, { type: 'string', pattern: '^\\s*(?:\\d+(?:\\.\\d+)?|\\.\\d+)\\s*%?\\s*$' }, { type: 'null' }] }, visibleEvidence: { type: 'string', maxLength: 500 }, recommendedVerification: { type: 'string', maxLength: 500 }, safetyDrivabilityImpact: { anyOf: [{ type: 'string', maxLength: 500 }, { type: 'null' }] }
    } }, maxItems: 12 },
...
function validateVisualConditionInspection(raw) {
  ...
  const connectionAssessments = Array.isArray(raw.connectionAssessments) ? raw.connectionAssessments.slice(0, 12).map((assessment, index) => {
    ...
    const evidence = String(assessment.visibleEvidence || '').trim().slice(0, 500), findingType = String(assessment.findingType || ''), severity = String(assessment.severity || ''), verification = String(assessment.recommendedVerification || '').trim().slice(0, 500), findingConfidence = normalizeSemanticConfidence(assessment.findingConfidence);
    if (!location || !['SEPARATION_OR_GAP_VISIBLE','POSSIBLE_IMPROPER_SEATING','NO_GAP_OR_SEPARATION_VISIBLE','NOT_RELIABLY_VISIBLE'].includes(seatingStatus) || !['CLEAR_DEFECT','POSSIBLE_CONCERN','RESIDUE_OR_STAINING','SEATING_NOT_RELIABLY_VISIBLE','NO_DEFECT_VISIBLE'].includes(findingType) || !['CRITICAL','HIGH','MODERATE','LOW','UNDETERMINED'].includes(severity) || !evidence || !verification || findingConfidence === null) throw new Error(`Visual condition connection assessment ${index + 1} lacks required finding evidence.`);
    ...
    return { location, seatingStatus, findingType, severity, findingConfidence, visibleEvidence: evidence, recommendedVerification: verification, safetyDrivabilityImpact: typeof assessment.safetyDrivabilityImpact === 'string' ? assessment.safetyDrivabilityImpact.trim().slice(0, 500) || null : null };
  }) : [];
  const uniqueConnectionAssessments = [...new Map(connectionAssessments.map(assessment => [`${assessment.location.toLowerCase()}|${assessment.seatingStatus}|${assessment.visibleEvidence.toLowerCase()}`, assessment])).values()];
...
First inspect every visible pipe or hose connection, coupler, joint, clamp, electrical connector, fastener, bracket, mount, sealing surface, alignment, seating edge, and gap/separation point independently. For each visible connection, return one non-duplicated connectionAssessment with its exact visible location, one seatingStatus, findingType, severity, independent findingConfidence, exact pixel-supported visibleEvidence, concise physical verification, and cautious safety/drivability impact. Use SEPARATION_OR_GAP_VISIBLE with CLEAR_DEFECT when a pipe end, coupler, or mating surface is visibly separated; classify it HIGH when likely to affect operation/drivability, MODERATE when the visible defect needs confirmation but impact is uncertain, or CRITICAL only for immediate safety or likely severe damage. Use POSSIBLE_IMPROPER_SEATING with POSSIBLE_CONCERN for limited evidence. Use RESIDUE_OR_STAINING only for residue/discoloration without claiming a disconnected pipe; severity is normally LOW or MODERATE. Use NOT_RELIABLY_VISIBLE with SEATING_NOT_RELIABLY_VISIBLE and UNDETERMINED severity when seating is obscured. Use NO_GAP_OR_SEPARATION_VISIBLE with NO_DEFECT_VISIBLE only when the image clearly shows the interface fully assembled. A visible gap, offset lip, pipe end outside a coupler, displaced clamp, missing retainer, or misaligned mating edge must never be treated as normal or downgraded to residue merely because the primary component is recognizable.
