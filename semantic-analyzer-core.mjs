...
function graphPidMeasurement(text, labels) {
  ...
  let current=null,minimum=null,maximum=null;
  for(const segment of segments){
    const rangeMatch=segment.match(/([-+]?\d+(?:\.\d+)?)\s*(?:%|V|RPM)?\s*(?:to|through|\u2013|\u2014)\s*([-+]?\d+(?:\.\d+)?)\s*(?:%|V|RPM)?/i);
    if(rangeMatch){minimum=Number(rangeMatch[1]);maximum=Number(rangeMatch[2]);continue;}
    const valueMatch=segment.match(new RegExp(`\\b(?:${labelPattern})\\b[^-+\\d]{0,32}([-+]?\\d+(?:\\.\\d+)?)`,'i'));
    if(!valueMatch)continue;
    const value=Number(valueMatch[1]);
    if(/\b(?:minimum|min)\b/i.test(segment))minimum=value;
    else if(/\b(?:maximum|max)\b/i.test(segment))maximum=value;
    else if(/\b(?:current|currently|now)\b/i.test(segment)||current===null)current=value;
  }
  return {current,range:minimum!==null&&maximum!==null?{minimum,maximum}:null};
}
...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const interpretation=(graph.interpretation||[]).filter(item=>!forbidden.some(rule=>rule.test(item))).filter(item=>temporalClaimSupported(item)).filter(item=>!dynamic||!staticContradiction.test(item)).filter(item=>dynamic||!temporalInference.test(item)).filter(item=>dynamic||!hasDisplayedRanges||!/(?:negative|positive).{0,24}(?:fuel trim|long-term|short-term)/i.test(item)).filter(item=>dynamic||!/(?:\blean\b|\brich\b|stuck lean|stuck rich)/i.test(item)).filter(item=>dynamic||supportedCriterion||!/(?:mildly abnormal|\babnormal\b|\bpass(?:es|ed)?\b|\bfail(?:s|ed|ure)?\b)/i.test(item)).filter(item=>!(rearO2!==null&&rearO2>=.7&&/(?:low.{0,20}lean|lean.{0,20}low)/i.test(item))).filter(item=>!(wideband&&/0\.1.{0,20}0\.9|narrowband/i.test(item)));
  ...
  const reasoningEvidence={analysisMode,pidPresentationType:dynamic?'TIME_SERIES_GRAPH':'STATIC_NUMERIC',traceEvidence:dynamic?'DETECTED':'NOT_DETECTED',snapshotStatisticalEvidence:hasDisplayedRanges?'CURRENT_MIN_MAX':'CURRENT_ONLY',visibleTraceEvidence:dynamic?'PRESENT':'ABSENT',temporalClaimEvidenceGate:'APPLIED',exactXAxisTimeScale,temporalBehavior,displayedRangesAvailable:hasDisplayedRanges,sensorTypeDetected:wideband?'WIDEBAND_AIR_FUEL_AND_CONVENTIONAL_DOWNSTREAM_O2':rearO2!==null?'CONVENTIONAL_NARROWBAND_O2':'NOT_CONFIRMED',fuelTrimPolarity:stft!==null||ltft!==null?((stft||0)+(ltft||0)<0?'NEGATIVE_PCM_REMOVING_FUEL':(stft||0)+(ltft||0)>0?'POSITIVE_PCM_ADDING_FUEL':'NEAR_ZERO'):'NOT_AVAILABLE',combinedTrim:stft!==null&&ltft!==null?Math.round((stft+ltft)*1000)/1000:null,operatingState:rpm!==null&&coolant!==null&&rpm>=500&&rpm<=1100&&coolant>=160?'WARM_NEAR_IDLE':'NOT_CONFIRMED',dynamicTraceEvidenceAvailable:dynamic,temporalEvidenceSource:dynamic?'VISIBLE_PLOTTED_TRACE_HISTORY':'STATIC_SNAPSHOT_ONLY',supportedCriterionAvailable:supportedCriterion,catalystComparisonEvidenceAvailable:catalystComparison,diagnosticCertainty:dynamic?'SUPPORTED_BY_DYNAMIC_TRACE':supportedCriterion?'SUPPORTED_BY_VISIBLE_CRITERION':'STATIC_SNAPSHOT_INDETERMINATE'};
  ...
  if(rpmReading.current!==null&&rpmReading.range)interpretation.push(`Engine Speed is currently ${rpmReading.current} RPM, with a displayed captured range of ${rpmReading.range.minimum}–${rpmReading.range.maximum} RPM. These statistics do not establish direction or change over time.`);
  ...
  if(rearO2!==null){const range=rearO2Reading.range?`, with a displayed range of ${rearO2Reading.range.minimum.toFixed(3)}–${rearO2Reading.range.maximum.toFixed(3)} V`:'';interpretation.push(dynamic?`O2S B1S2 is currently ${rearO2} V${rearO2>=.7?', on the high-voltage/rich side':rearO2<=.3?', on the low-voltage/lean side':', near the midrange/crossover region'}. Interpret its mixture relationship only from the visible trace behavior.`:`O2S B1S2 is currently ${rearO2} V${range}. This is a ${rearO2<=.3?'low':rearO2>=.7?'high':'midrange'} current voltage reading, but the available snapshot/range evidence is insufficient to determine mixture state, sensor switching behavior, sensor health, or catalyst efficiency.`);}
  ...
  const unreadableOrUncertain=dynamic?[...new Set([...(graph.unreadableOrUncertain||[]).filter(item=>!staticContradiction.test(item)&&!/temporal behavior (?:is )?(?:unreadable|unavailable)/i.test(item)),...(exactXAxisTimeScale==='UNREADABLE'?['Exact horizontal time scale is unreadable; relative trace behavior remains visible.']:[])])]:[...new Set([...(graph.unreadableOrUncertain||[]),hasDisplayedRanges?'Temporal behavior is unreadable or unavailable because minimum/maximum ranges are not a time-based trace.':'Temporal behavior is unreadable or unavailable because this capture contains only instantaneous PID values.'])];
  const finalInterpretation=[...new Set(interpretation)].filter(item=>!dynamic||!staticContradiction.test(item)).slice(0,16),contradictionPresent=dynamic&&finalInterpretation.some(item=>staticContradiction.test(item));
  return {...graph,analysisMode,interpretation:finalInterpretation,diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',nextTest,unreadableOrUncertain,reasoningEvidence,contradictionGuard:contradictionPresent?'FAIL':'PASS'};
}
