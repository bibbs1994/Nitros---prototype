...
export function validateVehicleAreaRelationship(raw) {
  ...
  const observedItems = Array.isArray(raw.observedItems) ? raw.observedItems.slice(0, 8).map((item, index) => {
    ...
    const observedItem=itemText(item.observedItem,240),itemLocationInImage=itemText(item.itemLocationInImage,240),nearestIdentifiableAssembly=itemText(item.nearestIdentifiableAssembly,240),likelyRelationshipOrDestination=itemText(item.likelyRelationshipOrDestination,500),visibleEvidence=itemText(item.visibleEvidence,500),vehicleContextEvidence=itemText(item.vehicleContextEvidence,500),whatCannotBeConfirmed=itemText(item.whatCannotBeConfirmed,500),recommendedNextPhotoVerification=itemText(item.recommendedNextPhotoVerification,500),relationshipConfidence=normalizeSemanticConfidence(item.relationshipConfidence),intendedDestination=itemText(item.intendedDestination,240)||nearestIdentifiableAssembly,intendedRelationship=itemText(item.intendedRelationship,500)||likelyRelationshipOrDestination,physicalConnectionState=['CONNECTED','DISCONNECTED','PARTIALLY_CONNECTED','LOOSE','CANNOT_VERIFY'].includes(item.physicalConnectionState)?item.physicalConnectionState:(hasVerifiedDisconnectionEvidence({visibleEvidence})?'DISCONNECTED':'CANNOT_VERIFY'),physicalStateConfidence=normalizeSemanticConfidence(item.physicalStateConfidence)??relationshipConfidence,visibleStateEvidence=itemText(item.visibleStateEvidence,500)||visibleEvidence;
...
export function reconcileVehicleAreaRelationship(relationship, condition) {
  ...
  const guidance = 'Capture the connector and the component-side electrical receptacle together at close range so physical mating/separation can be verified.';
  const observedItems = [...(relationship.observedItems || [])];
  disconnected.forEach(item => {
    const index = observedItems.findIndex(existing => /\b(?:connector|plug|terminal)\b/i.test(`${existing.observedItem} ${existing.visibleEvidence}`));
    ...
    const intendedRelationship = `Electrical connector for ${intendedDestination}`;
    const visibleStateEvidence = item.directVisibleEvidence || item.visibleEvidence;
    const physical = { intendedDestination, intendedRelationship, physicalConnectionState: 'DISCONNECTED', physicalStateConfidence: item.connectionStateConfidence ?? item.findingConfidence, visibleStateEvidence, likelyRelationshipOrDestination: intendedRelationship, visibleEvidence: visibleStateEvidence, recommendedNextPhotoVerification: guidance };
    if (index >= 0) observedItems[index] = { ...observedItems[index], ...physical };
    else if (observedItems.length < 8) observedItems.push({ observedItem: item.observedObject || 'Electrical connector', itemLocationInImage: item.location || 'Visible component area', nearestIdentifiableAssembly: intendedDestination, relationshipConfidence: item.connectionStateConfidence ?? item.findingConfidence, vehicleContextEvidence: '', whatCannotBeConfirmed: 'Exact component identity cannot be confirmed from this image alone.', ...physical });
  });
  const guardedItems = observedItems.map(item => item.physicalConnectionState === 'DISCONNECTED' ? { ...item, likelyRelationshipOrDestination: item.intendedRelationship || `Electrical connector for ${item.intendedDestination || item.nearestIdentifiableAssembly}`, visibleEvidence: item.visibleStateEvidence || item.visibleEvidence, recommendedNextPhotoVerification: guidance } : item);
  return { ...relationship, status: 'READY', vehicleAreaLocation: areaNeedsRepair && engineAreaVisible ? broadArea : relationship.vehicleAreaLocation, locationConfidence: areaNeedsRepair && engineAreaVisible ? Math.max(70, relationship.locationConfidence || 0) : relationship.locationConfidence, primaryVisibleAssembly: engineAreaVisible && /\b(?:unknown|uncertain|cannot be confirmed)\b/i.test(String(relationship.primaryVisibleAssembly || '')) ? broadArea : relationship.primaryVisibleAssembly, locationEvidence: [...new Set([...(relationship.locationEvidence || []), ...evidence])].slice(0, 12), observedItems: guardedItems, recommendedNextPhotoVerification: guidance };
}
