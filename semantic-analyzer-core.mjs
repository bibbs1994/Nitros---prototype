...
function graphPidMeasurement(text, labels) {
  ...
  return {current,range:rangeValid?range:null,invalidRange:range&&!rangeValid?range:null};
}
...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const redundantGraphCapture=/\b(?:capture|acquire|record).{0,80}(?:signals?|pids?|data).{0,40}(?:over time|time-series|graph)\b/i;
  const proposedNext=(graph.nextTest||[]).filter(item=>!dynamic||!redundantGraphCapture.test(item));
  const nextTest=dynamic?proposedNext.slice(0,3):['Capture upstream A/F sensor and downstream O2 sensor data simultaneously in closed loop over time so response and the relationship between the signals can be evaluated.'];
  ...
  const observed=(graph.observed||[]).filter(item=>!invalidRangeText(item)),valuesAndScales=(graph.valuesAndScales||[]).filter(item=>!invalidRangeText(item));
  ...
  return {...graph,analysisMode,observed,valuesAndScales,interpretation:finalInterpretation,diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',nextTest,unreadableOrUncertain,reasoningEvidence,contradictionGuard:contradictionPresent?'FAIL':'PASS'};
}
