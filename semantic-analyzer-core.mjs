...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const stft=graphNumber(all,'STFT B1'),ltft=graphNumber(all,'LTFT B1'),rpm=graphNumber(all,'Engine speed')??graphNumber(all,'RPM'),coolant=graphNumber(all,'Coolant'),rearO2=graphNumber(all,'O2S B1S2')??graphNumber(all,'B1S2'),afs=graphNumber(all,'AFS B1S1')??graphNumber(all,'A/F B1S1');
  ...
  if(rearO2!==null)interpretation.push(dynamic?`B1S2 is approximately ${rearO2} V at the captured instant${rearO2>=.7?', on the high-voltage/rich side':rearO2<=.3?', on the low-voltage/lean side':', near the midrange/crossover region'}. Interpret its mixture relationship only from the visible trace behavior.`:`B1S2 is approximately ${rearO2} V at the captured instant. This is a ${rearO2<=.3?'low':rearO2>=.7?'high':'midrange'} instantaneous voltage reading. A single static value does not establish mixture state, sensor switching behavior, sensor health, or catalyst efficiency.`);
  ...
  if(!dynamic)interpretation.push('Only instantaneous PID readings are available; no time-based behavior can be determined from this static snapshot.');
  ...
  return {...graph,analysisMode,interpretation:[...new Set(interpretation)].slice(0,16),diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',nextTest,unreadableOrUncertain:dynamic?graph.unreadableOrUncertain:[...new Set([...(graph.unreadableOrUncertain||[]),'Temporal behavior is unreadable or unavailable because this capture contains only instantaneous PID values.'])],reasoningEvidence,contradictionGuard:'PASS'};
}
