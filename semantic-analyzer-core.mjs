...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const interpretation=(graph.interpretation||[]).filter(item=>!forbidden.some(rule=>rule.test(item))).filter(item=>!dynamic||!staticContradiction.test(item)).filter(item=>dynamic||!temporalInference.test(item)).filter(item=>dynamic||!hasDisplayedRanges||!/(?:negative|positive).{0,24}(?:fuel trim|long-term|short-term)/i.test(item)).filter(item=>dynamic||!/(?:\blean\b|\brich\b|stuck lean|stuck rich)/i.test(item)).filter(item=>dynamic||supportedCriterion||!/(?:mildly abnormal|\babnormal\b|\bpass(?:es|ed)?\b|\bfail(?:s|ed|ure)?\b)/i.test(item)).filter(item=>!(rearO2!==null&&rearO2>=.7&&/(?:low.{0,20}lean|lean.{0,20}low)/i.test(item))).filter(item=>!(wideband&&/0\.1.{0,20}0\.9|narrowband/i.test(item)));
...
Set distinguishingFeaturesComplete true only when the selected exact drivetrain type has strong visible discriminators. Otherwise lower component confidence, include the closest competing type in possibleAlternatives and drivetrainDiscrimination.competingCandidate, and explain the ambiguity. Supporting evidence must specifically justify the selected primary component. Secondary items such as driveshaft, exhaust, heat shield, crossmember, transmission, differential, CV axle, or suspension must not override the visually dominant intended subject.`;
  ...
  if (semanticResult.category === 'AUTOMOTIVE_GRAPH') {
    ...
    try {
      ...
      automotiveGraphAnalysis = { ...correctAutomotiveGraphReasoning({...validatedGraph,observed:[...new Set([...validatedGraph.observed,...classificationTraceEvidence.map(item=>`Classifier graph evidence: ${item}`)])],traceFindings:[...new Set([...validatedGraph.traceFindings,...classificationTraceEvidence])]}), semanticRequestId: transactionId, imageHash };
