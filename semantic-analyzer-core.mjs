...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const observed=(graph.observed||[]).map(normalizePidTerminology).filter(item=>!invalidRangeText(item)).filter(item=>dynamic||!unsupportedSnapshotClaim(item)),valuesAndScales=(graph.valuesAndScales||[]).map(normalizePidTerminology).filter(item=>!invalidRangeText(item)).filter(item=>dynamic||!unsupportedSnapshotClaim(item)),traceFindings=dynamic?modelTraceEvidence:[];
  ...
  return {...graph,analysisMode,observed,valuesAndScales,traceFindings,interpretation:finalInterpretation,diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',evidenceInventory,evidenceInventoryStatus:'PASS',unresolvedQuestion,nextTest,nextTestReason,redundantTestCheck:'PASS',candidateNextTestRejected:rejectedCandidates.length?'DUPLICATES ACTIVE EVIDENCE':'NONE',nextTestSelection:'PASS',unreadableOrUncertain,reasoningEvidence,contradictionGuard:contradictionPresent?'FAIL':'PASS'};
}
