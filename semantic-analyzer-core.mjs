...
function graphPidMeasurement(text, labels) {
  ...
  if(rpmPid){const knownPidPattern='(?:LTFT B1|Long FT #1|STFT B1|Short FT #1|O2S B1S2|B1S2|AFS B1S1|A\\/F B1S1|AFS Voltage B1S1|Engine Speed)',ownedRegions=[];for(let index=0;index<tokens.length;index+=1){if(!new RegExp(`\\b(?:${labelPattern})\\b`,'i').test(tokens[index]))continue;const owned=[tokens[index]];for(let nearby=index+1;nearby<tokens.length;nearby+=1){if(new RegExp(`\\b${knownPidPattern}\\b`,'i').test(tokens[nearby])||/^RPM\b\s*:?[\s]*$/i.test(tokens[nearby]))break;owned.push(tokens[nearby]);if(owned.length>=9)break}const region=owned.join(' ');if(/\b(?:current|currently|now|minimum|min|maximum|max)\b/i.test(region))ownedRegions.push(region)}if(ownedRegions.length)segments=ownedRegions;}
...
function canonicalNumericEvidence(name,unit,reading) {
  ...
  const numericRange=hasRange?Object.freeze({minimum,maximum}):null;
  return Object.freeze({pidName:name,rawCurrent:reading.rawCurrent,rawMinimum:reading.rawMinimum,rawMaximum:reading.rawMaximum,current,minimum,maximum,unit,confidence:null,sourceField:'FINAL_NORMALIZED_PID_CURRENT_MIN_MAX',sourceRegions:reading.sourceRegions,bindingStatus:reading.bindingStatus,rejectedCandidates:reading.rejectedCandidates,sign:current===null?'ABSENT':current>tolerance?'POSITIVE':current<-tolerance?'NEGATIVE':'ZERO',currentSign:current===null?'ABSENT':current>tolerance?'POSITIVE':current<-tolerance?'NEGATIVE':'ZERO',numericRange,rangeSign,crossesZero,allObservedPositive:rangeSign==='POSITIVE_ONLY',allObservedNegative:rangeSign==='NEGATIVE_ONLY',includesZero,numericRangeConstant:hasRange&&Math.abs(maximum-minimum)<=tolerance,unreadable:false,currentPresent:current!==null,minimumPresent:minimum!==null,maximumPresent:maximum!==null,currentMinMaxConsistent:consistent,validityState:consistent?'VALID':'NUMERIC_INCONSISTENCY',inconsistencyReason:reading.inconsistencyReason,signNormalizationApplied:reading.signNormalizationApplied});
}
