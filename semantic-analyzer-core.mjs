...
function canonicalNumericEvidence(name,unit,reading) {
  ...
  return {pidName:name,current,minimum,maximum,unit,sign:current===null?'ABSENT':current>tolerance?'POSITIVE':current<-tolerance?'NEGATIVE':'ZERO',numericRange:hasRange?{minimum,maximum}:null,rangeSign,crossesZero,allObservedPositive:rangeSign==='POSITIVE_ONLY',allObservedNegative:rangeSign==='NEGATIVE_ONLY',includesZero,numericRangeConstant:hasRange&&Math.abs(maximum-minimum)<=tolerance,unreadable:false,currentPresent:current!==null,minimumPresent:minimum!==null,maximumPresent:maximum!==null,currentMinMaxConsistent:consistent};
}
