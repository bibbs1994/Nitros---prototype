...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const originalNarrative=[...(graph.observed||[]),...(graph.interpretation||[]),...(graph.unreadableOrUncertain||[])],snapshotConflictCount=dynamic?0:originalNarrative.filter(unsupportedSnapshotClaim).length;
  const interpretation=(graph.interpretation||[]).filter(item=>!forbidden.some(rule=>rule.test(item))).filter(item=>temporalClaimSupported(item)).filter(item=>!dynamic||!staticContradiction.test(item)).filter(item=>dynamic||!temporalInference.test(item)).filter(item=>dynamic||!unsupportedSnapshotClaim(item)).filter(item=>dynamic||!hasDisplayedRanges||!/(?:negative|positive).{0,24}(?:fuel trim|long-term|short-term)/i.test(item)).filter(item=>dynamic||!/(?:\blean\b|\brich\b|stuck lean|stuck rich)/i.test(item)).filter(item=>dynamic||supportedCriterion||!/(?:mildly abnormal|\babnormal\b|\bpass(?:es|ed)?\b|\bfail(?:s|ed|ure)?\b)/i.test(item)).filter(item=>!(rearO2!==null&&rearO2>=.7&&/(?:low.{0,20}lean|lean.{0,20}low)/i.test(item))).filter(item=>!(wideband&&/0\.1.{0,20}0\.9|narrowband/i.test(item)));
  ...
  const observed=(graph.observed||[]).filter(item=>!invalidRangeText(item)).filter(item=>dynamic||!unsupportedSnapshotClaim(item)),valuesAndScales=(graph.valuesAndScales||[]).filter(item=>!invalidRangeText(item)).filter(item=>dynamic||!unsupportedSnapshotClaim(item)),traceFindings=dynamic?(graph.traceFindings||[]):[];
  ...
  return {...graph,analysisMode,observed,valuesAndScales,traceFindings,interpretation:finalInterpretation,diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',nextTest,unreadableOrUncertain,reasoningEvidence,contradictionGuard:contradictionPresent?'FAIL':'PASS'};
}
