...
function graphNumber(text, label) {
  const escaped=label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),match=String(text).match(new RegExp(`\\b${escaped}\\b[^-+\\d]{0,24}([-+]?\\d+(?:\\.\\d+)?)\\s*(?:%|V|volts?|RPM|°?F)?`,'i'));
...
export function correctAutomotiveGraphReasoning(graph) {
  ...
  const preciseMeasurement=labels=>{const labelPattern=labels.map(label=>label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')).join('|'),segments=all.split('|').filter(segment=>new RegExp(`\\b(?:${labelPattern})\\b`,'i').test(segment));let current=null,range=null;for(const segment of segments){const rangeMatch=segment.match(/([-+]?\d+(?:\.\d+)?)\s*%?\s*(?:to|through|\u2013|\u2014)\s*([-+]?\d+(?:\.\d+)?)\s*%?/i);if(rangeMatch)range={minimum:Number(rangeMatch[1]),maximum:Number(rangeMatch[2])};else if(current===null){const valueMatch=segment.match(new RegExp(`\\b(?:${labelPattern})\\b[^-+\\d]{0,24}([-+]?\\d+(?:\\.\\d+)?)`,'i'));if(valueMatch)current=Number(valueMatch[1]);}}return{current,range};};
  const stftReading=preciseMeasurement(['STFT B1','Short FT #1']),ltftReading=preciseMeasurement(['LTFT B1','Long FT #1']),rearO2Reading=preciseMeasurement(['O2S B1S2','B1S2']),stft=stftReading.current,ltft=ltftReading.current,rpm=graphNumber(all,'Engine speed')??graphNumber(all,'RPM'),coolant=graphNumber(all,'Coolant'),rearO2=rearO2Reading.current,afs=graphNumber(all,'AFS B1S1')??graphNumber(all,'A/F B1S1'),displayedRanges=[stftReading.range,ltftReading.range,rearO2Reading.range].filter(Boolean),hasDisplayedRanges=displayedRanges.length>0;
  const temporalSource=[...(graph.traceFindings||[]),...(graph.observed||[])].join(' '),wideband=/\b(?:a\/f sensor|afs|air fuel ratio sensor|wideband|lambda sensor)\b/i.test(all),dynamic=/\b(?:multiple samples|multiple plotted points|plotted trace|time axis|time-series|successive samples|waveform (?:over|across) time|trace (?:switches|oscillates|changes|responds))\b/i.test(temporalSource),supportedCriterion=/\b(?:verified specification|specified (?:range|limit)|pass\/fail criterion|threshold (?:shown|displayed)|dtc-specific criterion)\b/i.test(all),catalystComparison=dynamic&&/(?:b1s1|upstream)/i.test(all)&&/(?:b1s2|downstream)/i.test(all);
  ...
  const interpretation=(graph.interpretation||[]).filter(item=>!forbidden.some(rule=>rule.test(item))).filter(item=>dynamic||!temporalInference.test(item)).filter(item=>dynamic||!hasDisplayedRanges||!/(?:negative|positive).{0,24}(?:fuel trim|long-term|short-term)/i.test(item)).filter(item=>dynamic||!/(?:\blean\b|\brich\b|stuck lean|stuck rich)/i.test(item)).filter(item=>dynamic||supportedCriterion||!/(?:mildly abnormal|\babnormal\b|\bpass(?:es|ed)?\b|\bfail(?:s|ed|ure)?\b)/i.test(item)).filter(item=>!(rearO2!==null&&rearO2>=.7&&/(?:low.{0,20}lean|lean.{0,20}low)/i.test(item))).filter(item=>!(wideband&&/0\.1.{0,20}0\.9|narrowband/i.test(item)));
  const analysisMode=dynamic?'TEMPORAL_GRAPH':'PID_SNAPSHOT';
  const reasoningEvidence={analysisMode,displayedRangesAvailable:hasDisplayedRanges,sensorTypeDetected:wideband?'WIDEBAND_AIR_FUEL_AND_CONVENTIONAL_DOWNSTREAM_O2':rearO2!==null?'CONVENTIONAL_NARROWBAND_O2':'NOT_CONFIRMED',fuelTrimPolarity:stft!==null||ltft!==null?((stft||0)+(ltft||0)<0?'NEGATIVE_PCM_REMOVING_FUEL':(stft||0)+(ltft||0)>0?'POSITIVE_PCM_ADDING_FUEL':'NEAR_ZERO'):'NOT_AVAILABLE',combinedTrim:stft!==null&&ltft!==null?Math.round((stft+ltft)*1000)/1000:null,operatingState:rpm!==null&&coolant!==null&&rpm>=500&&rpm<=1100&&coolant>=160?'WARM_NEAR_IDLE':'NOT_CONFIRMED',dynamicTraceEvidenceAvailable:dynamic,temporalEvidenceSource:dynamic?'DIRECT_MULTIPLE_SAMPLE_OR_TIME_AXIS_EVIDENCE':'STATIC_SNAPSHOT_ONLY',supportedCriterionAvailable:supportedCriterion,catalystComparisonEvidenceAvailable:catalystComparison,diagnosticCertainty:dynamic?'SUPPORTED_BY_DYNAMIC_TRACE':supportedCriterion?'SUPPORTED_BY_VISIBLE_CRITERION':'STATIC_SNAPSHOT_INDETERMINATE'};
  ...
  for(const [name,reading] of [['Long FT #1',ltftReading],['Short FT #1',stftReading]])if(reading.current!==null&&reading.range){const sign=reading.current<0?'removing':reading.current>0?'adding':'commanding no net';interpretation.push(`Current ${name} is ${reading.current>0?'+':''}${reading.current}%, indicating the PCM is ${sign} fuel at the captured instant. The displayed range extends from ${reading.range.minimum>0?'+':''}${reading.range.minimum}% to ${reading.range.maximum>0?'+':''}${reading.range.maximum}%, so the available snapshot does not support characterizing the entire capture as consistently ${reading.current<0?'negative':reading.current>0?'positive':'zero'}.`);}
  ...
  if(rearO2!==null)interpretation.push(dynamic?`O2S B1S2 is currently ${rearO2} V${rearO2>=.7?', on the high-voltage/rich side':rearO2<=.3?', on the low-voltage/lean side':', near the midrange/crossover region'}. Interpret its mixture relationship only from the visible trace behavior.`:`O2S B1S2 is currently ${rearO2} V, which is a ${rearO2<=.3?'low':rearO2>=.7?'high':'midrange'} instantaneous voltage reading. A single captured value does not establish mixture state, sensor switching behavior, sensor health, or catalyst efficiency.`);
  ...
  if(!dynamic)interpretation.push(hasDisplayedRanges?'Current PID values and displayed minimum/maximum ranges are available, but no readable time-based trace is present. Switching rate, response time, oscillation pattern, correlation, and other dynamic behavior cannot be determined from this snapshot.':'Only instantaneous PID readings are available; no time-based behavior can be determined from this static snapshot.');
  const nextTest=catalystComparison?(graph.nextTest||[]).slice(0,3):['Capture upstream A/F sensor and downstream O2 sensor data simultaneously in closed loop over time so response and the relationship between the signals can be evaluated.'];
  return {...graph,analysisMode,interpretation:[...new Set(interpretation)].slice(0,16),diagnosticSignificance:dynamic||supportedCriterion?graph.diagnosticSignificance:'INDETERMINATE',nextTest,unreadableOrUncertain:dynamic?graph.unreadableOrUncertain:[...new Set([...(graph.unreadableOrUncertain||[]),hasDisplayedRanges?'Temporal behavior is unreadable or unavailable because minimum/maximum ranges are not a time-based trace.':'Temporal behavior is unreadable or unavailable because this capture contains only instantaneous PID values.'])],reasoningEvidence,contradictionGuard:'PASS'};
}
...
Set distinguishingFeaturesComplete true only when the selected exact drivetrain type has strong visible discriminators. Otherwise lower component confidence, include the closest competing type in possibleAlternatives and drivetrainDiscrimination.competingCandidate, and explain the ambiguity. Supporting evidence must specifically justify the selected primary component. Secondary items such as driveshaft, exhaust, heat shield, crossmember, transmission, differential, CV axle, or suspension must not override the visually dominant intended subject.`;
  ...
  if (semanticResult.category === 'AUTOMOTIVE_GRAPH') {
    ...
    const graphPrompt = `Analyze only the visible pixels of this current automotive diagnostic graph or PID screen. This is a dedicated diagnostic interpretation stage after classification. Extract only labels, units, current numeric values, displayed minimum/maximum ranges, time scales, operating conditions, and vehicle identity that are genuinely readable. Never invent PID or sensor labels, voltages, scales, wire details, connector pins, specifications, OEM procedures, or vehicle-specific facts. Analyze every visible trace and panel, including RPM, fuel trim, oxygen/air-fuel sensors, temperature, pressure, frequency, duty cycle, upstream/downstream behavior, switching/cross-count activity, flatline or bias, correlation, and abnormal patterns when actually supported. Keep current values semantically separate from displayed minimum/maximum ranges, and never characterize an entire range from the sign or state of its current value. Displayed min/max values are not temporal evidence and do not establish switching rate, response time, oscillation, correlation, frequency, or trend. Preserve low non-zero voltage precision exactly as readable; never round a value such as 0.055 V to 0 V. Keep directly visible facts in observed and diagnostic inferences in interpretation, separately rate diagnosticSignificance, and put every limitation in unreadableOrUncertain. A single captured PID value is only an instantaneous reading. Never call it activity, stable, unstable, switching, oscillating, responding, stuck, biased, trending, fluctuating, lean, rich, normal, abnormal, good, bad, or failed unless direct temporal evidence or a verified diagnostic criterion supports that conclusion. Static voltage may be described as low, high, midrange, current, captured, or instantaneous, but must not be converted into a mixture or component state. Without temporal evidence or a visibly verified specification, threshold, DTC-specific criterion, or pass/fail rule, use INDETERMINATE and do not label a high/low snapshot abnormal, mildly abnormal, PASS, or FAIL. Do not apply narrowband rules to A/F, AFS, air-fuel ratio, wideband, or lambda sensors. Positive current trims mean the PCM adds fuel; negative current trims mean it removes fuel at that captured instant only. When same-bank current STFT and LTFT are visible under the same condition, add them but state that a single frame does not establish sustained mixture-control behavior. Use RPM, coolant, speed, loop state, load, and throttle when visible. For catalyst/P0420 analysis prioritize directly visible upstream/downstream traces and time relationships; never condemn a converter from the graph alone, and never clear or condemn a catalyst from one voltage. When only snapshot values or min/max ranges exist, request simultaneous upstream A/F and downstream O2 live data in closed loop over time. Before returning, discard temporal inferences from snapshot-only values and all unsupported diagnostic conclusions. In visibleVehicle, report a vehicle only if identifying text is visibly readable in the image.`;
    ...
    try {
      ...
      automotiveGraphAnalysis = { ...correctAutomotiveGraphReasoning(validateAutomotiveGraph(JSON.parse(extractOutputText(graphBody)))), semanticRequestId: transactionId, imageHash };
