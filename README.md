...
Version 10.13.144 includes direct Vercel Marketplace Upstash environment compatibility and a bounded photo-inspection call architecture. The visual evidence contracts, whole-image coverage, strict physical-state validation, deterministic contradiction handling, and final diagnostic result shape remain authoritative while repeated provider stages are consolidated. The Vercel write path records server-derived OpenAI Responses usage in a Redis/Valkey-compatible REST datastore; the protected admin API reads and aggregates that datastore for `/admin/ai-usage`.

## Photo-inspection provider architecture

The former production path issued separate sequential requests for semantic classification, visual condition, the ten-image regional observation, component identity, and vehicle-area relationships. Candidate localization and each localized verification could add further requests. A normal photo therefore produced five provider calls even with zero retries, and crop verification could raise the intentional fanout to seven or more. The original pixels, policy language, and overlapping reasoning duties were repeatedly transmitted; the production ledger showed that this was intentional stage fanout rather than primarily failed-call retry behavior.

The production server now selects a bounded hybrid path:

1. `COMPREHENSIVE_WHOLE_IMAGE_ANALYSIS` receives the original plus the same nine overlapping regions once. It returns classification, the vehicle-agnostic raw object/relationship inventory, the visible-condition sweep, and normalized locations for at most three high-value verification candidates as one strict structured result. It must inspect the whole image and every region before prioritizing a finding.
2. `COMPONENT_AND_RELATIONSHIP_REASONING` receives the original once plus a compact JSON handoff containing only relevant validated facts. It retains component discrimination, vehicle/area context, expected-component topology, missing-assembly evidence gates, contradiction resistance, and verification guidance.
3. `BATCHED_LOCALIZED_VISUAL_VERIFICATION`, when candidates exist, receives the original once plus all candidate detail/context crops in one request instead of one request per candidate. It independently checks physical mating, seating, routing, retention, breakage, and contradictory evidence.

Stages 2 and 3 depend only on stage 1 and execute concurrently. A normal automotive photo therefore uses two calls without crop candidates or three calls with candidates. The previous staged path remains available only as an internal compatibility/regression path; the Vercel production endpoint explicitly selects the bounded path. Provider/schema failures remain visible and optional localized-verification failure cannot erase a valid comprehensive diagnostic result.

The measured pre-change production baseline was approximately five successful provider calls, 309,000 total tokens, $0.8671 estimated AI cost, and roughly two minutes after the earlier latency hardening (historically three to four minutes). Repository tests establish the call-count change, parallel dependency structure, image-payload counts, output compatibility, and failure isolation without making billable calls. They do not fabricate post-deployment token, cost, or latency measurements. Structurally, the expected production range is a 40–60% call reduction (five to two or three), with token/cost reduction expected from eliminating four independent large-context reasoning passes and replacing repeated output handoffs with compact JSON. Exact token, cost, latency, and quality deltas must be measured from the durable production ledger using the same field-test photos.
...
Stable stage labels cover the comprehensive whole-image pass, combined component/relationship reasoning, batched localized verification, legacy compatibility stages, graph/PID interpretation and recovery, wiring-diagram analysis, and repair-document extraction. Every provider row is explicitly classified as an `INTENTIONAL_STAGE` or `TRUE_RETRY`. HTTP success does not imply schema acceptance: both are stored separately, and a response can be HTTP 200 while its locally parsed contract is `REJECTED`. Provider-level fields that OpenAI omits remain null/`UNAVAILABLE`; the ledger does not invent image resolution, token usage, provider cost, or timing data.
...
Server aggregation groups provider events into one logical photo inspection. It reports total calls, intentional stages, true retries, tokens, image inputs, accounted cost by provenance, end-to-end duration, the separate sum of call durations, recorded execution mode, slowest/costliest stages, timeout counts, and final operation status. The bounded production visual pipeline is recorded as `HYBRID_PARALLEL` when context and localized verification overlap and `SEQUENTIAL` when no localized call is needed; legacy and non-visual paths retain their truthful execution label.
...
4. Refresh the dashboard. Confirm the logical operation appears once with two intentional provider stages when no crop candidate exists or three when localized verification is needed. Confirm no unexpected retry appears, every provider call/retry has its own provider response ID where supplied, all calls share the logical operation ID, and RO A/case/vehicle/VIN attribution matches the captured request.
...
7. Refresh/retry the dashboard read and confirm it does not duplicate any provider response ID. If the diagnostic workflow legitimately retries a provider request, confirm it is labeled `TRUE_RETRY` and retained under the one logical operation rather than counted as an intentional analysis stage.
