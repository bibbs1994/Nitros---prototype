...
(()=>{'use strict';
  ...
  function finalizeRenderedNumericEvidence(graph){
    ...
    const rawRows=reconcileCanonicalNumericRows(graph.numericEvidence),renderedPidEvidence=Object.freeze(rawRows.map(createRenderedNumericTriplet)),assertion=assertFinalRenderedPidEvidence(renderedPidEvidence),finalCanonicalPidEvidence=renderedPidEvidence,failed=assertion.failed,finalRenderValidationStatus=assertion.status;
