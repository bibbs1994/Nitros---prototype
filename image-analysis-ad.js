/* Nitros 10.12.13 static snapshot temporal-inference guard with clean-room transaction isolation. */
(()=>{'use strict';
  const BUILD='10.12.13';
  ...
  function renderResult(run,result){
    ...
    if(graph){const section=document.createElement('section'),list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p>None reliably readable.</p>';section.className='automotive-graph-analysis';section.innerHTML=`<h3>AUTOMOTIVE GRAPH ANALYSIS</h3><strong>Observed:</strong>${list(graph.observed)}<strong>Interpretation:</strong>${list(graph.interpretation)}<strong>Diagnostic Significance:</strong><p>${escapeHtml(String(graph.diagnosticSignificance||'INCONCLUSIVE').replaceAll('_',' ').toLowerCase())}</p><strong>Next Test:</strong>${list(graph.nextTest)}${graph.unreadableOrUncertain?.length?`<strong>Unreadable / uncertain:</strong>${list(graph.unreadableOrUncertain)}`:''}`;host.appendChild(section)}
  ...
  function updateDeveloper(run,extra={}){
    ...
    const values={
      ...
      nitrosActiveClassifier:'NitrosSemanticImageAnalysis / static snapshot temporal-inference guard / 10.12.13',nitrosStaleResultLog:lastStaleMessage,
  ...
  function start(){document.title='Nitros Mobile Technician Portal v10.12.13 — Static Snapshot Temporal-Inference Guard';buildImportUi()}
