...
(()=>{'use strict';
  ...
  function finalizeRenderedNumericEvidence(graph){
    ...
    const format=(value,unit)=>{const numeric=Number(value),precision=String(unit||'').toUpperCase()==='RPM'?0:3;if(!Number.isFinite(numeric))return'unavailable';return`${numeric>0?'+':''}${numeric.toFixed(precision)}${unit||''}`},parseDisplayed=(display,unit)=>{const suffix=String(unit||''),text=String(display||'').trim(),numericText=suffix&&text.endsWith(suffix)?text.slice(0,-suffix.length).trim():text;return Number(numericText)},rawRows=Array.isArray(graph.numericEvidence)?graph.numericEvidence:[];
    const finalCanonicalPidEvidence=Object.freeze(rawRows.map(source=>{
      const unit=String(source.unit||''),complete=source.currentPresent!==false&&source.minimumPresent!==false&&source.maximumPresent!==false&&source.current!==null&&source.minimum!==null&&source.maximum!==null,displayCurrent=complete?format(source.current,unit):'unavailable',displayMinimum=complete?format(source.minimum,unit):'unavailable',displayMaximum=complete?format(source.maximum,unit):'unavailable',current=parseDisplayed(displayCurrent,unit),minimum=parseDisplayed(displayMinimum,unit),maximum=parseDisplayed(displayMaximum,unit),finite=complete&&[current,minimum,maximum].every(Number.isFinite),minimumLessThanOrEqualCurrent=finite&&minimum<=current,currentLessThanOrEqualMaximum=finite&&current<=maximum,minimumLessThanOrEqualMaximum=finite&&minimum<=maximum;
      let invariantResult='NOT_APPLICABLE',violationReason='';
      if(complete){if(!finite){invariantResult='FAIL';violationReason='Displayed Current, Min, and Max must all be numeric.'}else if(!minimumLessThanOrEqualMaximum){invariantResult='FAIL';violationReason='Displayed Min exceeds displayed Max.'}else if(!minimumLessThanOrEqualCurrent){invariantResult='FAIL';violationReason='Displayed Current is below displayed Min.'}else if(!currentLessThanOrEqualMaximum){invariantResult='FAIL';violationReason='Displayed Current exceeds displayed Max.'}else invariantResult='PASS'}
      return Object.freeze({pidName:String(source.pidName||'Unknown PID'),displayCurrent,displayMinimum,displayMaximum,current:finite?current:null,minimum:finite?minimum:null,maximum:finite?maximum:null,unit,minimumLessThanOrEqualCurrent,currentLessThanOrEqualMaximum,minimumLessThanOrEqualMaximum,invariantResult,violationReason});
    })),failed=finalCanonicalPidEvidence.filter(row=>row.invariantResult==='FAIL'),finalRenderValidationStatus=failed.length?'FAIL':'PASS';
    const renderedInvariantLog=Object.freeze(finalCanonicalPidEvidence.map(row=>Object.freeze({pidName:row.pidName,rendered:`Current=${row.displayCurrent} Min=${row.displayMinimum} Max=${row.displayMaximum}`,parsedRendered:`Current=${row.current} Min=${row.minimum} Max=${row.maximum}`,minimumLessThanOrEqualCurrent:row.minimumLessThanOrEqualCurrent?'PASS':'FAIL',currentLessThanOrEqualMaximum:row.currentLessThanOrEqualMaximum?'PASS':'FAIL',minimumLessThanOrEqualMaximum:row.minimumLessThanOrEqualMaximum?'PASS':'FAIL',renderedInvariant:row.invariantResult})));
    const result={...graph,finalCanonicalPidEvidence,renderedInvariantLog,finalRenderValidationStatus,numericValidation:{...(graph.numericValidation||{}),validationStage:'FINAL_RENDERED_VALUES_PRE_INTERPRETATION',authoritativeSource:'FINAL_CANONICAL_PID_DISPLAY_OBJECT',finalRenderValidationStatus,currentMinMaxConsistency:failed.length?'FAIL':graph.numericValidation?.currentMinMaxConsistency||'PASS',invalidPidEvidence:failed.map(row=>row.pidName)},freshResultVerification:failed.length?'FAIL':graph.freshResultVerification};
  ...
  function renderResult(run,result){
    ...
    if(graph){const section=document.createElement('section'),list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p>None required or reliably readable.</p>',heading=graph.analysisMode==='PID_SNAPSHOT'?'AUTOMOTIVE PID SNAPSHOT ANALYSIS':'AUTOMOTIVE PID GRAPH ANALYSIS',rows=graph.finalCanonicalPidEvidence||[],failed=rows.filter(row=>row.invariantResult==='FAIL'),numericFailure=graph.finalRenderValidationStatus==='FAIL',numericRows=rows.length?`<ul>${rows.map(row=>`<li><strong>${escapeHtml(row.pidName)} — ${escapeHtml(row.invariantResult)}</strong>: Current ${escapeHtml(row.displayCurrent)}, Min ${escapeHtml(row.displayMinimum)}, Max ${escapeHtml(row.displayMaximum)}${row.violationReason?`. ${escapeHtml(row.violationReason)}`:''}</li>`).join('')}</ul>`:'',failures=failed.length?`<strong>Numeric Evidence Consistency: FAIL</strong>${numericRows}<p><strong>${escapeHtml(failed.map(row=>`${row.pidName} contains contradictory displayed values: Current ${row.displayCurrent}, Min ${row.displayMinimum}, Max ${row.displayMaximum}. ${row.violationReason} Diagnostic interpretation of this PID is withheld until valid evidence is available.`).join(' '))}</strong></p>`:`<strong>Numeric Evidence Consistency: PASS</strong>${numericRows}`;section.className='automotive-graph-analysis';section.innerHTML=`<h3>${heading}</h3>${failures}<strong>Observed:</strong>${list(graph.observed)}${graph.analysisMode!=='PID_SNAPSHOT'&&graph.traceFindings?.length?`<strong>Trace Behavior:</strong>${list(graph.traceFindings)}`:''}<strong>Interpretation:</strong>${list(graph.interpretation)}<strong>Diagnostic Significance:</strong><p>${numericFailure?'Undetermined — numeric evidence inconsistent':escapeHtml(String(graph.diagnosticSignificance||'INCONCLUSIVE').replaceAll('_',' ').toLowerCase())}</p><strong>Next Test:</strong>${list(graph.nextTest)}${graph.nextTestReason?`<strong>Why:</strong><p>${escapeHtml(graph.nextTestReason)}</p>`:''}${graph.unreadableOrUncertain?.length?`<strong>Unreadable / Uncertain:</strong>${list(graph.unreadableOrUncertain)}`:''}`;host.appendChild(section)}
