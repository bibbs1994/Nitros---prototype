/* Nitros 10.12.7AO interactive guided electrical testing with clean-room transaction isolation. */
(()=>{'use strict';
  ...
  function updateDeveloper(run,extra={}){
    ...
    const values={
      ...
      nitrosActiveClassifier:'NitrosSemanticImageAnalysisAO / interactive guided electrical testing / 10.12.7AO',nitrosStaleResultLog:lastStaleMessage,
      ...
      nitrosDiagramStatus:result?.wiringDiagramAnalysis?.status||'Not run',nitrosDiagramConfidence:result?.wiringDiagramAnalysis?.confidence===null||result?.wiringDiagramAnalysis?.confidence===undefined?'Not provided':`${result.wiringDiagramAnalysis.confidence}%`,nitrosDiagramStructuralEvidence:result?.wiringDiagramAnalysis?.structuralEvidence?.join('; ')||'None',nitrosDiagramComponents:result?.wiringDiagramAnalysis?.detectedComponents?.join('; ')||'None',nitrosDiagramConnectors:result?.wiringDiagramAnalysis?.connectorsAndPins?.join('; ')||'None',nitrosDiagramPowerPath:result?.wiringDiagramAnalysis?.circuitPaths?.map(path=>`${path.label}: ${path.path} — ${path.function}`).join(' | ')||'None',nitrosDiagramGroundPath:'Neutral circuit paths used; no ground function inferred.',nitrosDiagramControlPath:'Circuit functions shown only when diagram-confirmed.',nitrosDiagramUnreadable:result?.wiringDiagramAnalysis?.unreadableFields?.join('; ')||'None',nitrosComponentTestSessionId:run?.componentTestSession?.id||'None',
  ...
  function start(){document.title='Nitros Mobile Technician Portal v10.12.7AO — Interactive Guided Electrical Testing';buildImportUi()}
