...
(()=>{'use strict';
  ...
  function renderResult(run,result){
    ...
    if(component){
      ...
      section.innerHTML=`<h3>SPECIFIC COMPONENT IDENTIFICATION</h3><div class="condition-field"><strong>Status</strong><span>${escapeHtml(component.status)}</span></div><div class="condition-field"><strong>Visible component</strong><span>${escapeHtml(component.primaryComponent)}</span></div><div class="condition-field"><strong>Confidence level</strong><span class="phase2-confidence">${escapeHtml(componentConfidence)}</span></div><div class="condition-field"><strong>System</strong><span>${escapeHtml(component.system||'Not determined')}</span></div><div class="condition-field"><strong>Visible defining evidence</strong>${list(component.supportingEvidence)}</div><div class="condition-field"><strong>Likely connection or destination (not confirmed)</strong>${list(component.likelyConnectionsOrDestinations)}</div><div class="condition-field"><strong>Secondary visible components</strong>${list(component.secondaryComponents)}</div><div class="condition-field"><strong>Possible identification</strong>${list(component.possibleAlternatives)}</div>${component.uncertaintyReason?`<div class="condition-field"><strong>What cannot be confirmed</strong><span>${escapeHtml(component.uncertaintyReason)}</span></div>`:''}`;
