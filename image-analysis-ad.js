...
(()=>{'use strict';
  ...
  async function routeFreshResult(run,result){
    ...
    if(result.category==='AUTOMOTIVE_GRAPH'){
      ...
      const extraction=result.documentRepairInformation,fresh=extraction?.semanticRequestId===run.analyzer.requestId&&extraction?.imageHash===run.imageHash&&extraction?.extractionRunId===run.runId,missing=[...(extraction?.missingRequiredFields||[])],complete=fresh&&extraction?.status==='COMPLETE'&&!missing.length;result.documentRepairInformation={...extraction,missingRequiredFields:missing,verificationMissingFields:[...missing],freshResultVerification:fresh?'PASS':'FAIL',extractionStatus:complete?'COMPLETE':'INCOMPLETE',verificationRunId:run.runId,synchronizationStatus:fresh?'PASS':'FAIL'};
