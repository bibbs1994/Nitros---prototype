...
(()=>{'use strict';
  ...
  function activeVehicleAnalysisContext(){
    ...
    const vehicle={year:/^\d{4}$/.test(year)?year:'',make:clean(context.make,80),model:clean(context.model,100),engine:clean(context.engine,100),fuelType:clean(context.fuelType||context.fuel,60),drivetrain:clean(context.drivetrain||context.transmission,100),configuration:clean(context.configuration||context.vehicleConfiguration,180),vin:/^[A-HJ-NPR-Z0-9]{17}$/.test(vin)?vin:''};
