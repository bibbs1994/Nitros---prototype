/* Nitros 10.13.138 visual condition analyzer timeout recovery and guaranteed structured result. */
(()=>{'use strict';
const BUILD='10.13.138';
  ...
  function renderResult(run,result){
    ...
    if(defectFirst){
      ...
      const list=(items,includeRecommendation=true)=>items.length?`<ul>${items.map(item=>`<li><strong>${escapeHtml(item.location)}</strong> — ${escapeHtml(observation(item))}${includeRecommendation&&item.recommendedVerification?` Recommendation: ${escapeHtml(item.recommendedVerification)}`:''}</li>`).join('')}</ul>`:'<p class="condition-empty">None</p>';
      const section=document.createElement('section');section.className='visual-condition-inspection defect-first-summary';
      section.innerHTML=`<h3>DEFECT-FIRST VISUAL ANALYSIS</h3>${pointerEvidence?`<div class="condition-field"><strong>TECHNICIAN-INDICATED TARGET — PRIMARY INSPECTION AREA</strong><span><strong>Pointer detected:</strong> Yes<br><strong>Primary target:</strong> ${escapeHtml(pointerTarget)}<br><strong>Target type:</strong> ${escapeHtml(pointerTargetType)}<br><strong>Target confidence:</strong> ${electricalPointerTarget?'Possible target — physical confirmation required':'Pointer observed; component not confirmed'}<br><strong>Pointer evidence:</strong> ${escapeHtml(pointerEvidence)}</span><br><span>Analyze this target and its immediate connections before secondary nearby components.</span>${electricalPointerTarget?'<br><strong>Visible circuit inspection:</strong> PERFORMED<br><strong>Component verification guidance:</strong> GENERATED':''}</div>`:''}<div class="condition-field"><strong>VISIBLE DEFECTS</strong>${list(confirmed,false)}</div><div class="condition-field"><strong>POSSIBLE CONCERNS — PHYSICAL VERIFICATION REQUIRED</strong>${list(possible)}</div><div class="condition-field"><strong>CANNOT BE VISUALLY VERIFIED</strong>${list(unverified)}${defectFirst.unableToInspectReason?`<span>${escapeHtml(defectFirst.unableToInspectReason)}</span>`:''}</div>${defectFirst.status==='NO_VISIBLE_CONCERN_DETECTED'?`<div class="condition-field"><strong>RESULT</strong><span>${pointerEvidence?'No definite defect can be confirmed at the technician-indicated location from this image. Physically inspect the indicated target and its wiring/connection before relying on surrounding components.':'No obvious visible defect detected in the areas that can be reliably inspected from this image.'}</span></div>`:''}${defectFirst.safetyDrivabilityImpact?`<div class="condition-field"><strong>SAFETY-CRITICAL OBSERVATION</strong><span>${escapeHtml(defectFirst.safetyDrivabilityImpact)}</span></div>`:''}<div class="condition-field"><strong>RECOMMENDED PHYSICAL CHECK</strong>${list([...defectFirst.recommendedVerification||[],...pointerGuidance].map(item=>({location:'Technician check',visibleEvidence:item,recommendedVerification:''})))}</div>`;
    ...
    if(visualCondition){
      ...
      const canonicalStates=new Map((result.canonicalVisualState?.connectionStates||[]).map(item=>[item.candidateId?`candidate:${item.candidateId}`:`${String(item.observedObject||'').toLowerCase()}|${String(item.location||'').toLowerCase()}`,item]));const canonicalConnection=item=>canonicalStates.get(item.candidateId?`candidate:${item.candidateId}`:`${String(item.observedObject||'').toLowerCase()}|${String(item.location||'').toLowerCase()}`)||item;const overallInspectionConfidence=visualCondition.conditionConfidence===null?'Not provided':`${visualCondition.conditionConfidence}%`,section=document.createElement('section');section.className='visual-condition-inspection';
      const classification=item=>({CLEAR_DEFECT:'CLEAR DEFECT',POSSIBLE_CONCERN:'POSSIBLE CONCERN DETECTED',UNVERIFIED_CONDITION:item.missingContext&&/removed|outside the frame|out of frame/i.test(item.missingContext)?'POSSIBLE REMOVED OR OUT-OF-FRAME COMPONENT':'UNVERIFIED CONDITION',SEATING_NOT_RELIABLY_VISIBLE:'UNABLE TO VERIFY FROM IMAGE',NO_DEFECT_VISIBLE:'NO VISIBLE DEFECT'}[item.findingType]||item.findingType.replaceAll('_',' '));
      const relationshipObserved=Boolean(result.vehicleAreaRelationshipAnalysis?.observedItems?.length),connectionList=visualCondition.connectionAssessments?.length?`<div class="condition-finding-cards">${visualCondition.connectionAssessments.map(original=>{const item={...original,...canonicalConnection(original),connectionStateConfidence:canonicalConnection(original).confidence??original.connectionStateConfidence,directVisibleEvidence:canonicalConnection(original).directVisibleEvidence||original.directVisibleEvidence};return `<article class="condition-field"><strong>Finding location</strong><span>${escapeHtml(item.location)}</span><strong>Observed object</strong><span>${escapeHtml(item.observedObject)}</span><strong>Connection state</strong><span>${escapeHtml(item.connectionState.replaceAll('_',' '))}</span><strong>Connection-state confidence</strong><span class="phase2-confidence">${escapeHtml(`${item.connectionStateConfidence}%`)}</span><strong>Classification</strong><span>${escapeHtml(classification(item))}</span><strong>Direct visible evidence</strong><span>${escapeHtml(item.directVisibleEvidence)}</span>${item.missingContext?`<strong>What cannot be confirmed</strong><span>${escapeHtml(item.missingContext)}</span>`:''}<strong>Finding confidence</strong><span class="phase2-confidence">${escapeHtml(`${item.findingConfidence}%`)}</span><strong>Recommended technician verification</strong><span>${escapeHtml(item.recommendedVerification)}</span>${item.safetyDrivabilityImpact?`<strong>Safety / drivability impact</strong><span>${escapeHtml(item.safetyDrivabilityImpact)}</span>`:''}</article>`}).join('')}</div>`:`<p class="condition-empty">${inspectionUnavailable?'No connection assessment was returned because the visual inspection could not complete.':relationshipObserved?'Possible item relationship observed; exact connection and destination cannot be confirmed from this image.':'No connection could be reliably assessed.'}</p>`;
  ...
  function start(){document.title=`Nitros Mobile Technician Portal v${BUILD} — Visual Condition Analyzer Timeout Recovery / Guaranteed Structured Result — Build 2026-09-02`;buildImportUi()}
