/* Nitros 10.12.17 PID time-series trace preservation / static-snapshot contradiction fix. */
(()=>{'use strict';
  const BUILD='10.12.17';
  ...
  function renderResult(run,result){
    ...
    if(graph){const section=document.createElement('section'),list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p>None reliably readable.</p>',heading=graph.analysisMode==='PID_SNAPSHOT'?'AUTOMOTIVE PID SNAPSHOT ANALYSIS':'AUTOMOTIVE GRAPH ANALYSIS';section.className='automotive-graph-analysis';section.innerHTML=`<h3>${heading}</h3><strong>Observed:</strong>${list(graph.observed)}<strong>Interpretation:</strong>${list(graph.interpretation)}<strong>Diagnostic Significance:</strong><p>${escapeHtml(String(graph.diagnosticSignificance||'INCONCLUSIVE').replaceAll('_',' ').toLowerCase())}</p><strong>Next Test:</strong>${list(graph.nextTest)}${graph.unreadableOrUncertain?.length?`<strong>Unreadable / uncertain:</strong>${list(graph.unreadableOrUncertain)}`:''}`;host.appendChild(section)}
  ...
  function updateDeveloper(run,extra={}){
    ...
    const values={
      ...
      nitrosActiveClassifier:'NitrosSemanticImageAnalysis / PID time-series trace preservation / 10.12.17',nitrosStaleResultLog:lastStaleMessage,
      ...
      nitrosPidPresentationType:result?.automotiveGraphAnalysis?.reasoningEvidence?.pidPresentationType||'Not run',nitrosTraceEvidence:result?.automotiveGraphAnalysis?.reasoningEvidence?.traceEvidence||'Not run',nitrosXAxisTimeScale:result?.automotiveGraphAnalysis?.reasoningEvidence?.exactXAxisTimeScale||'Not run',nitrosTemporalBehavior:result?.automotiveGraphAnalysis?.reasoningEvidence?.temporalBehavior||'Not run',nitrosGraphContradictionGuard:result?.automotiveGraphAnalysis?.contradictionGuard||'Not run',
  ...
  function start(){document.title='Nitros Mobile Technician Portal v10.12.17 — PID Time-Series Trace Preservation / Static-Snapshot Contradiction Fix — Build 2026-08-12';buildImportUi()}
