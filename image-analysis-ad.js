...
(()=>{'use strict';
  ...
  async function routeFreshResult(run,result){
    ...
    if(result.category==='AUTOMOTIVE_GRAPH'){
      ...
      const extraction=result.documentRepairInformation,diagnostic=window.NitrosDiagnosticV10120?.getState?.(),activeDtc=String(diagnostic?.activeDtc||''),fresh=extraction?.semanticRequestId===run.analyzer.requestId&&extraction?.imageHash===run.imageHash,applicable=!!activeDtc&&extraction?.dtcs?.includes(activeDtc),missing=[...(extraction?.missingRequiredFields||[])];if(activeDtc&&!applicable&&!missing.includes('DTC applicability'))missing.unshift('DTC applicability');
      const complete=fresh&&extraction?.status==='COMPLETE'&&applicable&&!missing.length;result.documentRepairInformation={...extraction,missingRequiredFields:missing,dtcApplicability:!activeDtc?'NO ACTIVE DTC':applicable?'APPLICABLE':'NOT APPLICABLE',freshResultVerification:fresh?'PASS':'FAIL',extractionStatus:complete?'COMPLETE':'INCOMPLETE'};
      result.isolationTests=complete?[{id:`document-${run.runId}`,verified:true,dtcs:[...extraction.dtcs],name:extraction.testName||extraction.componentOrCircuit,componentOrCircuit:extraction.componentOrCircuit,testLocation:extraction.testLocation,method:extraction.method,criterion:extraction.criterion,comparator:extraction.comparator,minimum:extraction.minimum,maximum:extraction.maximum,requestedResult:extraction.requestedResult}]:[];
