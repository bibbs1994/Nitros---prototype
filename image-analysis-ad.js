...
(()=>{'use strict';
  ...
  function normalizeVisualConditionInspection(raw,category,run){
    ...
    return {status:raw.status,conditionConfidence:confidence,rawConditionConfidence:raw.rawConditionConfidence??null,normalizedConditionConfidence:confidence,observedCondition:stringArray(raw.observedCondition||[],'visual condition observedCondition'),possibleConcerns,noVisibleConcernMessage:String(raw.noVisibleConcernMessage||'').trim(),unableToInspectReason:raw.unableToInspectReason?String(raw.unableToInspectReason).trim():null,visibleEvidence:stringArray(raw.visibleEvidence||[],'visual condition visibleEvidence'),recommendedVerification:stringArray(raw.recommendedVerification||[],'visual condition recommendedVerification'),semanticRequestId:raw.semanticRequestId,imageHash:raw.imageHash};
  }
  ...
  function renderResult(run,result){
    ...
    if(visualCondition){
      const list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'None';
      const concernList=visualCondition.possibleConcerns?.length?`<ul>${visualCondition.possibleConcerns.map(concern=>`<li><strong>Location:</strong> ${escapeHtml(concern.location)}<br><strong>What appears abnormal:</strong> ${escapeHtml(concern.appearance)}<br><strong>Recommended verification:</strong> ${escapeHtml(concern.recommendedVerification)}<br><em>Confirmation by physical inspection is required.</em></li>`).join('')}</ul>`:'None';
      ...
      section.innerHTML=`<h3>VISUAL CONDITION INSPECTION</h3><strong>Inspection status:</strong> ${escapeHtml(visualCondition.status.replaceAll('_',' '))}<br><strong>Condition confidence:</strong> <span class="phase2-confidence">${escapeHtml(confidence)}</span><br><strong>Observed condition:</strong>${list(visualCondition.observedCondition)}<strong>Possible concern:</strong>${concernList}${visualCondition.status==='NO_VISIBLE_CONCERN_DETECTED'?`<p>${escapeHtml(visualCondition.noVisibleConcernMessage||'No visible defect can be confirmed from this image. Inspect the component physically before making a repair decision.')}</p>`:''}${visualCondition.status==='UNABLE_TO_INSPECT'||visualCondition.status==='FAILED'?`<strong>Unable to inspect:</strong> ${escapeHtml(visualCondition.unableToInspectReason||'A reliable visual assessment could not be completed.')}`:''}<strong>Visible evidence:</strong>${list(visualCondition.visibleEvidence)}<strong>Recommended verification:</strong>${list(visualCondition.recommendedVerification)}`;
      host.appendChild(section);
    }
