...
(()=>{'use strict';
  ...
  async function routeFreshResult(run,result){
    ...
    if(result.category==='AUTOMOTIVE_GRAPH'){
      ...
      const graph=result.automotiveGraphAnalysis,verified=graph.status!=='FAILED'&&graph.freshResultVerification==='PASS'&&graph.semanticConsistencyStatus!=='FAIL';
  ...
  function renderResult(run,result){
    ...
    if(graph){const section=document.createElement('section'),list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p>None required or reliably readable.</p>',heading=graph.analysisMode==='PID_SNAPSHOT'?'AUTOMOTIVE PID SNAPSHOT ANALYSIS':'AUTOMOTIVE PID GRAPH ANALYSIS',numericFailure=graph.numericValidation?.currentMinMaxConsistency==='FAIL',failures=graph.evidenceConsistencyFailures?.length?`<strong>Numeric Evidence Consistency: FAIL</strong><ul>${graph.evidenceConsistencyFailures.map(item=>`<li><strong>${escapeHtml(item.pidName)}</strong> — Current: ${escapeHtml(item.current)}${escapeHtml(item.unit)}, Min: ${escapeHtml(item.minimum)}${escapeHtml(item.unit)}, Max: ${escapeHtml(item.maximum)}${escapeHtml(item.unit)}. <strong>Reason:</strong> ${escapeHtml(item.failure)}</li>`).join('')}</ul><p><strong>Diagnostic interpretation blocked. Re-read or reacquire the evidence before continuing.</strong></p>`:'';section.className='automotive-graph-analysis';section.innerHTML=numericFailure?`<h3>${heading}</h3>${failures}`:`<h3>${heading}</h3>${failures}<strong>Observed:</strong>${list(graph.observed)}${graph.analysisMode!=='PID_SNAPSHOT'?`<strong>Trace Behavior:</strong>${list(graph.traceFindings)}`:''}<strong>Interpretation:</strong>${list(graph.interpretation)}<strong>Diagnostic Significance:</strong><p>${escapeHtml(String(graph.diagnosticSignificance||'INCONCLUSIVE').replaceAll('_',' ').toLowerCase())}</p><strong>Next Test:</strong>${list(graph.nextTest)}${graph.nextTestReason?`<strong>Why:</strong><p>${escapeHtml(graph.nextTestReason)}</p>`:''}${graph.unreadableOrUncertain?.length?`<strong>Unreadable / Uncertain:</strong>${list(graph.unreadableOrUncertain)}`:''}`;host.appendChild(section)}
