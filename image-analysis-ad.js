...
(()=>{'use strict';
  ...
  function renderResult(run,result){
    ...
    if(visualCondition){
      ...
      const relationshipObserved=Boolean(result.vehicleAreaRelationshipAnalysis?.observedItems?.length),connectionList=visualCondition.connectionAssessments?.length?`<div class="condition-finding-cards">${visualCondition.connectionAssessments.map(item=>`<article class="condition-field"><strong>Finding location</strong><span>${escapeHtml(item.location)}</span><strong>Observed object</strong><span>${escapeHtml(item.observedObject)}</span><strong>Connection state</strong><span>${escapeHtml(item.connectionState.replaceAll('_',' '))}</span><strong>Connection-state confidence</strong><span class="phase2-confidence">${escapeHtml(`${item.connectionStateConfidence}%`)}</span><strong>Physical mating destination</strong><span>${escapeHtml(item.physicalMatingDestination||'Physical mating destination not visually confirmed.')}</span>${item.matingDestinationConfidence!==null&&item.matingDestinationConfidence!==undefined?`<strong>Mating-destination confidence</strong><span class="phase2-confidence">${escapeHtml(`${item.matingDestinationConfidence}%`)}</span>`:''}<strong>Classification</strong><span>${escapeHtml(classification(item))}</span><strong>Direct visible evidence</strong><span>${escapeHtml(item.directVisibleEvidence)}</span>${item.missingContext?`<strong>What cannot be confirmed</strong><span>${escapeHtml(item.missingContext)}</span>`:''}<strong>Finding confidence</strong><span class="phase2-confidence">${escapeHtml(`${item.findingConfidence}%`)}</span><strong>Recommended technician verification</strong><span>${escapeHtml(item.recommendedVerification)}</span>${item.safetyDrivabilityImpact?`<strong>Safety / drivability impact</strong><span>${escapeHtml(item.safetyDrivabilityImpact)}</span>`:''}</article>`).join('')}</div>`:`<p class="condition-empty">${inspectionUnavailable?'No connection assessment was returned because the visual inspection could not complete.':relationshipObserved?'Possible item relationship observed; exact connection and destination cannot be confirmed from this image.':'No connection could be reliably assessed.'}</p>`;
