...
(()=>{'use strict';
  ...
  function renderResult(run,result){
    ...
    if(visualCondition){
      ...
      section.innerHTML=`<h3>VISUAL CONDITION INSPECTION</h3><div class="condition-field"><strong>Inspection status</strong><span>${escapeHtml(visualCondition.status.replaceAll('_',' '))}</span></div><div class="condition-field"><strong>Condition confidence</strong><span class="phase2-confidence">${escapeHtml(confidence)}</span></div><div class="condition-field"><strong>Observed condition</strong>${list(visualCondition.observedCondition)}</div><div class="condition-field"><strong>Possible concern</strong>${concernList}</div><div class="condition-field"><strong>Connection assessment</strong>${connectionList}</div>${visualCondition.status==='NO_VISIBLE_CONCERN_DETECTED'?`<div class="condition-field"><strong>Result</strong><p>${escapeHtml(visualCondition.noVisibleConcernMessage||'No visible defect can be confirmed from this image. Inspect the component physically before making a repair decision.')}</p></div>`:''}${visualCondition.status==='UNABLE_TO_INSPECT'||visualCondition.status==='FAILED'?`<div class="condition-field"><strong>Unable to inspect</strong><span>${escapeHtml(visualCondition.unableToInspectReason||'A reliable visual assessment could not be completed.')}</span></div>`:''}<div class="condition-field"><strong>Exact visible evidence</strong>${list(visualCondition.visibleEvidence)}</div><div class="condition-field"><strong>Recommended physical verification</strong>${list(visualCondition.recommendedVerification)}</div>${visualCondition.safetyDrivabilityImpact?`<div class="condition-field"><strong>Safety / drivability impact</strong><span>${escapeHtml(visualCondition.safetyDrivabilityImpact)}</span></div>`:''}`;
      host.appendChild(section);
    }
