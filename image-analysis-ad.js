...
(()=>{'use strict';
  ...
  function normalizeDocumentRepairInformation(raw,category,run){
    ...
    const text=(field,limit)=>String(raw[field]||'').trim().slice(0,limit),dtcs=Array.isArray(raw.dtcs)?[...new Set(raw.dtcs.filter(code=>/^[PCBU][0-9A-F]{4}$/.test(code)))]:[],allowedMissing=['DTC applicability','component or circuit','test location','test method','criterion','requested technician result'];
    ...
    const required=[['DTC applicability',result.dtcApplicability],['component or circuit',result.componentOrCircuit],['test location',result.testLocation],['test method',result.method],['criterion',result.criterion],['requested technician result',result.requestedResult]];result.missingRequiredFields=[...new Set([...result.missingRequiredFields,...required.filter(([,value])=>!value).map(([field])=>field)])];if(result.status==='COMPLETE'&&result.missingRequiredFields.length)result.status='INCOMPLETE';return result
  }
  ...
  async function routeFreshResult(run,result){
    ...
    if(result.category==='AUTOMOTIVE_GRAPH'){
      ...
      const extraction=result.documentRepairInformation,diagnostic=window.NitrosDiagnosticV10120?.getState?.(),activeDtc=String(diagnostic?.activeDtc||''),fresh=extraction?.semanticRequestId===run.analyzer.requestId&&extraction?.imageHash===run.imageHash,activeDtcMatch=!!activeDtc&&extraction?.dtcs?.includes(activeDtc),resolvedApplicability=extraction?.dtcApplicability||'',applicable=resolvedApplicability==='APPLICABLE'&&activeDtcMatch,missing=[...(extraction?.missingRequiredFields||[])],complete=fresh&&extraction?.status==='COMPLETE'&&!missing.length;result.documentRepairInformation={...extraction,missingRequiredFields:missing,dtcApplicability:resolvedApplicability,freshResultVerification:fresh?'PASS':'FAIL',extractionStatus:complete?'COMPLETE':'INCOMPLETE'};
      result.isolationTests=complete&&applicable?[{id:`document-${run.runId}`,verified:true,dtcs:[...extraction.dtcs],name:extraction.testName||extraction.componentOrCircuit,componentOrCircuit:extraction.componentOrCircuit,testLocation:extraction.testLocation,method:extraction.method,criterion:extraction.criterion,comparator:extraction.comparator,minimum:extraction.minimum,maximum:extraction.maximum,requestedResult:extraction.requestedResult}]:[];
