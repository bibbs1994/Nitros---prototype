...
(()=>{'use strict';
  ...
  function parseFinalRenderedNumber(display){
    const text=typeof display==='string'?display.trim():'';if(!/^[+-]?(?:\d+(?:\.\d+)?|\.\d+)$/.test(text))return null;const value=Number(text);return Number.isFinite(value)?value:null;
  }
  function validateFinalRenderedPid(row){
    const currentDisplay=String(row?.currentDisplay??row?.displayCurrent??''),minDisplay=String(row?.minDisplay??row?.displayMinimum??''),maxDisplay=String(row?.maxDisplay??row?.displayMaximum??''),parsedCurrent=parseFinalRenderedNumber(currentDisplay),parsedMin=parseFinalRenderedNumber(minDisplay),parsedMax=parseFinalRenderedNumber(maxDisplay),numeric=[parsedCurrent,parsedMin,parsedMax].every(value=>value!==null),minimumLessThanOrEqualCurrent=numeric&&parsedMin<=parsedCurrent,currentLessThanOrEqualMaximum=numeric&&parsedCurrent<=parsedMax,minimumLessThanOrEqualMaximum=numeric&&parsedMin<=parsedMax;
    let invariantFailureReason='';if(!numeric)invariantFailureReason='A final rendered Current, Min, or Max value is missing or malformed.';else if(!minimumLessThanOrEqualMaximum)invariantFailureReason='Rendered Min exceeds rendered Max.';else if(!minimumLessThanOrEqualCurrent)invariantFailureReason='Rendered Current is below rendered Min.';else if(!currentLessThanOrEqualMaximum)invariantFailureReason='Rendered Current exceeds rendered Max.';
    const invariantPass=!invariantFailureReason;return Object.freeze({pidName:String(row?.pidName||'Unknown PID'),unit:String(row?.unit||''),currentDisplay,minDisplay,maxDisplay,parsedCurrent,parsedMin,parsedMax,invariantPass,invariantFailureReason,minimumLessThanOrEqualCurrent,currentLessThanOrEqualMaximum,minimumLessThanOrEqualMaximum,displayCurrent:currentDisplay,displayMinimum:minDisplay,displayMaximum:maxDisplay,current:parsedCurrent,minimum:parsedMin,maximum:parsedMax,invariantResult:invariantPass?'PASS':'FAIL',violationReason:invariantFailureReason});
  }
  function assertFinalRenderedPidEvidence(rows){const renderedPidEvidence=Object.freeze((Array.isArray(rows)?rows:[]).map(validateFinalRenderedPid)),failed=renderedPidEvidence.filter(row=>!row.invariantPass);return Object.freeze({renderedPidEvidence,failed,status:failed.length?'FAIL':'PASS'})}
  function finalizeRenderedNumericEvidence(graph){
    if(!graph||typeof graph!=='object')return graph;
    const format=(value,unit)=>{const numeric=Number(value),precision=String(unit||'').toUpperCase()==='RPM'?0:3;return Number.isFinite(numeric)?numeric.toFixed(precision):''},rawRows=Array.isArray(graph.numericEvidence)?graph.numericEvidence:[],displayRows=rawRows.map(source=>{const complete=source.currentPresent!==false&&source.minimumPresent!==false&&source.maximumPresent!==false&&source.current!==null&&source.minimum!==null&&source.maximum!==null;return{pidName:source.pidName,unit:source.unit,currentDisplay:complete?format(source.current,source.unit):'',minDisplay:complete?format(source.minimum,source.unit):'',maxDisplay:complete?format(source.maximum,source.unit):''}}),assertion=assertFinalRenderedPidEvidence(displayRows),renderedPidEvidence=assertion.renderedPidEvidence,finalCanonicalPidEvidence=renderedPidEvidence,failed=assertion.failed,finalRenderValidationStatus=assertion.status;
    const renderedInvariantLog=Object.freeze(renderedPidEvidence.map(row=>Object.freeze({pidName:row.pidName,rendered:`Current=${row.currentDisplay} Min=${row.minDisplay} Max=${row.maxDisplay}`,parsedRendered:`Current=${row.parsedCurrent} Min=${row.parsedMin} Max=${row.parsedMax}`,minimumLessThanOrEqualCurrent:row.minimumLessThanOrEqualCurrent?'PASS':'FAIL',currentLessThanOrEqualMaximum:row.currentLessThanOrEqualMaximum?'PASS':'FAIL',minimumLessThanOrEqualMaximum:row.minimumLessThanOrEqualMaximum?'PASS':'FAIL',renderedInvariant:row.invariantResult})));
    const result={...graph,renderedPidEvidence,finalCanonicalPidEvidence,renderedInvariantLog,finalRenderValidationStatus,numericValidation:{...(graph.numericValidation||{}),validationStage:'POST_RENDER_STRING_HARD_GATE',authoritativeSource:'EXACT_FINAL_RENDERED_PID_STRINGS',finalRenderValidationStatus,currentMinMaxConsistency:failed.length?'FAIL':graph.numericValidation?.currentMinMaxConsistency||'PASS',invalidPidEvidence:failed.map(row=>row.pidName)},freshResultVerification:failed.length?'FAIL':graph.freshResultVerification};
