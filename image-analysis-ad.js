/* Nitros 10.13.144 Vercel Upstash environment compatibility. */
(()=>{'use strict';
const BUILD='10.13.144';
  ...
  function syncSemanticStages(run){
    ...
    const relationshipStatus=()=>server.relationshipDiagnosticStatus==='DEGRADED'?'DEGRADED — FALLBACK PRESERVED':server.relationshipDiagnosticStatus==='INDETERMINATE'?'INDETERMINATE — INSUFFICIENT VISIBLE EVIDENCE':server.relationshipOutcome==='RELATIONSHIP_ESTABLISHED'?'PASS — RELATIONSHIP ESTABLISHED':server.relationshipDiagnosticStatus==='PASS'?'PASS — NO ABNORMAL RELATIONSHIP FOUND':server.vehicleAreaRelationshipResultPresent?'INDETERMINATE — INSUFFICIENT VISIBLE EVIDENCE':'FAIL';
    ...
    if(server.vehicleAreaRelationshipAttempted){set(21,server.vehicleAreaRelationshipResultPresent?'PASS':'FAIL');set(22,relationshipStatus());set(23,server.photoGuidanceSource==='fallback'?'DEGRADED — FALLBACK PRESERVED':server.vehicleAreaRelationshipResultPresent?'PASS':'FAIL')}
  ...
  function finalizeAcceptedAnalysisStages(run,result){
    ...
    if(automotive){
      ...
      const relationshipStatus=!complete?'FAIL':relationship.relationshipDiagnosticStatus==='DEGRADED'?'DEGRADED — FALLBACK PRESERVED':relationship.relationshipDiagnosticStatus==='INDETERMINATE'?'INDETERMINATE — INSUFFICIENT VISIBLE EVIDENCE':relationship.relationshipOutcome==='RELATIONSHIP_ESTABLISHED'?'PASS — RELATIONSHIP ESTABLISHED':'PASS — NO ABNORMAL RELATIONSHIP FOUND';
      set(21,complete?'PASS':'FAIL');set(22,relationshipStatus);set(23,!complete?'FAIL':relationship.photoGuidanceSource==='fallback'?'DEGRADED — FALLBACK PRESERVED':'PASS');
  ...
  function normalizeVisualConditionInspection(raw,category,run){
    ...
    const connectionAssessments=Array.isArray(raw.connectionAssessments)?raw.connectionAssessments.map(item=>({findingId:item?.findingId?String(item.findingId).trim():null,candidateId:item?.candidateId?String(item.candidateId).trim():null,location:String(item?.location||'').trim(),observedObject:String(item?.observedObject||'Connection candidate').trim(),seatingStatus:String(item?.seatingStatus||'NOT_RELIABLY_VISIBLE').trim(),findingType:String(item?.findingType||'').trim(),defectKind:item?.defectKind?String(item.defectKind).trim():null,diagnosticPriority:item?.diagnosticPriority?String(item.diagnosticPriority).trim():null,priorityRank:Number.isInteger(item?.priorityRank)?item.priorityRank:null,priorityScore:Number.isFinite(item?.priorityScore)?item.priorityScore:null,primaryFinding:item?.primaryFinding===true,severity:String(item?.severity||'').trim(),findingConfidence:item?.findingConfidence===null?null:Number(item?.findingConfidence),connectionState:String(item?.connectionState||'INDETERMINATE').trim(),connectionStateConfidence:item?.connectionStateConfidence===null?null:Number(item?.connectionStateConfidence??item?.findingConfidence),visibleEvidence:String(item?.visibleEvidence||'').trim(),directVisibleEvidence:String(item?.directVisibleEvidence||item?.visibleEvidence||'').trim(),matingComponentVisible:item?.matingComponentVisible===true,directDamageVisible:item?.directDamageVisible===true,missingContext:item?.missingContext?String(item.missingContext).trim():null,recommendedVerification:String(item?.recommendedVerification||'').trim(),safetyDrivabilityImpact:item?.safetyDrivabilityImpact?String(item?.safetyDrivabilityImpact).trim():null})).filter(item=>item.location&&['SEPARATION_OR_GAP_VISIBLE','POSSIBLE_IMPROPER_SEATING','NO_GAP_OR_SEPARATION_VISIBLE','NOT_RELIABLY_VISIBLE','COMPONENT_OR_CONNECTION_CONTEXT_NOT_VISIBLE'].includes(item.seatingStatus)&&['CLEAR_DEFECT','POSSIBLE_CONCERN','UNVERIFIED_CONDITION','RESIDUE_OR_STAINING','SEATING_NOT_RELIABLY_VISIBLE','NO_DEFECT_VISIBLE'].includes(item.findingType)&&['CONNECTED_VERIFIED','DISCONNECTED_VERIFIED','PARTIALLY_SEATED','PARTIALLY_SEATED_OR_SUSPECTED','LOOSE_OR_SUSPECT','INDETERMINATE','CONNECTED_BUT_RETENTION_NOT_VERIFIABLE','UNABLE_TO_DETERMINE_FROM_IMAGE'].includes(item.connectionState)&&['CRITICAL','HIGH','MODERATE','LOW','UNDETERMINED'].includes(item.severity)&&Number.isFinite(item.findingConfidence)&&item.findingConfidence>=0&&item.findingConfidence<=100&&Number.isFinite(item.connectionStateConfidence)&&item.connectionStateConfidence>=0&&item.connectionStateConfidence<=100&&item.visibleEvidence&&item.recommendedVerification).slice(0,12):[];
    ...
    const reconciliationStatus=['PASS','PARTIAL'].includes(raw?.crossFindingConsistency?.status)?raw.crossFindingConsistency.status:'FAIL';
    const conflictEvaluation=raw?.conflictEvaluation&&typeof raw.conflictEvaluation==='object'?{status:raw.conflictEvaluation.status==='PASS'?'PASS':'FAIL',executed:raw.conflictEvaluation.executed===true,conflicts:Array.isArray(raw.conflictEvaluation.conflicts)?raw.conflictEvaluation.conflicts:[],hasUnresolvedConflict:raw.conflictEvaluation.hasUnresolvedConflict===true,reason:String(raw.conflictEvaluation.reason||'UNKNOWN').slice(0,120)}:null;
    const finalEvidencePromotion=raw?.finalEvidencePromotion&&typeof raw.finalEvidencePromotion==='object'?{status:['PASS','BLOCKED_CONFLICT'].includes(raw.finalEvidencePromotion.status)?raw.finalEvidencePromotion.status:'FAIL',eligible:raw.finalEvidencePromotion.eligible===true,promotedCount:Number.isInteger(raw.finalEvidencePromotion.promotedCount)?raw.finalEvidencePromotion.promotedCount:0,reason:String(raw.finalEvidencePromotion.reason||'UNKNOWN').slice(0,120),evidence:Array.isArray(raw.finalEvidencePromotion.evidence)?raw.finalEvidencePromotion.evidence:[]}:null;
    const visibleFindings=Array.isArray(raw.visibleFindings)?raw.visibleFindings.filter(item=>item&&typeof item==='object').slice(0,24):connectionAssessments.map(item=>({findingType:item.findingType,physicalState:item.connectionState,observedObject:item.observedObject,location:item.location,evidence:item.directVisibleEvidence||item.visibleEvidence,confidence:item.connectionStateConfidence,recommendedVerification:item.recommendedVerification})),observedObjects=Array.isArray(raw.observedObjects)?raw.observedObjects.filter(item=>item&&typeof item==='object').slice(0,24):[],physicalRelationships=Array.isArray(raw.physicalRelationships)?raw.physicalRelationships.filter(item=>item&&typeof item==='object').slice(0,24):[],structuredEvidence=stringArray(raw.evidence||raw.visibleEvidence||[],'visual condition evidence'),uncertainty=stringArray(raw.uncertainty||[],'visual condition uncertainty'),visibleConditionAssessed=raw.visibleConditionAssessed===true||visibleFindings.length>0||raw.status==='NO_VISIBLE_CONCERN_DETECTED',inspectionCompleted=raw.inspectionCompleted===true||visibleConditionAssessed;
    return {status:raw.status,inspectionCompleted,visibleConditionAssessed,visibleFindings,observedObjects,physicalRelationships,evidence:structuredEvidence,uncertainty,primaryFindingId:raw.primaryFindingId?String(raw.primaryFindingId):connectionAssessments.find(item=>item.primaryFinding)?.findingId||null,visualCandidateSet:Array.isArray(raw.visualCandidateSet)?raw.visualCandidateSet.filter(item=>item&&typeof item==='object').slice(0,24):[],evidenceLedger:raw.evidenceLedger&&typeof raw.evidenceLedger==='object'?raw.evidenceLedger:null,confidenceCalibration:raw.confidenceCalibration&&typeof raw.confidenceCalibration==='object'?raw.confidenceCalibration:null,componentIdentity:raw.componentIdentity?String(raw.componentIdentity).trim():null,componentIdentityConfidence:raw.componentIdentityConfidence===null||raw.componentIdentityConfidence===undefined?null:Number(raw.componentIdentityConfidence),conditionConfidence:confidence,rawConditionConfidence:raw.rawConditionConfidence??null,normalizedConditionConfidence:confidence,observedCondition:stringArray(raw.observedCondition||[],'visual condition observedCondition'),possibleConcerns,connectionAssessments,noVisibleConcernMessage:String(raw.noVisibleConcernMessage||'').trim(),unableToInspectReason:raw.unableToInspectReason?String(raw.unableToInspectReason).trim():null,visibleEvidence:stringArray(raw.visibleEvidence||[],'visual condition visibleEvidence'),recommendedVerification:stringArray(raw.recommendedVerification||[],'visual condition recommendedVerification'),safetyDrivabilityImpact:raw.safetyDrivabilityImpact?String(raw.safetyDrivabilityImpact).trim():null,consistencyCorrections:stringArray(raw.consistencyCorrections||[],'visual condition consistencyCorrections'),crossFindingConsistency:{status:reconciliationStatus,conflictsResolved:raw?.crossFindingConsistency?.conflictsResolved===true,findingCount:Number.isInteger(raw?.crossFindingConsistency?.findingCount)?raw.crossFindingConsistency.findingCount:connectionAssessments.length,rejectedFindingCount:Number.isInteger(raw?.crossFindingConsistency?.rejectedFindingCount)?raw.crossFindingConsistency.rejectedFindingCount:0},conflictEvaluation,finalEvidencePromotion,semanticRequestId:raw.semanticRequestId,imageHash:raw.imageHash};
  }
  function normalizeVehicleAreaRelationship(raw,category,run){
    ...
    const locationConfidence=confidence(raw.locationConfidence),items=Array.isArray(raw.observedItems)?raw.observedItems.map(item=>({observedItem:text(item?.observedItem,240),itemLocationInImage:text(item?.itemLocationInImage,240),nearestIdentifiableAssembly:text(item?.nearestIdentifiableAssembly,240),likelyRelationshipOrDestination:text(item?.likelyRelationshipOrDestination,500),intendedDestination:text(item?.intendedDestination,240),intendedRelationship:text(item?.intendedRelationship,500),physicalConnectionState:['CONNECTED','DISCONNECTED','PARTIALLY_CONNECTED','LOOSE','CANNOT_VERIFY'].includes(item?.physicalConnectionState)?item.physicalConnectionState:'CANNOT_VERIFY',physicalStateConfidence:confidence(item?.physicalStateConfidence),visibleStateEvidence:text(item?.visibleStateEvidence,500),relationshipConfidence:confidence(item?.relationshipConfidence),visibleEvidence:text(item?.visibleEvidence,500),vehicleContextEvidence:text(item?.vehicleContextEvidence,500),whatCannotBeConfirmed:text(item?.whatCannotBeConfirmed,500),recommendedNextPhotoVerification:text(item?.recommendedNextPhotoVerification,500)})).filter(item=>item.observedItem&&item.itemLocationInImage&&item.visibleEvidence&&item.whatCannotBeConfirmed&&item.recommendedNextPhotoVerification):[];
  ...
  function renderResult(run,result){
    ...
    const evidenceLedger=result.visualConditionInspection?.evidenceLedger||null;
    ...
    // Preserve that signal as technician context without allowing pointer prominence or an
    // easier-to-identify neighboring component to replace the whole-image defect ranking.
    ...
    if(defectFirst){
      ...
      const findings=(defectFirst.connectionAssessments||[]).slice().sort((a,b)=>(a.priorityRank??99)-(b.priorityRank??99)||(b.priorityScore??0)-(a.priorityScore??0)||(priority[a.findingType]??9)-(priority[b.findingType]??9));
      ...
      const observation=item=>/\b(?:connector|plug)\b/i.test(item.visibleEvidence)&&/\b(?:disconnect|unplug|separat|gap)\b/i.test(`${item.visibleEvidence} ${item.seatingStatus}`)?`Visible observation: Electrical connector appears disconnected/unplugged. ${item.visibleEvidence}`:item.visibleEvidence;
      const list=(items,includeRecommendation=true)=>items.length?`<ul>${items.map(item=>`<li>${item.primaryFinding?'<strong>PRIMARY DIAGNOSTIC FINDING</strong> — ':''}<strong>${escapeHtml(item.location)}</strong> — ${escapeHtml(observation(item))}${Number.isFinite(item.findingConfidence)?` <strong>Finding confidence:</strong> ${escapeHtml(`${item.findingConfidence}%`)}`:''}${includeRecommendation&&item.recommendedVerification?` <strong>Recommended technician verification:</strong> ${escapeHtml(item.recommendedVerification)}`:''}</li>`).join('')}</ul>`:'<p class="condition-empty">None</p>';
      const section=document.createElement('section');section.className='visual-condition-inspection defect-first-summary';
      const focusedChecks=[...(defectFirst.recommendedVerification||[]),...pointerGuidance].slice(0,2);
      section.innerHTML=`<h3>WHOLE-IMAGE DEFECT PRIORITIZATION</h3>${pointerEvidence?`<div class="condition-field"><strong>TECHNICIAN-INDICATED TARGET — CONTEXTUAL INSPECTION AREA</strong><span><strong>Pointer detected:</strong> Yes<br><strong>Context target:</strong> ${escapeHtml(pointerTarget)}<br><strong>Target type:</strong> ${escapeHtml(pointerTargetType)}<br><strong>Target confidence:</strong> ${electricalPointerTarget?'Possible target — physical confirmation required':'Pointer observed; component not confirmed'}<br><strong>Pointer evidence:</strong> ${escapeHtml(pointerEvidence)}</span><br><span>The pointer informs inspection context; primary selection follows the completed whole-image candidate sweep.</span>${electricalPointerTarget?'<br><strong>Visible circuit inspection:</strong> PERFORMED<br><strong>Component verification guidance:</strong> GENERATED':''}</div>`:''}<div class="condition-field"><strong>VISIBLE DEFECTS</strong>${list(confirmed,false)}</div><div class="condition-field"><strong>POSSIBLE CONCERNS — PHYSICAL VERIFICATION REQUIRED</strong>${list(possible)}</div><div class="condition-field"><strong>CANNOT BE VISUALLY VERIFIED</strong>${list(unverified)}${defectFirst.unableToInspectReason?`<span>${escapeHtml(defectFirst.unableToInspectReason)}</span>`:''}</div>${defectFirst.status==='NO_VISIBLE_CONCERN_DETECTED'?`<div class="condition-field"><strong>RESULT</strong><span>${pointerEvidence?'No definite defect can be confirmed at the technician-indicated location from this image. Physically inspect the indicated target and its wiring/connection before relying on surrounding components.':'No obvious visible defect detected in the areas that can be reliably inspected from this image.'}</span></div>`:''}${defectFirst.safetyDrivabilityImpact?`<div class="condition-field"><strong>SAFETY-CRITICAL OBSERVATION</strong><span>${escapeHtml(defectFirst.safetyDrivabilityImpact)}</span></div>`:''}<div class="condition-field"><strong>FOCUSED TECHNICIAN CHECKS — Recommended technician verification</strong>${list(focusedChecks.map(item=>({location:'Technician check',visibleEvidence:item,recommendedVerification:''})))}</div>`;
      host.appendChild(section);
    }
      const component={...result.componentIdentification,primaryComponent:result.canonicalVisualState?.componentIdentity?.primaryComponent||result.componentIdentification?.primaryComponent,status:result.canonicalVisualState?.componentIdentity?.status||result.componentIdentification?.status,normalizedComponentConfidence:result.canonicalVisualState?.componentIdentity?.confidence??result.componentIdentification?.normalizedComponentConfidence};
    if(component){
      ...
      section.innerHTML=`<h3>SPECIFIC COMPONENT IDENTIFICATION</h3><div class="condition-field"><strong>Identification basis</strong><span>${escapeHtml(identificationLabel)}</span></div>${result.vehicleContextApplied?.available?`<div class="condition-field"><strong>Vehicle context used as non-visual reference</strong><span>${escapeHtml(result.vehicleContextApplied.summary||'Active repair-order vehicle context')}</span></div>`:''}<div class="condition-field"><strong>Status</strong><span>${escapeHtml(component.status)}</span></div><div class="condition-field"><strong>Identification level</strong><span>${escapeHtml((component.identificationLevel||'UNCERTAIN_CANDIDATE').replaceAll('_',' '))}</span></div><div class="condition-field"><strong>Reported component or family</strong><span>${escapeHtml(component.primaryComponent)}</span></div><div class="condition-field"><strong>Component-identification confidence</strong><span class="phase2-confidence">${escapeHtml(componentConfidence)}</span></div><div class="condition-field"><strong>Visible defining evidence</strong>${list(evidenceLedger?.componentEvidence||component.supportingEvidence)}</div><div class="condition-field"><strong>What cannot be confirmed</strong><span>${escapeHtml(component.uncertaintyReason||'No additional limitation was returned.')}</span></div><div class="condition-field"><strong>Likely connection or destination (not confirmed)</strong>${list(component.likelyConnectionsOrDestinations)}</div><div class="condition-field"><strong>Secondary visible components</strong>${list(component.secondaryComponents)}</div><div class="condition-field"><strong>Possible identification</strong>${list(component.possibleAlternatives)}</div><div class="condition-field"><strong>System</strong><span>${escapeHtml(component.system||'Not determined')}</span></div>`;
      host.appendChild(section);
    }
    const relationship=result.vehicleAreaRelationshipAnalysis;
    if(relationship){
      const pct=value=>value===null?'Not provided':`${value}%`,list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p class="condition-empty">None</p>',items=relationship.observedItems.length?relationship.observedItems.map(item=>`<article class="condition-field"><strong>Observed item</strong><span>${escapeHtml(item.observedItem)}</span><strong>Item location in image</strong><span>${escapeHtml(item.itemLocationInImage)}</span><strong>Nearest identifiable assembly</strong><span>${escapeHtml(item.nearestIdentifiableAssembly)}</span><strong>Intended destination</strong><span>${escapeHtml(item.intendedDestination||item.nearestIdentifiableAssembly)}</span><strong>Intended relationship</strong><span>${escapeHtml(item.intendedRelationship||item.likelyRelationshipOrDestination)}</span><strong>Current physical state</strong><span>${escapeHtml(item.physicalConnectionState.replaceAll('_',' '))}</span><strong>Physical-state confidence</strong><span class="phase2-confidence">${escapeHtml(pct(item.physicalStateConfidence))}</span><strong>Evidence handoff</strong><span>Direct finding evidence is shown once in the ranked whole-image section above.</span><strong>Relationship confidence</strong><span class="phase2-confidence">${escapeHtml(pct(item.relationshipConfidence))}</span>${item.vehicleContextEvidence?`<strong>Vehicle-context evidence</strong><span>${escapeHtml(item.vehicleContextEvidence)}</span>`:''}<strong>What cannot be confirmed</strong><span>${escapeHtml(item.whatCannotBeConfirmed)}</span><strong>Recommended next photo / verification</strong><span>${escapeHtml(item.recommendedNextPhotoVerification)}</span></article>`).join(''):'<p class="condition-empty">No distinct item relationship could be established.</p>';
      const relationshipLabel=`${relationship.relationshipDiagnosticStatus||'INDETERMINATE'} — ${(relationship.relationshipOutcome||'INSUFFICIENT_VISIBLE_EVIDENCE').replaceAll('_',' ')}`;
      const section=document.createElement('section');section.className='vehicle-area-relationship';section.innerHTML=`<h3>VEHICLE-AREA &amp; COMPONENT RELATIONSHIP</h3><div class="condition-field"><strong>Relationship result</strong><span>${escapeHtml(relationshipLabel)}</span></div>${result.vehicleContextApplied?.available?`<div class="condition-field"><strong>Vehicle context used</strong><span>${escapeHtml(result.vehicleContextApplied.summary||'Active repair-order vehicle context')}</span></div>`:''}<div class="condition-field"><strong>Vehicle-area location</strong><span>${escapeHtml(relationship.vehicleAreaLocation)}</span></div><div class="condition-field"><strong>Location confidence</strong><span class="phase2-confidence">${escapeHtml(pct(relationship.locationConfidence))}</span></div><div class="condition-field"><strong>Primary visible assembly</strong><span>${escapeHtml(relationship.primaryVisibleAssembly)}</span></div><div class="condition-field"><strong>Location evidence</strong>${list(evidenceLedger?.locationEvidence||relationship.locationEvidence)}</div><div class="condition-field"><strong>Additional relationship evidence</strong>${list(evidenceLedger?.relationshipEvidence||[])}</div><div class="condition-field"><strong>Vehicle-context support</strong>${list(relationship.vehicleContextSupport)}</div><div class="condition-field"><strong>Observed physical relationships</strong><div class="condition-finding-cards">${items}</div></div><div class="condition-field"><strong>What prevents confirmation</strong><span>${escapeHtml(relationship.whatPreventsConfirmation||'Current image context is limited.')}</span></div><div class="condition-field"><strong>Recommended next photo / verification</strong><span>${escapeHtml(relationship.recommendedNextPhotoVerification||'Take a wider, well-lit image that includes the item and surrounding mounting area.')}</span></div>`;host.appendChild(section);
    ...
    if(visualCondition){
      const inspectionUnavailable=visualCondition.status==='UNABLE_TO_INSPECT'||visualCondition.status==='FAILED',overallInspectionConfidence=visualCondition.conditionConfidence===null?'Not provided':`${visualCondition.conditionConfidence}%`,section=document.createElement('section');section.className='visual-condition-inspection';
      const relationshipObserved=Boolean(result.vehicleAreaRelationshipAnalysis?.observedItems?.length),connectionList=visualCondition.connectionAssessments?.length?'<p>Ranked findings and their direct evidence are shown once in the whole-image defect-prioritization section above.</p>':`<p class="condition-empty">${inspectionUnavailable?'No connection assessment was returned because the visual inspection could not complete.':relationshipObserved?'Possible item relationship observed; exact connection and destination cannot be confirmed from this image.':'No connection could be reliably assessed.'}</p>`;
      ...
      section.innerHTML=`<h3>VISUAL CONDITION INSPECTION</h3><div class="condition-field"><strong>Inspection status</strong><span>${escapeHtml(inspectionStatus)}</span></div><div class="condition-field"><strong>Overall visual-inspection confidence</strong><span class="phase2-confidence">${escapeHtml(overallInspectionConfidence)}</span></div>${visualCondition.confidenceCalibration?`<div class="condition-field"><strong>Confidence calibration</strong><span>${escapeHtml(String(visualCondition.confidenceCalibration.reason||'UNCHANGED_BY_AVAILABLE_EVIDENCE').replaceAll('_',' '))}</span></div>`:''}<div class="condition-field"><strong>Findings</strong>${connectionList}</div>${visualCondition.status==='NO_VISIBLE_CONCERN_DETECTED'?`<div class="condition-field"><strong>Result</strong><p>${escapeHtml(visualCondition.noVisibleConcernMessage||'No visible defect can be confirmed from this image. Inspect the component physically before making a repair decision.')}</p></div>`:''}${visualCondition.status==='UNABLE_TO_INSPECT'||visualCondition.status==='FAILED'?`<div class="condition-field"><strong>What cannot be confirmed</strong><span>${escapeHtml(visualCondition.unableToInspectReason||'A reliable visual assessment could not be completed.')}</span></div>`:''}`;
      host.appendChild(section);
    }
    const routingSection=document.createElement('section');routingSection.className='visual-routing-summary';routingSection.innerHTML=`<div class="condition-field"><strong>Detected category</strong><span>${escapeHtml(CATEGORY_LABELS[result.category]||result.category)}</span></div><div class="condition-field"><strong>Image/category routing confidence</strong><span class="phase2-confidence">${escapeHtml(routingConfidence)}</span></div><div class="condition-field"><strong>Observed objects</strong><span>${escapeHtml(result.objects?.join(', ')||'None reported')}</span></div><div class="condition-field"><strong>Analyzer evidence</strong><span>${escapeHtml((evidenceLedger?.analyzerEvidence||result.evidence).join('; ')||'None')}</span></div><div class="condition-field"><strong>Routing</strong><span>${escapeHtml(result.route)} — ${escapeHtml(result.routeResult?.status||'Not started')}</span></div><div class="condition-field"><strong>Fresh-result verification</strong><span>${freshVerification}</span></div>`;host.appendChild(routingSection);
  ...
  function hasElectricalPointerTarget(result){
    const evidence=[...(result?.evidence||[]),...(result?.automotiveEvidence||[]),...(result?.objects||[]),result?.componentIdentification?.primaryComponent||'',...(result?.componentIdentification?.secondaryComponents||[])].join(' ');
    const pointer=/\b(?:finger|hand|screwdriver|probe|pick|test lead|flashlight|arrow|pointer)\b.{0,100}\b(?:pointing|indicat|toward|at)\b|\b(?:pointing|indicat)\b.{0,100}\b(?:finger|hand|screwdriver|probe|pick|test lead|flashlight|arrow|pointer)\b/i.test(evidence);
    const electrical=/\b(?:abs|wheel[- ]?speed|sensor|connector|wiring|wire|harness|terminal|ground|actuator|solenoid|switch|fuse|relay|module|motor|electrical|electronic)\b/i.test(evidence);
    const wheelArea=/\b(?:wheel|suspension|axle|cv boot|brake|hub)\b/i.test(evidence);
