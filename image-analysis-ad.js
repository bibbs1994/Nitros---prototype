...
(()=>{'use strict';
  ...
  function normalizeVisualConditionInspection(raw,category,run){
    if(category!=='AUTOMOTIVE_COMPONENT_OR_VEHICLE')return null;
    if(!raw||typeof raw!=='object'||!['OBSERVED_CONDITION','POSSIBLE_CONCERN_DETECTED','NO_VISIBLE_CONCERN_DETECTED','UNABLE_TO_INSPECT','FAILED'].includes(raw.status))throw new Error('Visual condition inspection result is missing or invalid.');
  ...
  function normalizeVisionResult(raw,run){
    ...
    const componentIdentification=normalizeComponentIdentification(raw.componentIdentification,category,run),visualConditionInspection=normalizeVisualConditionInspection(raw.visualConditionInspection,category,run);
  ...
  function renderResult(run,result){
    ...
    if(component){
      ...
      section.innerHTML=`<h3>SPECIFIC COMPONENT IDENTIFICATION</h3><strong>Status:</strong> ${escapeHtml(component.status)}<br><strong>Visible component:</strong> ${escapeHtml(component.primaryComponent)}<br><strong>Confidence level:</strong> <span class="phase2-confidence">${escapeHtml(componentConfidence)}</span><br><strong>System:</strong> ${escapeHtml(component.system||'Not determined')}<br><strong>Visible defining evidence:</strong>${list(component.supportingEvidence)}<strong>Likely connection or destination (not confirmed):</strong>${list(component.likelyConnectionsOrDestinations)}<strong>Secondary visible components:</strong>${list(component.secondaryComponents)}<strong>Possible identification:</strong>${list(component.possibleAlternatives)}${component.uncertaintyReason?`<strong>What cannot be confirmed:</strong> ${escapeHtml(component.uncertaintyReason)}`:''}`;
    ...
    if(visualCondition){
      ...
      const concernList=visualCondition.possibleConcerns?.length?`<ul>${visualCondition.possibleConcerns.map(concern=>`<li><strong>Location:</strong> ${escapeHtml(concern.location)}<br><strong>What appears abnormal:</strong> ${escapeHtml(concern.appearance)}<br><strong>Recommended verification:</strong> ${escapeHtml(concern.recommendedVerification)}<br><em>Confirmation by physical inspection is required.</em></li>`).join('')}</ul>`:`<p class="condition-empty">${escapeHtml(emptyText)}</p>`;
      const confidence=visualCondition.conditionConfidence===null?'Not provided':`${visualCondition.conditionConfidence}%`,section=document.createElement('section');section.className='visual-condition-inspection';
      const connectionList=visualCondition.connectionAssessments?.length?`<ul>${visualCondition.connectionAssessments.map(item=>`<li><strong>Location:</strong> ${escapeHtml(item.location)}<br><strong>Finding:</strong> ${escapeHtml(item.findingType.replaceAll('_',' '))}<br><strong>Severity:</strong> ${escapeHtml(item.severity)}<br><strong>Finding confidence:</strong> ${escapeHtml(`${item.findingConfidence}%`)}<br><strong>Seating assessment:</strong> ${escapeHtml(item.seatingStatus.replaceAll('_',' '))}<br><strong>Visible evidence:</strong> ${escapeHtml(item.visibleEvidence)}<br><strong>Physical verification:</strong> ${escapeHtml(item.recommendedVerification)}${item.safetyDrivabilityImpact?`<br><strong>Safety / drivability impact:</strong> ${escapeHtml(item.safetyDrivabilityImpact)}`:''}<br><em>Physical inspection is required before final repair authorization.</em></li>`).join('')}</ul>`:`<p class="condition-empty">${inspectionUnavailable?'No connection assessment was returned because the visual inspection could not complete.':'No connection could be reliably assessed.'}</p>`;
      section.innerHTML=`<h3>VISUAL CONDITION INSPECTION</h3><div class="condition-field"><strong>Inspection status</strong><span>${escapeHtml(visualCondition.status.replaceAll('_',' '))}</span></div><div class="condition-field"><strong>Confidence level</strong><span class="phase2-confidence">${escapeHtml(confidence)}</span></div><div class="condition-field"><strong>Visible observations</strong>${list([...visualCondition.visibleEvidence,...visualCondition.observedCondition])}</div><div class="condition-field"><strong>Possible concern</strong>${concernList}</div><div class="condition-field"><strong>Connection assessment</strong>${connectionList}</div>${visualCondition.status==='NO_VISIBLE_CONCERN_DETECTED'?`<div class="condition-field"><strong>Result</strong><p>${escapeHtml(visualCondition.noVisibleConcernMessage||'No visible defect can be confirmed from this image. Inspect the component physically before making a repair decision.')}</p></div>`:''}${visualCondition.status==='UNABLE_TO_INSPECT'||visualCondition.status==='FAILED'?`<div class="condition-field"><strong>What cannot be confirmed</strong><span>${escapeHtml(visualCondition.unableToInspectReason||'A reliable visual assessment could not be completed.')}</span></div>`:''}<div class="condition-field"><strong>Recommended technician verification</strong>${list(visualCondition.recommendedVerification)}</div>${visualCondition.safetyDrivabilityImpact?`<div class="condition-field"><strong>Safety / drivability impact</strong><span>${escapeHtml(visualCondition.safetyDrivabilityImpact)}</span></div>`:''}`;
      host.appendChild(section);
    }
