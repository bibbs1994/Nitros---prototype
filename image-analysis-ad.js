...
(()=>{'use strict';
  ...
  function normalizeVisualConditionInspection(raw,category,run){
    ...
    const connectionAssessments=Array.isArray(raw.connectionAssessments)?raw.connectionAssessments.map(item=>({location:String(item?.location||'').trim(),seatingStatus:String(item?.seatingStatus||'NOT_RELIABLY_VISIBLE').trim(),visibleEvidence:String(item?.visibleEvidence||'').trim()})).filter(item=>item.location&&['SEPARATION_OR_GAP_VISIBLE','POSSIBLE_IMPROPER_SEATING','NO_GAP_OR_SEPARATION_VISIBLE','NOT_RELIABLY_VISIBLE'].includes(item.seatingStatus)&&item.visibleEvidence).slice(0,12):[];
    if(raw.status==='NO_VISIBLE_CONCERN_DETECTED'&&(!connectionAssessments.length||connectionAssessments.some(item=>item.seatingStatus!=='NO_GAP_OR_SEPARATION_VISIBLE')))throw new Error('No-visible-concern condition result lacks confirmed visible connection seating.');
  ...
  function renderResult(run,result){
    ...
    if(visualCondition){
      ...
      const connectionList=visualCondition.connectionAssessments?.length?`<ul>${visualCondition.connectionAssessments.map(item=>`<li><strong>Location:</strong> ${escapeHtml(item.location)}<br><strong>Seating assessment:</strong> ${escapeHtml(item.seatingStatus.replaceAll('_',' '))}<br><strong>Visible evidence:</strong> ${escapeHtml(item.visibleEvidence)}</li>`).join('')}</ul>`:'<p class="condition-empty">No connection could be reliably assessed.</p>';
