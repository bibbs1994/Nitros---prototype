...
(()=>{'use strict';
  ...
  function finalizeRenderedNumericEvidence(graph){
    ...
    const signed=(value,unit)=>`${value>0?'+':''}${value}${unit||''}`,rawRows=Array.isArray(graph.numericEvidence)?graph.numericEvidence:[];
    const finalCanonicalPidEvidence=Object.freeze(rawRows.map(source=>{
      const current=Number(source.current),minimum=Number(source.minimum),maximum=Number(source.maximum),complete=source.currentPresent!==false&&source.minimumPresent!==false&&source.maximumPresent!==false&&source.current!==null&&source.minimum!==null&&source.maximum!==null,finite=complete&&[current,minimum,maximum].every(Number.isFinite);
      let invariantResult='NOT_APPLICABLE',violationReason='';
      if(complete){if(!finite){invariantResult='FAIL';violationReason='Current, Min, and Max must all be numeric.'}else if(minimum>maximum){invariantResult='FAIL';violationReason='Min exceeds displayed Max.'}else if(current<minimum){invariantResult='FAIL';violationReason='Current is below displayed Min.'}else if(current>maximum){invariantResult='FAIL';violationReason='Current exceeds displayed Max.'}else invariantResult='PASS'}
      const unit=String(source.unit||'');return Object.freeze({pidName:String(source.pidName||'Unknown PID'),current:finite?current:null,minimum:finite?minimum:null,maximum:finite?maximum:null,unit,displayCurrent:finite?signed(current,unit):'unavailable',displayMinimum:finite?signed(minimum,unit):'unavailable',displayMaximum:finite?signed(maximum,unit):'unavailable',invariantResult,violationReason});
    })),failed=finalCanonicalPidEvidence.filter(row=>row.invariantResult==='FAIL'),finalRenderValidationStatus=failed.length?'FAIL':'PASS';
    const result={...graph,finalCanonicalPidEvidence,finalRenderValidationStatus,numericValidation:{...(graph.numericValidation||{}),finalRenderValidationStatus,currentMinMaxConsistency:failed.length?'FAIL':graph.numericValidation?.currentMinMaxConsistency||'PASS',invalidPidEvidence:failed.map(row=>row.pidName)},freshResultVerification:failed.length?'FAIL':graph.freshResultVerification};
    if(failed.length){const names=failed.map(row=>row.pidName),namePattern=new RegExp(names.map(name=>name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').replace('Long FT #1','(?:Long FT #1|LTFT(?: B1)?)').replace('Short FT #1','(?:Short FT #1|STFT(?: B1)?)')).join('|'),'i'),first=failed[0];result.interpretation=(graph.interpretation||[]).filter(item=>!namePattern.test(item));result.traceFindings=(graph.traceFindings||[]).filter(item=>!namePattern.test(item));result.diagnosticSignificance='INDETERMINATE';result.diagnosticSignificanceReason='NUMERIC_EVIDENCE_INCONSISTENT';result.semanticConsistencyStatus='FAIL_NUMERIC_EVIDENCE';result.nextTest=[`Reacquire ${names.join(', ')} data.`];result.nextTestReason=`${first.pidName}: ${first.violationReason} The existing numeric evidence is internally inconsistent and cannot support diagnostic interpretation.`;result.nextTestSelection='TARGETED_INVALID_PID_REACQUISITION'}
    return result;
  }
