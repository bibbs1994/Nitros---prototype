...
(()=>{'use strict';
  ...
  function renderedNumberText(value,unit,precision){if(!Number.isFinite(value))return'unavailable';return`${value>0?'+':''}${value.toFixed(precision)}${unit}`}
  function renderedTripletValidation(row){const finiteNumbers=[row.currentNumeric,row.minNumeric,row.maxNumeric].every(Number.isFinite),minimumLessThanOrEqualMaximum=finiteNumbers&&row.minNumeric<=row.maxNumeric,minimumLessThanOrEqualCurrent=finiteNumbers&&row.minNumeric<=row.currentNumeric,currentLessThanOrEqualMaximum=finiteNumbers&&row.currentNumeric<=row.maxNumeric,chainPass=minimumLessThanOrEqualCurrent&&currentLessThanOrEqualMaximum,textMatchesNumeric=row.currentText===renderedNumberText(row.currentNumeric,row.unit,row.precision)&&row.minText===renderedNumberText(row.minNumeric,row.unit,row.precision)&&row.maxText===renderedNumberText(row.maxNumeric,row.unit,row.precision);let invariantFailureReason='';if(!finiteNumbers)invariantFailureReason='Current, Min, or Max is unavailable.';else if(!textMatchesNumeric)invariantFailureReason='Rendered numeric text does not match its authoritative numeric source.';else if(!minimumLessThanOrEqualMaximum)invariantFailureReason='Min exceeds Max.';else if(!minimumLessThanOrEqualCurrent)invariantFailureReason='Current is below Min.';else if(!currentLessThanOrEqualMaximum)invariantFailureReason='Current exceeds Max.';return{finiteNumbers,minimumLessThanOrEqualMaximum,minimumLessThanOrEqualCurrent,currentLessThanOrEqualMaximum,chainPass,textMatchesNumeric,invariantPass:!invariantFailureReason,invariantFailureReason}}
  function createRenderedNumericTriplet(source){const unit=String(source?.unit||''),precision=Number.isInteger(source?.precision)?source.precision:unit.toUpperCase()==='RPM'?0:3,round=value=>Number.isFinite(value)?Number(value.toFixed(precision)):null,currentNumeric=round(canonicalSourceNumber(source?.currentNumeric,source?.current,source?.rawCurrent)),minNumeric=round(canonicalSourceNumber(source?.minNumeric,source?.minimum,source?.numericRange?.minimum,source?.rawMinimum)),maxNumeric=round(canonicalSourceNumber(source?.maxNumeric,source?.maximum,source?.numericRange?.maximum,source?.rawMaximum)),base={pidName:String(source?.pidName||'Unknown PID'),unit,precision,currentNumeric,minNumeric,maxNumeric,currentText:renderedNumberText(currentNumeric,unit,precision),minText:renderedNumberText(minNumeric,unit,precision),maxText:renderedNumberText(maxNumeric,unit,precision),sourceMetadata:Object.freeze({sourceField:String(source?.sourceField||'FINAL_NORMALIZED_PID_CURRENT_MIN_MAX'),confidence:source?.confidence??null})},validation=renderedTripletValidation(base);return Object.freeze({...base,...validation,currentDisplay:base.currentText,minDisplay:base.minText,maxDisplay:base.maxText,parsedCurrent:currentNumeric,parsedMin:minNumeric,parsedMax:maxNumeric,displayCurrent:base.currentText,displayMinimum:base.minText,displayMaximum:base.maxText,current:currentNumeric,minimum:minNumeric,maximum:maxNumeric,invariantResult:validation.invariantPass?'PASS':'FAIL',violationReason:validation.invariantFailureReason})}
  function validateFinalRenderedPid(row){return renderedTripletValidation(row)}
  function assertFinalRenderedPidEvidence(rows){const renderedPidEvidence=Array.isArray(rows)?rows:[],failed=renderedPidEvidence.filter(row=>!renderedTripletValidation(row).invariantPass);return Object.freeze({renderedPidEvidence,failed,status:failed.length?'FAIL':'PASS'})}
  function finalizeRenderedNumericEvidence(graph){
    if(!graph||typeof graph!=='object')return graph;
    const rawRows=Array.isArray(graph.numericEvidence)?graph.numericEvidence:[],renderedPidEvidence=Object.freeze(rawRows.map(createRenderedNumericTriplet)),assertion=assertFinalRenderedPidEvidence(renderedPidEvidence),finalCanonicalPidEvidence=renderedPidEvidence,failed=assertion.failed,finalRenderValidationStatus=assertion.status;
    const renderedInvariantLog=Object.freeze(renderedPidEvidence.map(row=>Object.freeze({pidName:row.pidName,currentNumeric:row.currentNumeric,minNumeric:row.minNumeric,maxNumeric:row.maxNumeric,currentText:row.currentText,minText:row.minText,maxText:row.maxText,unit:row.unit,finiteNumberValidation:row.finiteNumbers?'PASS':'FAIL',minimumLessThanOrEqualMaximum:row.minimumLessThanOrEqualMaximum?'PASS':'FAIL',minimumLessThanOrEqualCurrentAndCurrentLessThanOrEqualMaximum:row.chainPass?'PASS':'FAIL',renderedInvariant:row.invariantResult})));
    const result={...graph,renderedPidEvidence,finalCanonicalPidEvidence,renderedInvariantLog,finalRenderValidationStatus,numericValidation:{...(graph.numericValidation||{}),validationStage:'SHARED_RENDERED_NUMERIC_TRIPLET_HARD_GATE',authoritativeSource:'SHARED_IMMUTABLE_RENDERED_NUMERIC_TRIPLET',finalRenderValidationStatus,currentMinMaxConsistency:failed.length?'FAIL':graph.numericValidation?.currentMinMaxConsistency||'PASS',invalidPidEvidence:failed.map(row=>row.pidName)},freshResultVerification:failed.length?'FAIL':graph.freshResultVerification};
    if(failed.length){const names=failed.map(row=>row.pidName),namePattern=new RegExp(names.map(name=>name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').replace('Long FT #1','(?:Long FT #1|LTFT(?: B1)?)').replace('Short FT #1','(?:Short FT #1|STFT(?: B1)?)')).join('|'),'i'),first=failed[0],conflicts=failed.map(row=>`${row.pidName} displays Current ${row.displayCurrent}, Min ${row.displayMinimum}, and Max ${row.displayMaximum}; ${row.violationReason}`).join(' ');result.interpretation=(graph.interpretation||[]).filter(item=>!namePattern.test(item));result.interpretation.unshift(`Numeric evidence is internally inconsistent. ${conflicts} Interpretation of the conflicting PID evidence is indeterminate.`);result.traceFindings=(graph.traceFindings||[]).filter(item=>!namePattern.test(item));result.diagnosticSignificance='INDETERMINATE';result.diagnosticSignificanceReason='RENDERED_NUMERIC_EVIDENCE_INCONSISTENT';result.semanticConsistencyStatus='FAIL_NUMERIC_EVIDENCE';result.nextTest=['Acquire Fuel System Status / Closed Loop Status to verify whether the engine was operating in closed loop when the PID evidence was captured.'];result.nextTestReason=`${first.pidName}: ${first.violationReason} Closed-loop status is the next missing evidence required before reliable fuel-trim interpretation can continue.`;result.nextTestSelection='MISSING_CLOSED_LOOP_STATUS';result.unresolvedQuestion='Was the engine operating in closed loop when this evidence was captured?'}
    return result;
  }
  ...
  function normalizeVisionResult(raw,run){
    ...
    const wiringDiagramAnalysis=normalizeWiringDiagram(raw.wiringDiagramAnalysis,category,run),documentRepairInformation=normalizeDocumentRepairInformation(raw.documentRepairInformation,category,run),automotiveGraphAnalysis=category==='AUTOMOTIVE_GRAPH'&&raw.automotiveGraphAnalysis?finalizeRenderedNumericEvidence({...raw.automotiveGraphAnalysis}):null;
  ...
  async function routeFreshResult(run,result){
    ...
    if(result.category==='AUTOMOTIVE_GRAPH'){
      ...
      const graph=result.automotiveGraphAnalysis,verified=graph.status!=='FAILED'&&graph.finalRenderValidationStatus==='PASS'&&graph.freshResultVerification==='PASS'&&graph.semanticConsistencyStatus!=='FAIL';
      result.routeResult={status:verified?'Diagnostic interpretation complete':'Analysis withheld pending evidence reconciliation',evidence:[...graph.observed],freshResultVerification:verified?'PASS':'FAIL'};
  ...
  function renderResult(run,result){
    ...
    const freshVerification=result.category==='UNKNOWN_OR_ANALYSIS_UNAVAILABLE'?'FAIL':result.category==='AUTOMOTIVE_GRAPH'?(result.routeResult?.freshResultVerification||'FAIL'):'PASS';host.innerHTML=`<strong>Detected category:</strong> ${escapeHtml(CATEGORY_LABELS[result.category]||result.category)}<br><strong>Confidence:</strong> <span class="phase2-confidence">${escapeHtml(confidence)}</span><br><strong>Observed objects:</strong> ${escapeHtml(result.objects?.join(', ')||'None reported')}<br><strong>Analyzer evidence:</strong> ${escapeHtml(result.evidence.join('; ')||'None')}<br><strong>Routing:</strong> ${escapeHtml(result.route)} — ${escapeHtml(result.routeResult?.status||'Not started')}<br><strong>Fresh-result verification:</strong> ${freshVerification}`;
    ...
    if(graph){const section=document.createElement('section'),list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p>None required or reliably readable.</p>',heading=graph.analysisMode==='PID_SNAPSHOT'?'AUTOMOTIVE PID SNAPSHOT ANALYSIS':'AUTOMOTIVE PID GRAPH ANALYSIS',postRenderAssertion=assertFinalRenderedPidEvidence(graph.renderedPidEvidence||graph.finalCanonicalPidEvidence||[]),rows=postRenderAssertion.renderedPidEvidence,failed=postRenderAssertion.failed,numericFailure=postRenderAssertion.status==='FAIL';if(numericFailure&&graph.finalRenderValidationStatus==='PASS'){console.error('POST_RENDER_INVARIANT_BLOCKED_FALSE_PASS',failed.map(row=>({pidName:row.pidName,currentNumeric:row.currentNumeric,minNumeric:row.minNumeric,maxNumeric:row.maxNumeric,currentText:row.currentText,minText:row.minText,maxText:row.maxText,finiteNumbers:row.finiteNumbers,minimumLessThanOrEqualCurrent:row.minimumLessThanOrEqualCurrent,currentLessThanOrEqualMaximum:row.currentLessThanOrEqualMaximum,minimumLessThanOrEqualMaximum:row.minimumLessThanOrEqualMaximum})));window.NitrosDeveloperMode=window.NitrosDeveloperMode||{};window.NitrosDeveloperMode.postRenderInvariantAssertion={status:'FAIL',failed}}const numericRows=rows.length?`<ul>${rows.map(row=>`<li><strong>${escapeHtml(row.pidName)} — ${escapeHtml(row.invariantResult)}</strong>: Current ${escapeHtml(row.currentText)}, Min ${escapeHtml(row.minText)}, Max ${escapeHtml(row.maxText)}${row.invariantFailureReason?`. ${escapeHtml(row.invariantFailureReason)}`:''}</li>`).join('')}</ul>`:'',failures=failed.length?`<strong>Numeric Evidence Consistency: FAIL</strong>${numericRows}<p><strong>${escapeHtml(failed.map(row=>`${row.pidName} — FAIL: Current ${row.currentText}, Min ${row.minText}, Max ${row.maxText}. Violates Min <= Current <= Max. ${row.invariantFailureReason} Diagnostic interpretation of this PID is withheld until valid evidence is available.`).join(' '))}</strong></p>`:`<strong>Numeric Evidence Consistency: PASS</strong>${numericRows}`,safeInterpretation=numericFailure?[]:graph.interpretation,safeTraceFindings=numericFailure?[]:graph.traceFindings;section.className='automotive-graph-analysis';section.innerHTML=`<h3>${heading}</h3>${failures}<strong>Observed:</strong>${list(graph.observed)}${graph.analysisMode!=='PID_SNAPSHOT'&&safeTraceFindings?.length?`<strong>Trace Behavior:</strong>${list(safeTraceFindings)}`:''}<strong>Interpretation:</strong>${list(safeInterpretation)}<strong>Diagnostic Significance:</strong><p>${numericFailure?'Undetermined — numeric evidence inconsistent':escapeHtml(String(graph.diagnosticSignificance||'INCONCLUSIVE').replaceAll('_',' ').toLowerCase())}</p><strong>Next Test:</strong>${list(graph.nextTest)}${graph.nextTestReason?`<strong>Why:</strong><p>${escapeHtml(graph.nextTestReason)}</p>`:''}${graph.unreadableOrUncertain?.length?`<strong>Unreadable / Uncertain:</strong>${list(graph.unreadableOrUncertain)}`:''}`;host.appendChild(section)}
