...
(()=>{'use strict';
  ...
  function finalizeRenderedNumericEvidence(graph){
    ...
    const renderedInvariantLog=Object.freeze(renderedPidEvidence.map(row=>Object.freeze({pidName:row.pidName,rawCurrentCandidate:row.sourceMetadata.rawCurrentCandidate,rawMinCandidate:row.sourceMetadata.rawMinCandidate,rawMaxCandidate:row.sourceMetadata.rawMaxCandidate,boundCurrent:row.sourceMetadata.boundCurrent,boundMin:row.sourceMetadata.boundMin,boundMax:row.sourceMetadata.boundMax,currentNumeric:row.currentNumeric,minNumeric:row.minNumeric,maxNumeric:row.maxNumeric,currentText:row.currentText,minText:row.minText,maxText:row.maxText,unit:row.unit,parseStatus:row.sourceMetadata.parseStatus,bindingIdentifier:row.sourceMetadata.bindingIdentifier,rejectedCandidates:row.sourceMetadata.rejectedCandidates,finiteNumberValidation:row.finiteNumbers?'PASS':'FAIL',minimumLessThanOrEqualMaximum:row.minimumLessThanOrEqualMaximum?'PASS':'FAIL',minimumLessThanOrEqualCurrentAndCurrentLessThanOrEqualMaximum:row.chainPass?'PASS':'FAIL',renderedInvariant:row.invariantResult})));
