...
(()=>{'use strict';
  ...
  function renderResult(run,result){
    ...
    if(graph){const section=document.createElement('section'),list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p>None required or reliably readable.</p>',heading=graph.analysisMode==='PID_SNAPSHOT'?'AUTOMOTIVE PID SNAPSHOT ANALYSIS':'AUTOMOTIVE PID GRAPH ANALYSIS',failures=graph.evidenceConsistencyFailures?.length?`<strong>Evidence Consistency Failure:</strong><ul>${graph.evidenceConsistencyFailures.map(item=>`<li>${escapeHtml(item.pidName)} — Current: ${escapeHtml(item.current)}${escapeHtml(item.unit)}, Min: ${escapeHtml(item.minimum)}${escapeHtml(item.unit)}, Max: ${escapeHtml(item.maximum)}${escapeHtml(item.unit)}. Failure: ${escapeHtml(item.failure)}</li>`).join('')}</ul>`:'';section.className='automotive-graph-analysis';section.innerHTML=`<h3>${heading}</h3>${failures}<strong>Observed:</strong>${list(graph.observed)}${graph.analysisMode!=='PID_SNAPSHOT'?`<strong>Trace Behavior:</strong>${list(graph.traceFindings)}`:''}<strong>Interpretation:</strong>${list(graph.interpretation)}<strong>Diagnostic Significance:</strong><p>${graph.diagnosticSignificanceReason==='NUMERIC_EVIDENCE_INCONSISTENCY_DETECTED'?'Indeterminate — numeric evidence inconsistency detected.':escapeHtml(String(graph.diagnosticSignificance||'INCONCLUSIVE').replaceAll('_',' ').toLowerCase())}</p><strong>Next Test:</strong>${list(graph.nextTest)}${graph.nextTestReason?`<strong>Why:</strong><p>${escapeHtml(graph.nextTestReason)}</p>`:''}${graph.unreadableOrUncertain?.length?`<strong>Unreadable / Uncertain:</strong>${list(graph.unreadableOrUncertain)}`:''}`;host.appendChild(section)}
