...
(()=>{'use strict';
  ...
  function renderResult(run,result){
    ...
    if(visualCondition){
      const list=items=>items?.length?`<ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:'<p class="condition-empty">None</p>';
      const concernList=visualCondition.possibleConcerns?.length?`<ul>${visualCondition.possibleConcerns.map(concern=>`<li><strong>Location:</strong> ${escapeHtml(concern.location)}<br><strong>What appears abnormal:</strong> ${escapeHtml(concern.appearance)}<br><strong>Recommended verification:</strong> ${escapeHtml(concern.recommendedVerification)}<br><em>Confirmation by physical inspection is required.</em></li>`).join('')}</ul>`:'<p class="condition-empty">None</p>';
