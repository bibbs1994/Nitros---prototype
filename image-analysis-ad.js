...
(()=>{'use strict';
  ...
  function normalizeDocumentRepairInformation(raw,category,run){
    ...
    const applicability=['APPLICABLE','NOT APPLICABLE','UNKNOWN / CANNOT DETERMINE'].includes(raw.dtcApplicability)?raw.dtcApplicability:'',visibleTextEvidence=Array.isArray(raw.visibleTextEvidence)?raw.visibleTextEvidence.map(String).filter(Boolean).slice(0,24):[],claimedCriterion=text('criterion',300),criterionEvidence=text('criterionEvidence',500)||visibleTextEvidence.find(item=>claimedCriterion&&item.toLowerCase().includes(claimedCriterion.toLowerCase()))||'',criterionNumbers=claimedCriterion.match(/\d+(?:\.\d+)?/g)||[],evidenceNumbers=criterionEvidence.match(/\d+(?:\.\d+)?/g)||[],criterionEvidenceVisible=!!criterionEvidence&&visibleTextEvidence.some(item=>item.toLowerCase().includes(criterionEvidence.toLowerCase())||criterionEvidence.toLowerCase().includes(item.toLowerCase())),criterionGrounded=criterionEvidenceVisible&&criterionNumbers.every(number=>evidenceNumbers.includes(number));
