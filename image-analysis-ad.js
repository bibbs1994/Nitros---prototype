...
(()=>{'use strict';
  ...
  function activeVehicleAnalysisContext(){
    ...
    const vehicle={year:/^\d{4}$/.test(year)?year:'',make:clean(context.make,80),model:clean(context.model,100),engine:clean(context.engine,100),configuration:clean(context.configuration||context.vehicleConfiguration,180),vin:/^[A-HJ-NPR-Z0-9]{17}$/.test(vin)?vin:''};
  ...
  function normalizeVisionResult(raw,run){
    ...
    const vehicleContextApplied=raw.vehicleContextApplied&&typeof raw.vehicleContextApplied==='object'?{available:raw.vehicleContextApplied.available===true,summary:String(raw.vehicleContextApplied.summary||'').trim()}:null;
    return {runId:run.runId,semanticRequestId:raw.transactionId,imageHash:raw.imageHash,category,confidence,rawConfidence:raw.rawConfidence??null,normalizedConfidence:confidence,componentIdentification,visualConditionInspection,automotiveGraphAnalysis,wiringDiagramAnalysis,documentRepairInformation,objects,evidence,description:String(raw.description||'').trim(),automotiveEvidence,graphEvidence,documentEvidence,vehicleContextApplied,source:String(raw.source||'NitrosVisionAnalyzer semantic result'),transportStatus:raw.transportStatus??null,routingData:raw.routingData??null};
  }
