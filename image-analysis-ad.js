...
const BUILD='10.13.133';
  ...
  function renderResult(run,result){
    ...
    if(defectFirst){
      ...
      const list=items=>items.length?`<ul>${items.map(item=>`<li><strong>${escapeHtml(item.location)}</strong> — ${escapeHtml(observation(item))}${item.recommendedVerification?` Recommendation: ${escapeHtml(item.recommendedVerification)}`:''}</li>`).join('')}</ul>`:'<p class="condition-empty">None</p>';
