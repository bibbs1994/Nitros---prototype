...
export function buildUsageEvent({ body = {}, result, error, now = new Date() } = {}) {
  ...
  return {
    ...
    estimatedCostUsd: error ? null : estimateCost({ model, inputTokens, cachedInputTokens, outputTokens, serviceTier: telemetry.serviceTier, longContext: telemetry.longContext }), actualProviderCostUsd: null,
...
export class UsageLedgerRepository {
  ...
  async report() { const data = await this.data(), events = data.events.slice().sort((a,b) => b.timestamp.localeCompare(a.timestamp)); const now = new Date(), day = now.toISOString().slice(0,10), month = day.slice(0,7), week = new Date(now - 6*864e5).toISOString().slice(0,10); const cost = e => Number.isFinite(e.estimatedCostUsd) ? e.estimatedCostUsd : null;
    const scope = predicate => events.filter(predicate), summary = list => { const priced=list.filter(e=>cost(e)!==null), successful=list.filter(e=>e.status==='SUCCEEDED'), failed=list.filter(e=>e.status==='FAILED'), photos=list.filter(e=>e.operationCategory==='photo_inspection'), estimatedSpend=priced.reduce((n,e)=>n+cost(e),0), unavailableCostCount=list.length-priced.length; return { estimatedSpend, estimatedCostUnavailableCount:unavailableCostCount, requests:list.length, successfulRequests:successful.length, failedRequests:failed.length, photos:photos.length, averageRequest:priced.length?estimatedSpend/priced.length:null, averagePhotoInspection:priced.filter(e=>e.operationCategory==='photo_inspection').length?estimatedSpend/priced.filter(e=>e.operationCategory==='photo_inspection').length:null, averageCase:null }; };
    ...
    const group = key => Object.entries(events.reduce((map,e)=>{const k=(key === 'day' ? e.timestamp.slice(0,10) : e[key]) || 'Unattributed';(map[k]??=[]).push(e);return map;},{})).map(([name,list])=>({name,...summary(list)})).sort((a,b)=>b.estimatedSpend-a.estimatedSpend);
    const settings={ monthlyBudgetUsd: 40, warningPercent: 70, criticalPercent: 90, hardStopEnabled: false, ...data.settings }; const used=thisMonth.estimatedCostUnavailableCount ? null : thisMonth.estimatedSpend, remaining=used===null?null:Math.max(0,settings.monthlyBudgetUsd-used), elapsed=Math.max(1,now.getDate()), projected=used===null||!thisMonth.requests?null:used/elapsed*new Date(now.getFullYear(),now.getMonth()+1,0).getDate();
    return { generatedAt:now.toISOString(), settings, current:{today,thisWeek,thisMonth,all}, budget:{used,remaining,consumedPercent:used===null||!settings.monthlyBudgetUsd?null:used/settings.monthlyBudgetUsd*100,projected}, byModel:group('model'), byOperation:group('operationCategory'), byCase:group('repairOrderId'), daily:group('day'), recent:events.slice(0,30), highestCost:events.slice().sort((a,b)=>(cost(b) ?? -1)-(cost(a) ?? -1)).slice(0,10) };
  }
